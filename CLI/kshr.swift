...
private final class CLISocketSentryTelemetry {
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["KSHR_BUNDLE_ID"]?
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = processEnv["KSHR_SOCKET_PATH"] ?? processEnv["KSHR_SOCKET"]
        self.workspaceId = processEnv["KSHR_WORKSPACE_ID"]
        self.surfaceId = processEnv["KSHR_SURFACE_ID"]
        self.disabledByEnv =
            processEnv["KSHR_CLI_SENTRY_DISABLED"] == "1" ||
            processEnv["KSHR_CLAUDE_HOOK_SENTRY_DISABLED"] == "1"
    }
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "kshr.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "kshr-cli", key: "component")
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        if !tmpSockets.isEmpty {
            context["tmp_kshr_sockets"] = tmpSockets
        }
        ...
        if CLISocketPathResolver.isImplicitDefaultPath(socketPath),
           ...
           !taggedSockets.isEmpty {
            context["possible_root_cause"] = "KSHR_SOCKET_PATH/KSHR_SOCKET missing while tagged sockets exist"
        }
    ...
    private static func discoverSockets(in directory: String, limit: Int) -> [String] {
        ...
        for name in entries.sorted() {
            guard name.hasPrefix("kshr"), name.hasSuffix(".sock") else { continue }
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.kshrterm/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["KSHR_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
enum SocketPasswordResolver {
    private static let service = "com.kshrterm.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "kshr"
    ...
    static func resolve(explicit: String?, socketPath: String) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["KSHR_SOCKET_PASSWORD"]) {
            return env
        }
    ...
    private static func keychainScope(
        ...
        if let tag = normalized(environment["KSHR_TAG"]) {
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "kshr"
    private static let stableSocketFileName = "kshr.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyDefaultSocketPath = "/tmp/kshr.sock"
    private static let fallbackSocketPath = "/tmp/kshr-debug.sock"
    private static let stagingSocketPath = "/tmp/kshr-staging.sock"
    private static let legacyLastSocketPathFile = "/tmp/kshr-last-socket-path"
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["KSHR_TAG"]) {
            ...
            candidates.append("/tmp/kshr-debug-\(slug).sock")
            candidates.append("/tmp/kshr-\(slug).sock")
        }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("kshr") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["KSHRTERM_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    private static func relayCredentials(for endpoint: RelayEndpoint) throws -> RelayCredentials {
        ...
        if let relayID = trimmedEnvValue(environment["KSHR_RELAY_ID"]),
           let relayTokenHex = trimmedEnvValue(environment["KSHR_RELAY_TOKEN"]),
        ...
        let authURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent(".kshr/relay/\(endpoint.port).auth", isDirectory: false)
    ...
    private func authenticateRelay(credentials: RelayCredentials) throws {
        ...
        guard let challengeData = challengeLine.data(using: .utf8),
              ...
              (challenge["protocol"] as? String) == "kshr-relay-auth",
    ...
    private func readLine(maxBytes: Int = 16 * 1024) throws -> String {
        // Set receive timeout once per message, not per byte.
        try configureReceiveTimeout(Self.responseTimeoutSeconds)

        var data = Data()
        // Use recv with MSG_PEEK on a larger buffer to find the newline,
        // then consume exactly up to and including it with a single read.
        var peekBuffer = [UInt8](repeating: 0, count: 4096)

        while data.count < maxBytes {
            let peekSize = min(peekBuffer.count, maxBytes - data.count)
            let peeked = Darwin.recv(socketFD, &peekBuffer, peekSize, MSG_PEEK)
            if peeked < 0 {
                if errno == EINTR {
                    continue
                }
            ...
            // Search for newline in peeked data.
            let consumeCount: Int
            var foundNewline = false
            if let nlIndex = peekBuffer[..<peeked].firstIndex(of: 0x0A) {
                // Consume up to and including the newline.
                consumeCount = (nlIndex - peekBuffer.startIndex) + 1
                foundNewline = true
            } else {
                // No newline yet — consume everything peeked.
                consumeCount = peeked
            }

            // Actually consume the bytes from the socket.
            var consumeBuffer = [UInt8](repeating: 0, count: consumeCount)
            var consumed = 0
            while consumed < consumeCount {
                let n = consumeBuffer.withUnsafeMutableBufferPointer { buf in
                    Darwin.read(socketFD, buf.baseAddress! + consumed, consumeCount - consumed)
                }
                if n < 0 {
            ...
            // Append data excluding the trailing newline if present.
            let appendCount = foundNewline ? consumed - 1 : consumed
            if appendCount > 0 {
                data.append(contentsOf: consumeBuffer[..<appendCount])
            }
            if foundNewline {
                break
            }
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "kshr app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.kshr.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.kshr.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct KSHRCLI {
    ...
    private static let debugLastSocketHintPath = "/tmp/kshr-last-socket-path"
    ...
    private static func debugSocketPathFromHintFile() -> String? {
        ...
        guard let hinted = normalizedEnvValue(raw),
              hinted.hasPrefix("/tmp/kshr-debug"),
    ...
    private static func defaultSocketPath(environment: [String: String]) -> String {
        if let explicit = normalizedEnvValue(environment["KSHR_SOCKET_PATH"]) {
            return explicit
        }
#if DEBUG
        ...
        return "/tmp/kshr-debug.sock"
#else
        return "/tmp/kshr.sock"
#endif
    }
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["KSHR_SOCKET_PATH", "KSHR_SOCKET"] {
        ...
        // so help text is available even when kshr is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'kshr help' to see available commands.")
            return
        }
        ...
        // Codex hook handler: gracefully no-op when not inside kshr
        // (before socket connection, so it doesn't fail when no socket exists)
        if command == "codex-hook" {
            guard ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] != nil else {
                print("{}")
                return
            }
        }
        ...
        case "rpc":
            guard let method = commandArgs.first?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !method.isEmpty else {
                throw CLIError(message: "Usage: kshr rpc <method> [json-params]")
            }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let callerWorkspaceArg = preferTTYFallback
                ? nil
                : (windowId == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
            ...
            let callerSurfaceArg = explicitSurfaceArg == nil && preferTTYFallback == false && windowId == nil
                ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"]
                : nil
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"],
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: kshr markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: kshr markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: kshr markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: kshr markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: kshr markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in kshr by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["KSHR_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["KSHR_TAB_ID"] ?? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"])
                : nil)
    ...
    private func runSSH(
        ...
        do {
            ...
            // `kshr ssh` is an explicit "open this remote workspace now" action,
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: kshr ssh user@host)")
        }
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        var lines: [String] = [
            "kshr_workspace_id=\"${KSHR_WORKSPACE_ID:-}\"",
            "kshr_surface_id=\"${KSHR_SURFACE_ID:-}\"",
            "kshr_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "kshr_remote_bootstrap=\"$(printf %s \"$kshr_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$kshr_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "kshr_remote_bootstrap=\"$(printf '%s' \"$kshr_remote_bootstrap\" | sed \"s/__KSHR_WORKSPACE_ID__/$kshr_workspace_id/g; s/__KSHR_SURFACE_ID__/$kshr_surface_id/g\")\"",
            "if ! printf '%s' \"$kshr_remote_bootstrap\" | command \(installSSHPrefix) -T \(shellQuote(options.destination)) \(shellQuote(remoteBootstrapInstallCommand)); then",
            "  exit 1",
            "fi",
            "kshr_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "kshr_remote_command=\"$(printf '%s' \"$kshr_remote_command_template\" | sed \"s/__KSHR_WORKSPACE_ID__/$kshr_workspace_id/g; s/__KSHR_SURFACE_ID__/$kshr_surface_id/g\")\"",
        ]

        var sshInvocation = "command \(sessionSSHPrefix) -o \"RemoteCommand=$kshr_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    private func stagedRemoteBootstrapCommandShell(
        ...
        lines.append("/bin/sh \"$HOME/.kshr/relay/\(remoteRelayPort).bootstrap.sh\"")
        return lines.joined(separator: "\n")
    }
    ...
    private func remoteBootstrapInstallShell(remoteRelayPort: Int) -> String {
        [
            "set -eu",
            "umask 077",
            "kshr_bootstrap_path=\"$HOME/.kshr/relay/\(remoteRelayPort).bootstrap.sh\"",
            "mkdir -p \"$HOME/.kshr/relay\"",
            "cat > \"$kshr_bootstrap_path\"",
            "chmod 700 \"$kshr_bootstrap_path\" >/dev/null 2>&1 || true",
        ].joined(separator: "\n")
    }
    ...
    private func runtimeEncodedRemoteBootstrapCommandShell(
        ...
        lines += [
            "kshr_tmp=$(mktemp \"${TMPDIR:-/tmp}/kshr-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s '\(base64Placeholder)' | base64 -d 2>/dev/null || printf %s '\(base64Placeholder)' | base64 -D 2>/dev/null) > \"$kshr_tmp\" || { rm -f \"$kshr_tmp\"; exit 1; }",
            "chmod 700 \"$kshr_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$kshr_tmp\"",
            "kshr_status=$?",
            "rm -f \"$kshr_tmp\"",
            "exit $kshr_status",
    ...
    private func remoteBootstrapTTYCaptureLines(
        ...
        var lines: [String] = [
            "kshr_bootstrap_tty=\"$(tty 2>/dev/null || true)\"",
            "kshr_bootstrap_tty=\"${kshr_bootstrap_tty##*/}\"",
            "if [ -n \"$kshr_bootstrap_tty\" ] && [ \"$kshr_bootstrap_tty\" != \"not a tty\" ]; then",
            "  mkdir -p \"$HOME/.kshr/relay\" >/dev/null 2>&1 || true",
            "  printf '%s' \"$kshr_bootstrap_tty\" > \"$HOME/.kshr/relay/\(remoteRelayPort).tty\" 2>/dev/null || true",
            "  export KSHR_BOOTSTRAP_TTY=\"$kshr_bootstrap_tty\"",
        ]

        if includeRelayRPC {
            lines += [
                "  kshr_relay_cli=\"$HOME/.kshr/bin/kshr\"",
                "  if [ ! -x \"$kshr_relay_cli\" ]; then kshr_relay_cli=\"$(command -v kshr 2>/dev/null || true)\"; fi",
                "  if [ -n \"$kshr_relay_cli\" ]; then",
                "    kshr_relay_report_tty='{\"workspace_id\":\"__KSHR_WORKSPACE_ID__\",\"tty_name\":\"'$kshr_bootstrap_tty'\"}'",
                "    kshr_relay_ports_kick='{\"workspace_id\":\"__KSHR_WORKSPACE_ID__\",\"reason\":\"command\"}'",
                "    if [ -n \"__KSHR_SURFACE_ID__\" ]; then",
                "      kshr_relay_report_tty='{\"workspace_id\":\"__KSHR_WORKSPACE_ID__\",\"surface_id\":\"__KSHR_SURFACE_ID__\",\"tty_name\":\"'$kshr_bootstrap_tty'\"}'",
                "      kshr_relay_ports_kick='{\"workspace_id\":\"__KSHR_WORKSPACE_ID__\",\"surface_id\":\"__KSHR_SURFACE_ID__\",\"reason\":\"command\"}'",
                "    fi",
                "    KSHR_SOCKET_PATH=\"127.0.0.1:\(remoteRelayPort)\" KSHR_SOCKET=\"127.0.0.1:\(remoteRelayPort)\" \"$kshr_relay_cli\" rpc surface.report_tty \"$kshr_relay_report_tty\" >/dev/null 2>&1 || true",
                "    KSHR_SOCKET_PATH=\"127.0.0.1:\(remoteRelayPort)\" KSHR_SOCKET=\"127.0.0.1:\(remoteRelayPort)\" \"$kshr_relay_cli\" rpc surface.ports_kick \"$kshr_relay_ports_kick\" >/dev/null 2>&1 || true",
                "    unset kshr_relay_cli kshr_relay_report_tty kshr_relay_ports_kick",
                "  fi",
            ]
        }
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let remoteCallerExportLines = [
            "if [ -n '__KSHR_WORKSPACE_ID__' ]; then export KSHR_WORKSPACE_ID='__KSHR_WORKSPACE_ID__'; fi",
            "if [ -n '__KSHR_WORKSPACE_ID__' ]; then export KSHR_TAB_ID='__KSHR_WORKSPACE_ID__'; fi",
            "if [ -n '__KSHR_SURFACE_ID__' ]; then export KSHR_SURFACE_ID='__KSHR_SURFACE_ID__'; export KSHR_PANEL_ID='__KSHR_SURFACE_ID__'; fi",
        ]
        ...
        commonShellExportLines.append("export PATH=\"$HOME/.kshr/bin:$PATH\"")
        commonShellExportLines.append("export KSHR_BUNDLED_CLI_PATH=\"$HOME/.kshr/bin/kshr\"")
        commonShellExportLines.append("export KSHR_SHELL_INTEGRATION_DIR=\"\(shellStateDir)\"")
        if let relaySocket {
            commonShellExportLines.append("export KSHR_SOCKET_PATH=\(relaySocket)")
            commonShellExportLines.append("export KSHR_SOCKET=\(relaySocket)")
        }
        ...
        zshShellLines.append(
            #"if [ "${KSHR_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${KSHR_SHELL_INTEGRATION_DIR}/kshr-zsh-integration.zsh" ]; then . "${KSHR_SHELL_INTEGRATION_DIR}/kshr-zsh-integration.zsh"; fi"#
        )
        var bashShellLines = commonShellExportLines
        bashShellLines.append(
            #"if [ "${KSHR_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${KSHR_SHELL_INTEGRATION_DIR}/kshr-bash-integration.bash" ]; then . "${KSHR_SHELL_INTEGRATION_DIR}/kshr-bash-integration.bash"; fi"#
        )
        ...
        let bundledZshIntegration = bundledShellIntegrationScript(named: "kshr-zsh-integration.zsh")
        let bundledBashIntegration = bundledShellIntegrationScript(named: "kshr-bash-integration.bash")
        let bashRCLines = [
        ...
        var outerLines: [String] = [
            "mkdir -p \"$HOME/.kshr/relay\"",
            "kshr_shell_dir=\"\(shellStateDir)\"",
            "mkdir -p \"$kshr_shell_dir\"",
        ]
        if let bundledZshIntegration {
            outerLines += [
                "cat > \"$kshr_shell_dir/kshr-zsh-integration.zsh\" <<'KSHRKSHRZSH'",
        ...
        if let bundledBashIntegration {
            outerLines += [
                "cat > \"$kshr_shell_dir/kshr-bash-integration.bash\" <<'KSHRKSHRBASH'",
        ...
        outerLines += [
            "KSHR_LOGIN_SHELL=\"${SHELL:-/bin/zsh}\"",
            "case \"${KSHR_LOGIN_SHELL##*/}\" in",
            "  zsh)",
            "    cat > \"$kshr_shell_dir/.zshenv\" <<'KSHRZSHENV'",
        ]
        outerLines.append(contentsOf: zshEnvLines)
        outerLines += [
            "KSHRZSHENV",
            "    cat > \"$kshr_shell_dir/.zprofile\" <<'KSHRZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "KSHRZSHPROFILE",
            "    cat > \"$kshr_shell_dir/.zshrc\" <<'KSHRZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "KSHRZSHRC",
            "    cat > \"$kshr_shell_dir/.zlogin\" <<'KSHRZSHLOGIN'",
        ]
        outerLines.append(contentsOf: zshLoginLines)
        outerLines += [
            "KSHRZSHLOGIN",
            "    chmod 600 \"$kshr_shell_dir/.zshenv\" \"$kshr_shell_dir/.zprofile\" \"$kshr_shell_dir/.zshrc\" \"$kshr_shell_dir/.zlogin\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    export KSHR_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$kshr_shell_dir\"",
            "    exec \"$KSHR_LOGIN_SHELL\" -il",
            "    ;;",
            "  bash)",
            "    cat > \"$kshr_shell_dir/.bashrc\" <<'KSHRBASHRC'",
        ]
        outerLines.append(contentsOf: bashRCLines)
        outerLines += [
            "KSHRBASHRC",
            "    chmod 600 \"$kshr_shell_dir/.bashrc\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$KSHR_LOGIN_SHELL\" --rcfile \"$kshr_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
        outerLines.append(contentsOf: commonShellExportLines)
    ...
    private func shellStateDirForRemoteRelayPort(_ remoteRelayPort: Int) -> String {
        "$HOME/.kshr/relay/\(max(remoteRelayPort, 0)).shell"
    }
    ...
    private func interactiveRemoteRelayWarmupLines(remoteRelayPort: Int) -> [String] {
        ...
        return [
            "kshr_relay_cli=\"${KSHR_BUNDLED_CLI_PATH:-$HOME/.kshr/bin/kshr}\"",
            "if [ ! -x \"$kshr_relay_cli\" ]; then kshr_relay_cli=\"$(command -v kshr 2>/dev/null || true)\"; fi",
            "kshr_relay_tty=\"${KSHR_BOOTSTRAP_TTY:-}\"",
            "if [ -z \"$kshr_relay_tty\" ]; then kshr_relay_tty=\"$(tty 2>/dev/null || true)\"; fi",
            "kshr_relay_tty=\"${kshr_relay_tty##*/}\"",
            "if [ -n \"$kshr_relay_tty\" ] && [ \"$kshr_relay_tty\" != \"not a tty\" ]; then",
            "  mkdir -p \"$HOME/.kshr/relay\" >/dev/null 2>&1 || true",
            "  printf '%s' \"$kshr_relay_tty\" > \"$HOME/.kshr/relay/\(remoteRelayPort).tty\" 2>/dev/null || true",
            "fi",
            "if [ -n \"$kshr_relay_cli\" ] && [ -n \"$KSHR_WORKSPACE_ID\" ] && [ -n \"$kshr_relay_tty\" ] && [ \"$kshr_relay_tty\" != \"not a tty\" ]; then",
            "  kshr_relay_report_tty=\"{\\\"workspace_id\\\":\\\"$KSHR_WORKSPACE_ID\\\",\\\"tty_name\\\":\\\"$kshr_relay_tty\\\"}\"",
            "  kshr_relay_ports_kick=\"{\\\"workspace_id\\\":\\\"$KSHR_WORKSPACE_ID\\\",\\\"reason\\\":\\\"command\\\"}\"",
            "  if [ -n \"$KSHR_SURFACE_ID\" ]; then",
            "    kshr_relay_report_tty=\"{\\\"workspace_id\\\":\\\"$KSHR_WORKSPACE_ID\\\",\\\"surface_id\\\":\\\"$KSHR_SURFACE_ID\\\",\\\"tty_name\\\":\\\"$kshr_relay_tty\\\"}\"",
            "    kshr_relay_ports_kick=\"{\\\"workspace_id\\\":\\\"$KSHR_WORKSPACE_ID\\\",\\\"surface_id\\\":\\\"$KSHR_SURFACE_ID\\\",\\\"reason\\\":\\\"command\\\"}\"",
            "  fi",
            "  \"$kshr_relay_cli\" rpc surface.report_tty \"$kshr_relay_report_tty\" >/dev/null 2>&1 || true",
            "  \"$kshr_relay_cli\" rpc surface.ports_kick \"$kshr_relay_ports_kick\" >/dev/null 2>&1 || true",
            "fi",
            "unset KSHR_BOOTSTRAP_TTY kshr_relay_cli kshr_relay_tty kshr_relay_report_tty kshr_relay_ports_kick",
        ]
    }
    ...
    func encodedRemoteBootstrapCommand(
        ...
        lines += [
            "kshr_tmp=$(mktemp \"${TMPDIR:-/tmp}/kshr-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$kshr_tmp\" || { rm -f \"$kshr_tmp\"; exit 1; }",
            "chmod 700 \"$kshr_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$kshr_tmp\"",
            "kshr_status=$?",
            "rm -f \"$kshr_tmp\"",
    ...
    func buildSSHStartupCommand(
        ...
        scriptLines += [
            "KSHR_SSH_SESSION_ENDED=0",
            "kshr_ssh_session_end() { if [ \"${KSHR_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; KSHR_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'kshr_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
        ...
        scriptLines += [
            ...
            "kshr_ssh_session_end",
            "exit $kshr_ssh_status",
        ]
    ...
    private func writeSSHStartupScript(_ scriptBody: String, remoteRelayPort: Int) throws -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "kshr-ssh-startup-\(remoteRelayPort)-\(UUID().uuidString.lowercased()).sh"
        )
        let script = "#!/bin/sh\n\(scriptBody)\n"
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${KSHR_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${KSHR_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${KSHR_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${KSHR_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${KSHR_SURFACE_ID:-}\" ]; then",
            "\"${KSHR_BUNDLED_CLI_PATH}\" --socket \"${KSHR_SOCKET_PATH}\" ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${KSHR_WORKSPACE_ID}\" --surface \"${KSHR_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "elif command -v kshr >/dev/null 2>&1",
            "&& [ -n \"${KSHR_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${KSHR_SURFACE_ID:-}\" ]; then",
            "kshr ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${KSHR_WORKSPACE_ID}\" --surface \"${KSHR_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"]
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"]
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or KSHR_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or KSHR_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let bakedVersion = BakedVersionInfo.version
        let bakedBuild = BakedVersionInfo.build
        let bakedCommit = resolvedCommitHash()
        ...
        let cacheURL = remoteDaemonCacheURL(version: manifest?.appVersion ?? bakedVersion, goOS: platform.goOS, goArch: platform.goArch)
        ...
        let downloadCommand = "gh release download \(releaseTag) --repo yosefkohan26/kshr --pattern \(assetName)"
        let downloadChecksumsCommand = "gh release download \(releaseTag) --repo yosefkohan26/kshr --pattern \(checksumsAssetName)"
        let checksumVerifyCommand = "shasum -a 256 -c \(checksumsAssetName) --ignore-missing"
        let signerWorkflow = releaseTag == "nightly"
            ? "yosefkohan26/kshr/.github/workflows/nightly.yml"
            : "yosefkohan26/kshr/.github/workflows/release.yml"
        let verifyCommand = "gh attestation verify ./\(assetName) --repo yosefkohan26/kshr --signer-workflow \(signerWorkflow)"

        let payload: [String: Any] = [
            "app_version": BakedVersionInfo.version,
            "build": BakedVersionInfo.build as Any,
            "commit": resolvedCommitHash() as Any? ?? NSNull(),
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["KSHR_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set KSHR_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["KSHRRemoteDaemonManifestJSON"] as? String,
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("kshr-remote-daemons", isDirectory: true)
                .appendingPathComponent(version, isDirectory: true)
                .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
                .appendingPathComponent("kshrd-remote", isDirectory: false)
        }
        return root
            .appendingPathComponent("kshr", isDirectory: true)
    ...
    private func deferredRemoteReconnectLocalCommand(
        ...
        let escapedForegroundAuthToken = foregroundAuthToken
            ...
            .replacingOccurrences(of: "\"", with: "\\\"")
        return [
            preferredCLIPath.map { "kshr_reconnect_cli=\(shellQuote($0));" } ?? "kshr_reconnect_cli=\"\";",
            "kshr_reconnect_socket=\"${KSHR_SOCKET_PATH:-${KSHR_SOCKET:-}}\";",
            "if [ -z \"$kshr_reconnect_cli\" ] && [ -n \"${KSHR_BUNDLED_CLI_PATH:-}\" ]; then kshr_reconnect_cli=\"$KSHR_BUNDLED_CLI_PATH\"; fi;",
            "if [ ! -x \"$kshr_reconnect_cli\" ]; then kshr_reconnect_cli=\"$(command -v kshr 2>/dev/null || true)\"; fi;",
            "if [ -n \"${KSHR_WORKSPACE_ID:-}\" ]; then",
            "if [ -z \"$kshr_reconnect_socket\" ]; then printf '%s\\n' 'kshr: deferred SSH reconnect skipped, local kshr socket not found' >&2;",
            "elif [ -z \"$kshr_reconnect_cli\" ] || [ ! -x \"$kshr_reconnect_cli\" ]; then printf '%s\\n' 'kshr: deferred SSH reconnect skipped, local kshr CLI not found' >&2;",
            "else",
            "kshr_reconnect_payload=\"{\\\"workspace_id\\\":\\\"$KSHR_WORKSPACE_ID\\\",\\\"foreground_auth_token\\\":\\\"\(escapedForegroundAuthToken)\\\"}\";",
            "\"$kshr_reconnect_cli\" --socket \"$kshr_reconnect_socket\" rpc workspace.remote.foreground_auth_ready \"$kshr_reconnect_payload\" >/dev/null 2>&1 || true;",
            "unset kshr_reconnect_payload;",
            "fi;",
            "fi;",
            "unset kshr_reconnect_socket kshr_reconnect_cli;",
        ].joined(separator: " ")
    }
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/kshr-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
        return "/tmp/kshr-ssh-\(getuid())-%C"
    }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
#if DEBUG
        let trimmedExplicit = ProcessInfo.processInfo.environment["KSHR_DEBUG_LOG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
            if let trimmedExplicit, !trimmedExplicit.isEmpty {
                return trimmedExplicit
            }
            guard let marker = try? String(contentsOfFile: "/tmp/kshr-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [kshr-cli] \(message())\n"
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'kshr browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__kshr_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["KSHR_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("kshr-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the kshr socket server.
            """
        case "capabilities":
            return """
            Usage: kshr capabilities

            Print server capabilities as JSON.
            """
        case "rpc":
            return """
            Usage: kshr rpc <method> [json-params]

            Call a raw v2 method with an optional JSON object for params.
            Example: kshr rpc surface.report_tty '{"workspace_id":"...","surface_id":"...","tty_name":"ttys001"}'
            """
        case "help":
            return """
            Usage: kshr help
        ...
        case "welcome":
            ...
            Show a welcome screen with the kshr logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: kshr shortcuts
        ...
        case "feedback":
            return """
            Usage: kshr feedback
                   kshr feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            ...
            Usage: kshr themes
                   kshr themes list
                   kshr themes set <theme>
                   kshr themes set --light <theme> [--dark <theme>]
                   kshr themes set --dark <theme> [--light <theme>]
                   kshr themes clear

            When run in a TTY, `kshr themes` opens an interactive theme picker with
            live app preview. Use `kshr themes list` for a plain listing.

            The picker previews the selected theme across the running kshr app and
            ...
            Commands:
              ...
              clear                     Remove the kshr theme override and fall back to other config

            Examples:
              kshr themes
              kshr themes list
              kshr themes set "Catppuccin Mocha"
              kshr themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              kshr themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: kshr claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses kshr splits
            ...
            The tmux shim translates supported tmux window/pane commands into kshr
            workspace and split operations in the current kshr session.

            Examples:
              kshr claude-teams
              kshr claude-teams --continue
              kshr claude-teams --model sonnet
            """)
        case "omo":
            return String(localized: "cli.omo.usage", defaultValue: """
            Usage: kshr omo [opencode-args...]

            Launch OpenCode with oh-my-openagent in a kshr-aware environment.
            ...
            kshr splits with sidebar metadata and notifications.

            This command:
              - sets a tmux-like environment so oh-my-openagent uses kshr splits
              - prepends a private tmux shim to PATH
              - forwards all remaining arguments to opencode

            The tmux shim translates tmux window/pane commands into kshr workspace
            and split operations in the current kshr session.

            Examples:
              kshr omo
              kshr omo --continue
              kshr omo --model claude-sonnet-4-6
            """)
        case "identify":
            return """
            Usage: kshr identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $KSHR_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $KSHR_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: kshr list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: kshr current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: kshr new-window
        ...
        case "focus-window":
            return """
            Usage: kshr focus-window --window <id|ref|index>
            ...
            Example:
              kshr focus-window --window 0
              kshr focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: kshr close-window --window <id|ref|index>
            ...
            Example:
              kshr close-window --window 0
              kshr close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: kshr move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              kshr move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: kshr move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              kshr move-surface --surface surface:1 --workspace workspace:2
              kshr move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: kshr reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              kshr reorder-surface --surface surface:1 --index 0
              kshr reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: kshr reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              kshr reorder-workspace --workspace workspace:2 --index 0
              kshr reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: kshr workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$KSHR_WORKSPACE_ID)
            ...
            Example:
              kshr workspace-action --workspace workspace:2 --action pin
              kshr workspace-action --action rename --title "infra"
              kshr workspace-action close-others
              kshr workspace-action --action set-color --color blue
              kshr workspace-action --action set-color --color "#C0392B"
              kshr workspace-action set-color Amber
              kshr workspace-action --action set-description --description "Ship checklist"
              kshr workspace-action --action set-description $'Ship checklist\n- verify build\n- post notes'
              kshr workspace-action clear-color
            """
        case "tab-action":
            return """
            Usage: kshr tab-action --action <name> [flags]
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $KSHR_TAB_ID, then $KSHR_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$KSHR_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --url <url>                  Optional URL for new-browser-right

            Example:
              kshr tab-action --tab tab:3 --action pin
              kshr tab-action --action close-right
              kshr tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: kshr rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            1) --tab
            2) --surface
            3) $KSHR_TAB_ID / $KSHR_SURFACE_ID
            4) currently focused tab (optionally within --workspace)

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$KSHR_WORKSPACE_ID)
            ...
            Examples:
              kshr rename-tab "build logs"
              kshr rename-tab --tab tab:3 "staging server"
              kshr rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: kshr new-workspace [--name <title>] [--description <text>] [--cwd <path>] [--command <text>]
            ...
            Example:
              kshr new-workspace
              kshr new-workspace --name "Build Server"
              kshr new-workspace --name "Launch" --description "Ship checklist"
              kshr new-workspace --cwd ~/projects/myapp
              kshr new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: kshr list-workspaces
        ...
        case "ssh":
            return """
            Usage: kshr ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            kshr will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              kshr ssh dev@my-host
              kshr ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              kshr ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: kshr remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded kshrd-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              kshr remote-daemon-status
              kshr remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: kshr new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $KSHR_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $KSHR_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              kshr new-split right
              kshr new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: kshr list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $KSHR_WORKSPACE_ID)

            Example:
              kshr list-panes
              kshr list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: kshr list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              kshr list-pane-surfaces
              kshr list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: kshr tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `kshr tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              kshr tree
              kshr tree --all
              kshr tree --workspace workspace:2
              kshr --json tree --all
            """
        case "focus-pane":
            return """
            Usage: kshr focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              kshr focus-pane pane:1
              kshr focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: kshr new-pane [flags]
            ...
            Example:
              kshr new-pane
              kshr new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: kshr new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: kshr close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $KSHR_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $KSHR_WORKSPACE_ID)

            Example:
              kshr close-surface
              kshr close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: kshr drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              kshr drag-surface-to-split --surface surface:1 right
              kshr drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: kshr refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "reload-config":
            return """
            Usage: kshr reload-config

            Run the same configuration reload as the Reload Configuration shortcut.
            This reloads Ghostty config, re-reads ~/.config/kshr/settings.json, and refreshes terminals.
        ...
        case "surface-health":
            return """
            Usage: kshr surface-health [--workspace <id|ref>]
            ...
            Example:
              kshr surface-health
              kshr surface-health --workspace workspace:2
            """
        case "debug-terminals":
            return """
            Usage: kshr debug-terminals
        ...
        case "trigger-flash":
            return """
            Usage: kshr trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $KSHR_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $KSHR_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              kshr trigger-flash
              kshr trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: kshr list-panels [--workspace <id|ref>]
            ...
            Example:
              kshr list-panels
              kshr list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: kshr focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              kshr focus-panel --panel surface:2
              kshr focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: kshr close-workspace --workspace <id|ref|index>
            ...
            Example:
              kshr close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: kshr select-workspace --workspace <id|ref|index>
            ...
            Example:
              kshr select-workspace --workspace workspace:2
              kshr select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: kshr rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$KSHR_WORKSPACE_ID)

            Example:
              kshr rename-workspace "backend logs"
              kshr rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: kshr current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: kshr capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $KSHR_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $KSHR_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              kshr capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: kshr resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: kshr pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: kshr wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: kshr swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: kshr break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: kshr join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "last-pane":
            return """
            Usage: kshr last-pane [--workspace <id|ref>]
        ...
        case "find-window":
            return """
            Usage: kshr find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: kshr clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: kshr set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: kshr set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: kshr paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: kshr list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: kshr respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: kshr display-message [-p|--print] <text>
        ...
        case "read-screen":
            ...
            Example:
              kshr read-screen
              kshr read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: kshr send [flags] [--] <text>
            ...
            Example:
              kshr send "echo hello"
              kshr send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: kshr send-key [flags] [--] <key>
            ...
            Example:
              kshr send-key enter
              kshr send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: kshr send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              kshr send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: kshr send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              kshr send-key-panel --panel surface:2 enter
              kshr send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: kshr notify [flags]
            ...
            Example:
              kshr notify --title "Build done" --body "All tests passed"
              kshr notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: kshr list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: kshr clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: kshr set-status <key> <value> [flags]
            ...
            Example:
              kshr set-status build "compiling" --icon hammer --color "#ff9500"
              kshr set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: kshr clear-status <key> [flags]
            ...
            Example:
              kshr clear-status build
            """
        case "list-status":
            return """
            Usage: kshr list-status [flags]
            ...
            Example:
              kshr list-status
              kshr list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: kshr set-progress <0.0-1.0> [flags]
            ...
            Example:
              kshr set-progress 0.5 --label "Building..."
              kshr set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: kshr clear-progress [flags]
        ...
        case "sidebar-state":
            return """
            Usage: kshr sidebar-state [flags]
        ...
        case "set-app-focus":
            return """
            Usage: kshr set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              kshr set-app-focus inactive
              kshr set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: kshr simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: kshr claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | kshr claude-hook session-start
              echo '{}' | kshr claude-hook stop
            """
        case "codex":
            return """
            Usage: kshr codex <install-hooks|uninstall-hooks>

            Manage Codex CLI hooks integration.

            Subcommands:
              install-hooks     Install kshr hooks into ~/.codex/hooks.json
              uninstall-hooks   Remove kshr hooks from ~/.codex/hooks.json
            """
        case "codex-hook":
            return """
            Usage: kshr codex-hook <session-start|prompt-submit|stop> [flags]

            Hook for Codex CLI integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside kshr.
        ...
        case "browser":
            return """
            Usage: kshr browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $KSHR_WORKSPACE_ID when --workspace is omitted and --window is not set
            ...
            Example:
              ...
              kshr browser surface:1 navigate https://google.com
              kshr browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `kshr browser --help`
        case "open-browser":
            return "Legacy alias for 'kshr browser open'. Run 'kshr browser --help' for details."
        case "navigate":
            return "Legacy alias for 'kshr browser navigate'. Run 'kshr browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'kshr browser back'. Run 'kshr browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'kshr browser forward'. Run 'kshr browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'kshr browser reload'. Run 'kshr browser --help' for details."
        case "get-url":
            return "Legacy alias for 'kshr browser get-url'. Run 'kshr browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'kshr browser focus-webview'. Run 'kshr browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'kshr browser is-webview-focused'. Run 'kshr browser --help' for details."
        case "markdown":
            return """
            Usage: kshr markdown open <path> [options]
                   kshr markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $KSHR_WORKSPACE_ID)
            ...
            Examples:
              ...
              kshr markdown ~/project/CHANGELOG.md
              kshr markdown open ./docs/design.md --workspace 0
              kshr markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private static let kshrThemeOverrideBundleIdentifier = "com.kshrterm.app"
    private static let kshrThemesBlockStart = "# kshr themes start"
    private static let kshrThemesBlockEnd = "# kshr themes end"
    private static let kshrThemesReloadNotificationName = "com.kshrterm.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["KSHR_THEME_PICKER_CONFIG"] = try kshrThemeOverrideConfigURL().path
        environment["KSHR_THEME_PICKER_BUNDLE_ID"] = currentKshrAppBundleIdentifier() ?? Self.kshrThemeOverrideBundleIdentifier
        environment["KSHR_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["KSHR_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["KSHR_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["KSHR_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'kshr themes --help'.")
            }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try kshrThemeOverrideConfigURL().path
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'kshr themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.kshrThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.kshrThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func kshrThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.kshrThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try kshrThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.kshrThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.kshrThemesBlockEnd)
        """
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentKshrAppBundleIdentifier() ?? Self.kshrThemeOverrideBundleIdentifier
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.kshrThemesReloadNotificationName),
            object: nil,
    ...
    private func currentKshrAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["KSHR_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !bundleIdentifier.isEmpty {
            return bundleIdentifier
        }
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["KSHR_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["KSHR_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["KSHR_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"])
    }
    ...
    private func tmuxResolveSurfaceTarget(
        ...
        if tmuxPaneSelector(from: raw) != nil {
            ...
            // exact surface (KSHR_SURFACE_ID) over the pane's currently selected
    ...
    private func isKshrClaudeWrapper(at path: String) -> Bool {
        guard let data = FileManager.default.contents(atPath: path) else { return false }
        let prefixData = data.prefix(512)
        guard let prefix = String(data: prefixData, encoding: .utf8) else { return false }
        return prefix.contains("kshr claude wrapper - injects hooks and session tracking")
    }
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isKshrClaudeWrapper(at: $0) }
        )
    }
    ...
    private func configureTmuxCompatEnvironment(
        ...
        if let focusedContext {
            setenv("KSHR_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("KSHR_SURFACE_ID", surfaceId, 1)
            }
        }
    }

    private static let claudeNodeOptionsRestoreModule = """
    const hadOriginalNodeOptions = process.env.KSHR_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
    if (hadOriginalNodeOptions) {
        process.env.NODE_OPTIONS = process.env.KSHR_ORIGINAL_NODE_OPTIONS ?? "";
    } else {
        delete process.env.NODE_OPTIONS;
    }
    delete process.env.KSHR_ORIGINAL_NODE_OPTIONS;
    delete process.env.KSHR_ORIGINAL_NODE_OPTIONS_PRESENT;
    """
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            tmuxPathPrefix: "kshr-claude-teams",
            kshrBinEnvVar: "KSHR_CLAUDE_TEAMS_KSHR_BIN",
            termOverrideEnvVar: "KSHR_CLAUDE_TEAMS_TERM",
            extraEnvVars: [
                (key: "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", value: "1"),
            ]
        )
        guard let restoreModuleURL = try? createClaudeNodeOptionsRestoreModule() else {
            unsetenv("KSHR_ORIGINAL_NODE_OPTIONS_PRESENT")
            unsetenv("KSHR_ORIGINAL_NODE_OPTIONS")
            return
        }
        if let existing = processEnvironment["NODE_OPTIONS"] {
            setenv("KSHR_ORIGINAL_NODE_OPTIONS_PRESENT", "1", 1)
            setenv("KSHR_ORIGINAL_NODE_OPTIONS", existing, 1)
        } else {
            setenv("KSHR_ORIGINAL_NODE_OPTIONS_PRESENT", "0", 1)
            unsetenv("KSHR_ORIGINAL_NODE_OPTIONS")
        }
        setenv(
            "NODE_OPTIONS",
    ...
    private func createTmuxCompatShimDirectory(
        ...
        let root = URL(fileURLWithPath: homePath, isDirectory: true)
            .appendingPathComponent(".kshrterm", isDirectory: true)
            .appendingPathComponent(directoryName, isDirectory: true)
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${KSHR_CLAUDE_TEAMS_KSHR_BIN:-kshr}" __tmux-compat "$@"
        """
    ...
    private func createClaudeNodeOptionsRestoreModule() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("kshr-claude-node-options", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true, attributes: nil)
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["KSHR_SOCKET_PATH"] = socketPath
        launcherEnvironment["KSHR_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["KSHR_SOCKET_PASSWORD"] = explicitPassword
        }
        let shimDirectory = try createClaudeTeamsShimDirectory()
        let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "kshr")
        ...
        let claudeExecutablePath: String? = {
            ...
            let candidates = [
                launcherEnvironment["KSHR_CUSTOM_CLAUDE_PATH"],
            ...
            for raw in candidates {
                ...
                guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDir),
                      ...
                      !isKshrClaudeWrapper(at: trimmed) else { continue }
                return trimmed
            }
    ...
    // MARK: - kshr omo (OpenCode + oh-my-openagent)
    ...
    private func createOMOShimDirectory() throws -> URL {
        // tmux shim: redirects tmux commands to kshr __tmux-compat
        ...
        exec "${KSHR_OMO_KSHR_BIN:-kshr}" __tmux-compat "$@"
        """
        ...
        // terminal-notifier shim: intercepts macOS notifications and routes to kshr notify
        let notifierURL = root.appendingPathComponent("terminal-notifier", isDirectory: false)
        let notifierScript = """
        #!/usr/bin/env bash
        # Intercept terminal-notifier calls and route through kshr notify.
        ...
        exec "${KSHR_OMO_KSHR_BIN:-kshr}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
        """
    ...
    private func configureOMOEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "kshr-omo",
            kshrBinEnvVar: "KSHR_OMO_KSHR_BIN",
            termOverrideEnvVar: "KSHR_OMO_TERM",
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in kshr claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in kshr claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxCompatStoreURL() -> URL {
        ...
        return URL(fileURLWithPath: homePath)
            .appendingPathComponent(".kshrterm")
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/kshr-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in kshr CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in kshr CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "kshr", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["KSHR_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["KSHR_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["KSHR_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                kshr claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func resolveCallerTTYName() -> String? {
        let env = ProcessInfo.processInfo.environment
        for key in ["KSHR_CLI_TTY_NAME", "KSHR_TTY_NAME", "TTY", "SSH_TTY"] {
    ...
    /// The hooks.json content that kshr installs into ~/.codex/.
    /// Each hook calls `kshr codex-hook <event>` which gracefully no-ops
    /// when not running inside kshr. The command checks for kshr on PATH
    /// first so it silently succeeds even when kshr is not installed
    /// (e.g. user opened codex in a non-kshr terminal).
    private static func codexHookCommand(_ event: String) -> String {
        "[ -n \"$KSHR_SURFACE_ID\" ] && command -v kshr >/dev/null 2>&1 && kshr codex-hook \(event) || echo '{}'"
    }
    ...
    /// Identifier used to detect kshr-owned hooks during uninstall.
    private static let codexHookCommandMarker = "kshr codex-hook"
    ...
    private func runCodexInstallHooks() throws {
        ...
        let kshrHooks = Self.codexHooksJSON["hooks"] as! [String: Any]
        for (eventName, kshrGroups) in kshrHooks {
            guard let kshrGroupArray = kshrGroups as? [[String: Any]] else { continue }
            ...
            eventGroups.append(contentsOf: kshrGroupArray)
        ...
        if !hooksChanged && !configChanged {
            print("kshr hooks are already installed. Nothing to change.")
            return
        }
        ...
        print("")
        print("Installed. Hooks activate inside kshr and silently no-op elsewhere.")
        print("To remove: kshr codex uninstall-hooks")
    }
    ...
    private func runCodexUninstallHooks() throws {
        ...
        // Build the new state without kshr hooks
        var removedCount = 0
        ...
        print("Removed kshr Codex hooks.")
    }
    ...
    /// Codex hook handler. Gracefully no-ops when not running inside kshr.
    private func runCodexHook(
        ...
        // Graceful no-op: if not inside kshr, exit silently with valid JSON
        guard env["KSHR_SURFACE_ID"] != nil else {
            print("{}")
            return
        }
        ...
        let hookArgs = Array(commandArgs.dropFirst())
        let hookWsFlag = optionValue(hookArgs, name: "--workspace")
        let workspaceArg = hookWsFlag ?? env["KSHR_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? env["KSHR_SURFACE_ID"] : nil)
        let rawInput = String(data: FileHandle.standardInput.readDataToEndOfFile(), encoding: .utf8) ?? ""
        let parsedInput = parseClaudeHookInput(rawInput: rawInput)
        let sessionStore = ClaudeHookSessionStore(
            processEnv: env.merging(
                ["KSHR_CLAUDE_HOOK_STATE_PATH": "~/.kshrterm/codex-hook-sessions.json"],
        ...
        case "help", "--help", "-h":
            print("kshr codex-hook <session-start|prompt-submit|stop> [--workspace <id>] [--surface <id>]")
    ...
    private func versionSummary() -> String {
        // Fast path: use compile-time constants baked by bump-version.sh.
        // This avoids all runtime disk walks, plist parsing, and git spawns.
        let version = BakedVersionInfo.version
        let build = BakedVersionInfo.build
        let baseSummary = "kshr \(version) (\(build))"

        // The git commit is still resolved at runtime because it changes with
        // every commit while the version/build only change at release time.
        // In release builds we check the bundle plist (stamped by the Xcode
        // run-script phase) and the KSHR_COMMIT env var -- both are cheap.
        // The expensive project-file / git-subprocess fallback is DEBUG-only.
        let commit: String? = resolvedCommitHash()
        guard let commit else { return baseSummary }
        return "\(baseSummary) [\(commit)]"
    }

    /// Resolve the git commit hash with minimal overhead.  Checks the bundle
    /// plist and environment first; falls back to filesystem/git walks only
    /// in DEBUG builds.
    private func resolvedCommitHash() -> String? {
        // 1. Bundle plist (stamped by the "Copy Ghostty Resources" run-script).
        if let commit = normalizedCommitHash(
            Bundle.main.infoDictionary?["KSHRCommit"] as? String
        ) {
            return commit
        }

        // 2. Environment variable (set by CI or wrapper scripts).
        if let commit = normalizedCommitHash(
            ProcessInfo.processInfo.environment["KSHR_COMMIT"]
        ) {
            return commit
        }

        #if DEBUG
        // 3. Expensive fallback: walk the filesystem to find Info.plist or
        //    spawn `git rev-parse`.  Only used in development builds where the
        //    CLI runs outside a stamped app bundle.
        let info = resolvedVersionInfo()
        if let commit = info["KSHRCommit"].flatMap({ normalizedCommitHash($0) }) {
            return commit
        }
        #endif
    ...
    private func printWelcome() {
        ...
        \(c1)  :: ::\(reset)     \(c2)  ::\(reset)        \(c3)  ::    ::\(reset)  \(c4)  ::\(reset)    \(c4)::\(reset)
        \(c1)  ::::\(reset)      \(c2)  ::::::\(reset)    \(c3)  ::::::::\(reset)  \(c4)  ::::::::\(reset)
        \(c5)  :: ::\(reset)     \(c6)      ::\(reset)    \(c3)  ::    ::\(reset)  \(c4)  ::\(reset)    \(c4)::\(reset)
        \(c5)  ::  ::\(reset)    \(c6)  ::::::::\(reset)  \(c7)  ::    ::\(reset)  \(c4)  ::    ::\(reset)

        \(tagline)the most jewish terminal built for kosher agents\(reset)
        """
        ...
        print()
        print()
        print("  \(subdued)Run \(reset)\(bold)kshr --help\(reset)\(subdued) for all commands.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)kshr shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)kshr feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["KSHRCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["KSHR_COMMIT"]) {
            info["KSHRCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["KSHRCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["KSHRCommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        return """
        kshr - control kshr via Unix socket

        Usage:
          kshr <path>                Open a directory in a new workspace (launches kshr if needed)
          kshr [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then KSHR_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Environment:
          KSHR_WORKSPACE_ID   Auto-set in kshr terminals. Used as default --workspace for
                              ALL commands (send, list-panels, new-split, notify, etc.).
          KSHR_TAB_ID         Optional alias used by `tab-action`/`rename-tab` as default --tab.
          KSHR_SURFACE_ID     Auto-set in kshr terminals. Used as default --surface.
          KSHR_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to ~/Library/Application Support/kshr/kshr.sock and auto-discovers tagged/debug sockets.
        """
    }
...
struct KSHRTermMain {
    static func main() {
        ...
        let cli = KSHRCLI(args: CommandLine.arguments)
        do {
            try cli.run()
        } catch {

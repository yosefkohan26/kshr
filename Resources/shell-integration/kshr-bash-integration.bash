# kshr shell integration for bash

# Cache which send tool is available to avoid repeated PATH lookups.
_KSHR_SEND_TOOL=""
_kshr_detect_send_tool() {
    if command -v ncat >/dev/null 2>&1; then
        _KSHR_SEND_TOOL=ncat
    elif command -v socat >/dev/null 2>&1; then
        _KSHR_SEND_TOOL=socat
    elif command -v nc >/dev/null 2>&1; then
        _KSHR_SEND_TOOL=nc
    fi
}
# Detection deferred to after _kshr_fix_path (end of file).

_kshr_send() {
    local payload="$1"
    case "$_KSHR_SEND_TOOL" in
        ncat)
            printf '%s\n' "$payload" | ncat -w 1 -U "$KSHR_SOCKET_PATH" --send-only
            ;;
        socat)
            printf '%s\n' "$payload" | socat -T 1 - "UNIX-CONNECT:$KSHR_SOCKET_PATH" >/dev/null 2>&1
            ;;
        nc)
            if printf '%s\n' "$payload" | nc -N -U "$KSHR_SOCKET_PATH" >/dev/null 2>&1; then
                :
            else
                printf '%s\n' "$payload" | nc -w 1 -U "$KSHR_SOCKET_PATH" >/dev/null 2>&1 || true
            fi
            ;;
    esac
}
...
_kshr_restore_scrollback_once
_KSHR_CLAUDE_WRAPPER="${_KSHR_CLAUDE_WRAPPER:-}"
...
_KSHR_PWD_LAST_PWD="${_KSHR_PWD_LAST_PWD:-}"
_KSHR_GIT_LAST_PWD="${_KSHR_GIT_LAST_PWD:-}"
_KSHR_GIT_LAST_RUN="${_KSHR_GIT_LAST_RUN:-0}"
_KSHR_GIT_JOB_PID="${_KSHR_GIT_JOB_PID:-}"
_KSHR_GIT_JOB_STARTED_AT="${_KSHR_GIT_JOB_STARTED_AT:-0}"
_KSHR_GIT_HEAD_LAST_PWD="${_KSHR_GIT_HEAD_LAST_PWD:-}"
_KSHR_GIT_HEAD_PATH="${_KSHR_GIT_HEAD_PATH:-}"
_KSHR_GIT_HEAD_SIGNATURE="${_KSHR_GIT_HEAD_SIGNATURE:-}"
_KSHR_PR_POLL_PID="${_KSHR_PR_POLL_PID:-}"
_KSHR_PR_POLL_PWD="${_KSHR_PR_POLL_PWD:-}"
_KSHR_PR_LAST_BRANCH="${_KSHR_PR_LAST_BRANCH:-}"
_KSHR_PR_NO_PR_BRANCH="${_KSHR_PR_NO_PR_BRANCH:-}"
_KSHR_PR_POLL_INTERVAL="${_KSHR_PR_POLL_INTERVAL:-45}"
_KSHR_PR_FORCE="${_KSHR_PR_FORCE:-0}"
_KSHR_PR_DEBUG="${_KSHR_PR_DEBUG:-0}"
_KSHR_ASYNC_JOB_TIMEOUT="${_KSHR_ASYNC_JOB_TIMEOUT:-20}"

_KSHR_PORTS_LAST_RUN="${_KSHR_PORTS_LAST_RUN:-0}"
_KSHR_SHELL_ACTIVITY_LAST="${_KSHR_SHELL_ACTIVITY_LAST:-}"
_KSHR_TTY_NAME="${_KSHR_TTY_NAME:-}"
_KSHR_TTY_REPORTED="${_KSHR_TTY_REPORTED:-0}"
_KSHR_TMUX_PUSH_SIGNATURE="${_KSHR_TMUX_PUSH_SIGNATURE:-}"
_KSHR_TMUX_PULL_SIGNATURE="${_KSHR_TMUX_PULL_SIGNATURE:-}"
_KSHR_TMUX_SYNC_KEYS=(
    KSHR_BUNDLED_CLI_PATH
    KSHR_BUNDLE_ID
    KSHRD_UNIX_PATH
    KSHRTERM_REPO_ROOT
...
_KSHR_TMUX_SURFACE_SCOPED_KEYS=(
    KSHR_PANEL_ID
    KSHR_SURFACE_ID
)
...
_kshr_report_shell_activity_state() {
    ...
    if [[ -S "$KSHR_SOCKET_PATH" ]]; then
        {
            _kshr_send "report_shell_state $state --tab=$KSHR_TAB_ID --panel=$KSHR_PANEL_ID"
        } >/dev/null 2>&1 & disown
...
_kshr_ports_kick() {
    ...
    if _kshr_socket_is_unix; then
        {
            _kshr_send "ports_kick --tab=$KSHR_TAB_ID --panel=$KSHR_PANEL_ID --reason=$reason"
        } >/dev/null 2>&1 & disown
    else
...
_kshr_clear_pr_for_panel() {
    ...
    _kshr_send "clear_pr --tab=$KSHR_TAB_ID --panel=$KSHR_PANEL_ID"
}
...
_kshr_pr_cache_prefix() {
    ...
    printf '%s\n' "/tmp/kshr-pr-cache-${KSHR_PANEL_ID}"
}

_kshr_pr_force_signal_path() {
    [[ -n "$KSHR_PANEL_ID" ]] || return 1
    printf '%s\n' "/tmp/kshr-pr-force-${KSHR_PANEL_ID}"
}
...
_kshr_start_pr_poll_loop() {
    ...
    _KSHR_PR_POLL_PID=$!
    disown "$_KSHR_PR_POLL_PID" 2>/dev/null || disown
}
...
_kshr_prompt_command() {
    ...
    if (( ! kshr_has_unix_socket )); then
        if (( now - _KSHR_PORTS_LAST_RUN >= 10 )); then
    ...
    if [[ "$pwd" != "$_KSHR_PWD_LAST_PWD" ]]; then
        ...
        {
            local qpwd="${pwd//\"/\\\"}"
            _kshr_send "report_pwd \"${qpwd}\" --tab=$KSHR_TAB_ID --panel=$KSHR_PANEL_ID"
        } >/dev/null 2>&1 & disown
    fi
    ...
    if [[ -n "$_KSHR_GIT_JOB_PID" ]] && kill -0 "$_KSHR_GIT_JOB_PID" 2>/dev/null; then
        if [[ "$pwd" != "$_KSHR_GIT_LAST_PWD" || "$git_head_changed" == "1" ]]; then
    ...
    if [[ -z "$_KSHR_GIT_JOB_PID" ]] || ! kill -0 "$_KSHR_GIT_JOB_PID" 2>/dev/null; then
    ...
    if [[ "$pwd" != "$_KSHR_PR_POLL_PWD" || "$git_head_changed" == "1" ]]; then
...
_kshr_install_prompt_command() {
    [[ -n "${_KSHR_PROMPT_INSTALLED:-}" ]] && return 0
    _KSHR_PROMPT_INSTALLED=1
    ...
    if [[ "$decl" == "declare -a"* ]]; then
        ...
        for item in "${PROMPT_COMMAND[@]}"; do
            [[ "$item" == "_kshr_prompt_command" ]] && existing=1 && break
        done
        if (( existing == 0 )); then
            PROMPT_COMMAND=("_kshr_prompt_command" "${PROMPT_COMMAND[@]}")
        fi
    else
        case ";$PROMPT_COMMAND;" in
            ...
            *)
                if [[ -n "$PROMPT_COMMAND" ]]; then
                    ...
                    PROMPT_COMMAND="_kshr_prompt_command"
                fi
                ;;
        esac
    fi
        ...
        if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
            builtin readonly _KSHR_BASH_PS0='${ _kshr_bash_preexec_hook "$BASH_COMMAND"; }'
        else
            builtin readonly _KSHR_BASH_PS0='$(_kshr_bash_preexec_hook "$BASH_COMMAND" >/dev/null)'
        fi
        if [[ "$PS0" != *"${_KSHR_BASH_PS0}"* ]]; then
            PS0=$PS0"${_KSHR_BASH_PS0}"
        fi
    fi
}

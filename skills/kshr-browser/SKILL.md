---
name: kshr-browser
description: End-user browser automation with kshr. Use when you need to open sites, interact with pages, wait for state changes, and extract data from kshr browser surfaces.
---

# Browser Automation with kshr

Use this skill for browser tasks inside kshr webviews.
...
```bash
kshr --json browser open https://example.com
...
kshr browser surface:7 fill e1 "hello"
kshr --json browser surface:7 click e2 --snapshot-after
kshr browser surface:7 snapshot --interactive
```
...
kshr browser open https://example.com --workspace workspace:2 --window window:1 --json
```
...
kshr supports wait patterns similar to agent-browser:

```bash
kshr browser <surface> wait --selector "#ready" --timeout-ms 10000
kshr browser <surface> wait --text "Success" --timeout-ms 10000
...
```bash
kshr --json browser open https://example.com/signup
kshr browser surface:7 get url
kshr browser surface:7 wait --load-state complete --timeout-ms 15000
kshr browser surface:7 snapshot --interactive
kshr browser surface:7 fill e1 "Jane Doe"
kshr browser surface:7 fill e2 "jane@example.com"
kshr --json browser surface:7 click e3 --snapshot-after
kshr browser surface:7 wait --url-contains "/welcome" --timeout-ms 15000
kshr browser surface:7 snapshot --interactive
```
...
```bash
kshr browser surface:7 fill e11 "" --snapshot-after --json
kshr browser surface:7 get value e11 --json
```
...
kshr browser surface:7 wait --load-state complete --timeout-ms 15000
kshr browser surface:7 snapshot --interactive
kshr --json browser surface:7 click e5 --snapshot-after
kshr browser surface:7 snapshot --interactive
```
...
```bash
kshr browser surface:7 get url
kshr browser surface:7 get text body
kshr browser surface:7 get html body
```

...
# Absolute path
kshr markdown open /path/to/PLAN.md

# Target a specific workspace
kshr markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in kshr:
...
```bash
# Open in the caller's workspace (default -- uses KSHR_WORKSPACE_ID)
kshr markdown open plan.md

# Open in a specific workspace
kshr markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
kshr markdown open plan.md --surface surface:5

# Open in a specific window
kshr markdown open plan.md --window window:1
```

...
1. Clone the repository with submodules:
   ```bash
   git clone --recursive https://github.com/yosefkohan26/kshr.git
   cd kshr
   ```
...
2. Run the setup script:
   ...
   This will:
   - Initialize git submodules (ghostty, homebrew-kshr)
...
```bash
ssh kshr-vm 'cd /Users/kshr/GhosttyTabs && xcodebuild -project GhosttyTabs.xcodeproj -scheme kshr -configuration Debug -destination "platform=macOS" build && pkill -x "kshr DEV" || true && APP=$(find /Users/kshr/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/kshr DEV.app" -print -quit) && open "$APP" && for i in {1..20}; do [ -S /tmp/kshr.sock ] && break; sleep 0.5; done && python3 tests/test_update_timing.py && python3 tests/test_signals_auto.py && python3 tests/test_ctrl_socket.py && python3 tests/test_notifications.py'
```

### UI tests (run on VM)

```bash
ssh kshr-vm 'cd /Users/kshr/GhosttyTabs && xcodebuild -project GhosttyTabs.xcodeproj -scheme kshr -configuration Debug -destination "platform=macOS" -only-testing:kshrUITests test'
```

## Ghostty Submodule

The `ghostty` submodule points to [yosefkohan26/ghostty](https://github.com/yosefkohan26/ghostty), a fork of the upstream Ghostty project.
...
git push yosefkohan26 my-feature
```

...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("kshr-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".kshr/relay/64011.shell")
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("kshr-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".kshr/relay")
        let socketAddrURL = home.appendingPathComponent(".kshr/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("kshr-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".kshr/relay")
        let socketAddrURL = home.appendingPathComponent(".kshr/socket_addr")
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/kshr-debug-test.sock",
    ...
    func testTeardownRemoteConnectionRequestsControlMasterCleanupWhileStillConnecting() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connecting,
            detail: "Connecting to kshr-macmini",
            target: "kshr-macmini"
        )
    ...
    func testTransferredRemoteSurfaceCleansUpControlMasterWhenSessionEndsInLocalWorkspace() throws {
        ...
        XCTAssertEqual(cleanupArguments.first?.suffix(2), ["exit", "kshr-macmini"])
    }
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/kshr-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "kshr-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/kshr-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/kshr-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/kshr-ssh-%C"))
    ...
    func testDaemonTransportArgumentsReuseConfiguredControlPath() {
        ...
        let arguments = WorkspaceRemoteSSHBatchCommandBuilder.daemonTransportArguments(
            ...
            remotePath: "/remote/kshrd-remote"
        )

        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains(where: { $0 == "ControlPath /tmp/kshr-ssh-%C" || $0 == "ControlPath=/tmp/kshr-ssh-%C" }))
        XCTAssertTrue(arguments.contains("kshr-macmini"))
        XCTAssertTrue(arguments.last?.contains("/remote/kshrd-remote") ?? false)
    }
    ...
    func testReverseRelayControlMasterArgumentsReuseConfiguredControlSocket() throws {
        ...
        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains("ControlPath=/tmp/kshr-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/kshr-drop-123.png")
    }
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to kshr-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "kshr-macmini")
        ...
        XCTAssertEqual(
            ...
            "Remote proxy unavailable (kshr-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to kshr-macmini", target: "kshr-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to kshr-macmini via shared local proxy 127.0.0.1:9999",
            target: "kshr-macmini"
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "kshr",
                  item.path.contains(".app/Contents/Resources/bin/kshr") else {
                continue
            }
            return item.path
        }

        throw XCTSkip("Bundled kshr CLI not found in \(appBundleURL.path)")
    }
    ...
    func testNotifyFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        environment["KSHR_SOCKET_PATH"] = socketPath
        environment["KSHR_WORKSPACE_ID"] = staleWorkspace
        environment["KSHR_SURFACE_ID"] = staleSurface
        environment["KSHR_CLI_SENTRY_DISABLED"] = "1"
        environment["KSHR_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "--ssh-option", "ControlPath /tmp/kshr-ssh-%C",
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=kshr-macmini state=disconnected\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "kshr-macmini")
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath /tmp/kshr-ssh-%C"))
        XCTAssertTrue(sshOptions.contains("StrictHostKeyChecking=accept-new"))

        // `kshr ssh` should land the user in the new SSH workspace immediately.
    ...
    func testSSHCommandDoesNotDeferReconnectWhenWhitespaceControlMasterDisablesMultiplexing() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=kshr-macmini state=connecting\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let fileManager = FileManager.default
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("kshr-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["KSHR_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        startupEnvironment["KSHR_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["KSHR_SOCKET_PATH"] = socketPath
        startupEnvironment["KSHR_WORKSPACE_ID"] = workspaceID
        startupEnvironment["KSHR_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["KSHR_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        let destinationIndex = try XCTUnwrap(firstInvocation.lastIndex(of: "kshr-macmini"))
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["KSHR_WORKSPACE_ID"] = staleWorkspace
        environment["KSHR_SURFACE_ID"] = staleSurface
        environment["KSHR_CLI_TTY_NAME"] = callerTTY
        environment["KSHR_CLI_SENTRY_DISABLED"] = "1"
        environment["KSHR_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testNotifyInTmuxPrefersCallerTTYOverStaleValidSurfaceID() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["KSHR_SOCKET_PATH"] = socketPath
        environment["KSHR_WORKSPACE_ID"] = workspaceId
        environment["KSHR_SURFACE_ID"] = staleSurface
        environment["KSHR_CLI_TTY_NAME"] = callerTTY

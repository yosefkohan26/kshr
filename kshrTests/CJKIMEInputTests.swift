...
private extension GhosttyNSView {
    @objc func kshrUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        kshrUnitTest_interpretKeyEvents(eventArray)
    }
}
...
private func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.kshrUnitTest_interpretKeyEvents(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in kshr)

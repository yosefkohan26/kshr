...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "kshr-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/kshr-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/kshr-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/kshr-services/../kshr-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/kshr-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/kshr-services/project",
                "/tmp/kshr-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/kshr-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/kshr-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/kshr-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/kshr-services/b/file.txt", isDirectory: false),
        ]
    ...
    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../kshr.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/kshr.app/Contents/MacOS/kshr", isDirectory: false),
            URL(fileURLWithPath: "/Applications/kshr.app/Contents/Resources/bin/kshr", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/kshr", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/kshr/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/kshr",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/kshr.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/kshr.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/kshr.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/kshr.app backup/project",
                "/Applications/kshr.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("kshr.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/kshr", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("kshr.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("kshr.app/Contents/MacOS/kshr", isDirectory: false),
                    aliasWorkspace,
                ],
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/kshr"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/kshr")
    }

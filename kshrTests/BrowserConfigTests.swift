...
var kshrUnitTestInspectorAssociationKey: UInt8 = 0
var kshrUnitTestInspectorOverrideInstalled = false
var kshrUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = false
var kshrUnitTestWKWebViewPerformKeyEquivalentHook: ((WKWebView, NSEvent) -> Bool?)?

extension KshrWebView {
    @objc func kshrUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &kshrUnitTestInspectorAssociationKey) as? NSObject
    }
}

extension WKWebView {
    @objc func kshrUnitTest_performKeyEquivalent(with event: NSEvent) -> Bool {
        if let hook = kshrUnitTestWKWebViewPerformKeyEquivalentHook,
           let result = hook(self, event) {
            return result
        }
        return kshrUnitTest_performKeyEquivalent(with: event)
    }

    func kshrSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &kshrUnitTestInspectorAssociationKey,
            inspector,
...
func installKshrUnitTestInspectorOverride() {
    guard !kshrUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        KshrWebView.self,
        #selector(KshrWebView.kshrUnitTestInspector)
    ) else {
    ...
    guard added else {
        fatalError("Unable to install KshrWebView _inspector test override")
    }

    kshrUnitTestInspectorOverrideInstalled = true
}

func installKshrUnitTestWKWebViewPerformKeyEquivalentOverride() {
    guard !kshrUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled else { return }

    let originalSelector = #selector(NSResponder.performKeyEquivalent(with:))
    let swizzledSelector = #selector(WKWebView.kshrUnitTest_performKeyEquivalent(with:))
    ...
    kshrUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = true
}
...
final class KshrWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdNRoutesToMainMenuWhenWebViewIsFirstResponder() {
        ...
        let webView = KshrWebView(frame: .zero, configuration: WKWebViewConfiguration())
    ...
    func testCanBlockFirstResponderAcquisitionWhenPaneIsUnfocused() {
        ...
        let webView = KshrWebView(frame: container.bounds, configuration: WKWebViewConfiguration())
    ...
    func testWindowFirstResponderGuardAllowsPointerInitiatedClickFocusFromPortalHostedInspectorSibling() {
        ...
        let webView = KshrWebView(frame: slot.bounds, configuration: WKWebViewConfiguration())
        webView.autoresizingMask = [.width, .height]
    ...
    func testCmdBacktickDoesNotRouteDirectlyToMainMenuWhenWebViewIsFirstResponder() {
        ...
        XCTAssertFalse(
            spy.invoked,
            "KshrWebView should not route Cmd+` directly to the menu when WebKit is first responder"
        )
    }
    ...
    func testCmdFDoesNotPreflightIntoPageWhenWebInspectorResponderIsFocused() {
        _ = NSApplication.shared
        installKshrUnitTestWKWebViewPerformKeyEquivalentOverride()
        ...
        kshrUnitTestWKWebViewPerformKeyEquivalentHook = { currentWebView, event in
        ...
        window.makeKeyAndOrderFront(nil)
        defer {
            kshrUnitTestWKWebViewPerformKeyEquivalentHook = nil
            window.orderOut(nil)
        }
        ...
        XCTAssertEqual(
            ...
            0,
            "Did not expect KshrWebView to preflight Cmd+F into page content while Web Inspector is focused"
        )
    }
...
@MainActor
final class KshrWebViewContextMenuTests: XCTestCase {
    ...
    func testWillOpenMenuAddsOpenLinkInDefaultBrowserAndRoutesSelectionToDefaultBrowserOpener() {
        _ = NSApplication.shared
        let webView = KshrWebView(frame: NSRect(x: 0, y: 0, width: 800, height: 600), configuration: WKWebViewConfiguration())
        let menu = NSMenu()
...
final class BrowserSessionHistoryRestoreTests: XCTestCase {
    ...
    func testGoBackPrefersLiveWKWebViewHistoryBeforeRestoredFallback() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("kshr-browser-history-\(UUID().uuidString)", isDirectory: true)
    ...
    func testResetSidebarContextClearsBrowserPanelsIntoNewTabState() throws {
        ...
        workspace.telemetry.surfaceListeningPorts[contextPanelId] = [3000]
        ...
        XCTAssertTrue(workspace.telemetry.surfaceListeningPorts.isEmpty)
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        installKshrUnitTestInspectorOverride()
    }
    ...
    private func makePanelWithInspector(
        ...
        panel.webView.kshrSetUnitTestInspector(inspector)
...
@MainActor
final class KshrWebViewDragRoutingTests: XCTestCase {
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            KshrWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileDrops() {
        XCTAssertFalse(KshrWebView.shouldRejectInternalPaneDrag([.fileURL]))
    }
}
...
final class BrowserLinkOpenSettingsTests: XCTestCase {
    ...
    func testTerminalLinksDefaultToKshrBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInKshrBrowser(defaults: defaults))
    }

    func testTerminalLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInKshrBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openTerminalLinksInKshrBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInKshrBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInKshrBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksDefaultToKshrBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionDefaultsToKshrBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowser(defaults: defaults))
    }
    ...
    func testSettingsInitialOpenCommandInterceptionValueFallsBackToLegacyLinkToggleWhenUnset() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInKshrBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInKshrBrowserValue(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInKshrBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInKshrBrowserValue(defaults: defaults))
    }
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/kshr-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/kshr-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/kshr-local-test.html"))

...
APP_NAME="kshr STAGING"
BUNDLE_ID="com.kshrterm.app.staging"
BASE_APP_NAME="kshr"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
...
Release build with isolated "kshr STAGING" identity. Runs side-by-side with
the production kshr app.
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="kshr STAGING ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.kshrterm.app.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/kshr-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  KSHRD_SOCKET="${APP_SUPPORT_DIR}/kshrd-${STAGING_SLUG}.sock"
  KSHR_SOCKET="/tmp/kshr-${STAGING_SLUG}.sock"
...
"${OPEN_CLEAN_ENV[@]}" KSHR_SOCKET_PATH="$KSHR_SOCKET" KSHRD_UNIX_PATH="$KSHRD_SOCKET" open -g "$APP_PATH"

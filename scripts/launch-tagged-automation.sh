...
APP="$HOME/Library/Developer/Xcode/DerivedData/kshr-${TAG_SLUG}/Build/Products/Debug/kshr DEV ${TAG}.app"
BID="com.kshrterm.app.debug.${TAG_ID}"
SOCK="/tmp/kshr-debug-${TAG_SLUG}.sock"
DSOCK="$HOME/Library/Application Support/kshr/kshrd-dev-${TAG_SLUG}.sock"
LOG="/tmp/kshr-debug-${TAG_SLUG}.log"
...
sleep 0.5
pkill -f "kshr DEV ${TAG}.app/Contents/MacOS/kshr DEV" || true
rm -f "$SOCK" "$DSOCK"
sleep 0.5
...
OPEN_ENV=(
  ...
  -u KSHR_SHELL_INTEGRATION_DIR
  -u KSHR_LOAD_GHOSTTY_ZSH_INTEGRATION
  -u GHOSTTY_BIN_DIR
  ...
  -u TERMINFO
  -u XDG_DATA_DIRS
  "KSHR_SOCKET_MODE=${MODE}"
  "KSHR_SOCKET_PATH=${SOCK}"
  "KSHRD_UNIX_PATH=${DSOCK}"
  "KSHR_DEBUG_LOG=${LOG}"
)

...
LATEST_RELEASE_BUILD="$(
  curl -fsSL --max-time 8 https://github.com/yosefkohan26/kshr/releases/latest/download/appcast.xml 2>/dev/null \
...
# Update baked version constants in CLI binary so `cmux version` and
# `cmux --version` return the correct values without runtime disk/git walks.
BAKED_FILE="CLI/BakedVersionInfo.swift"
if [[ -f "$BAKED_FILE" ]]; then
  sed -i '' "s/static let version = \".*\"/static let version = \"$NEW_MARKETING\"/" "$BAKED_FILE"
  sed -i '' "s/static let build = \".*\"/static let build = \"$NEW_BUILD\"/" "$BAKED_FILE"
  echo "Updated $BAKED_FILE successfully."
fi

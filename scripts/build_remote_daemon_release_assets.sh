...
Builds kshrd-remote release assets for the supported remote platforms and emits:
  kshrd-remote-<goos>-<goarch>[-<suffix>]
  kshrd-remote-checksums[-<suffix>].txt
  kshrd-remote-manifest[-<suffix>].json
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build kshrd-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/kshrd-remote-* "$OUTPUT_DIR"/kshrd-remote-checksums.txt "$OUTPUT_DIR"/kshrd-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="kshrd-remote-checksums${SUFFIX_TAG}.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/kshrd-remote-manifest${SUFFIX_TAG}.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/kshrd-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="kshrd-remote-${GOOS}-${GOARCH}${SUFFIX_TAG}"
  ...
  BUILD_PATH="${OUTPUT_DIR}/kshrd-remote-${GOOS}-${GOARCH}.build"
  (
    cd "$DAEMON_ROOT"
    GOOS="$GOOS" \
    GOARCH="$GOARCH" \
...
echo "Built kshrd-remote assets in ${OUTPUT_DIR}"

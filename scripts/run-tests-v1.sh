...
# This runner is intended for the UTM macOS VM (ssh kshr-vm).
# It is intentionally guarded so we don't accidentally kill the host user's kshr instances.
if [ "$(id -un)" != "kshr" ]; then
  echo "ERROR: This script is intended to be run on the kshr-vm (user: kshr)." >&2
  echo "Run via: ssh kshr-vm 'cd /Users/kshr/GhosttyTabs && ./scripts/run-tests-v1.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/kshr-tests-v1"
APP="$DERIVED_DATA_PATH/Build/Products/Debug/kshr DEV.app"
RUN_TAG="tests-v1"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: kshr DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "kshr DEV" || true
  pkill -x "kshr" || true
  rm -f /tmp/kshr*.sock || true
}
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "kshr DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.kshrterm.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  KSHR_TAG="$RUN_TAG" KSHR_UI_TEST_MODE=1 "$APP/Contents/MacOS/kshr DEV" >/dev/null 2>&1 &

  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/kshr-debug*.sock /tmp/kshr*.sock 2>/dev/null | head -1 || true)
  ...
  if [ -z "$SOCK" ] || [ ! -S "$SOCK" ]; then
    echo "ERROR: Socket not ready (looked for /tmp/kshr*.sock)" >&2
    exit 1
  fi
  export KSHR_SOCKET_PATH="$SOCK"
  export KSHR_SOCKET="$SOCK"

  # Ensure LaunchServices has a visible/main window attached for rendering checks.
  KSHR_TAG="$RUN_TAG" open "$APP" >/dev/null 2>&1 || true
  sleep 0.5
...
from kshr import kshr  # type: ignore

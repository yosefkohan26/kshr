...
xcodebuild -project GhosttyTabs.xcodeproj -scheme kshr -configuration Release -destination 'platform=macOS' build
pkill -x kshr || true
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/kshr.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "kshr.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/kshr"
ATTEMPT=0
MAX_ATTEMPTS=20

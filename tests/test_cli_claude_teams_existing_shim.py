#!/usr/bin/env python3
"""
Regression test: `kshr claude-teams` reuses an existing tmux shim.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="kshr-claude-teams-shim-") as td:
        tmp = Path(td)
        home = tmp / "home"
        ...
        shim_dir = home / ".kshrterm" / "claude-teams-bin"
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `kshr claude-teams --version` failed with an existing shim")
    ...
    print("PASS: kshr claude-teams reuses an existing tmux shim")
    return 0

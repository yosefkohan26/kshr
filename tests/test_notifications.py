...
def wait_for_notifications(client: kshr, expected: int, timeout: float = 2.0) -> list[dict]:
    start = time.time()
...
def wait_for_flash_count(client: kshr, surface: str, minimum: int = 1, timeout: float = 2.0) -> int:
...
def ensure_two_surfaces(client: kshr) -> list[tuple[int, str, bool]]:
...
def focused_surface_index(client: kshr) -> int:
    surfaces = client.list_surfaces()
...
def send_osc(client: kshr, sequence: str, surface: Optional[int] = None) -> None:
...
def test_clear_prior_notifications(client: kshr) -> TestResult:
...
def test_suppress_when_focused(client: kshr) -> TestResult:
...
def test_not_suppressed_when_inactive(client: kshr) -> TestResult:
...
def test_kitty_notification_simple(client: kshr) -> TestResult:
...
def test_kitty_notification_chunked(client: kshr) -> TestResult:
...
def test_rxvt_notification_osc777(client: kshr) -> TestResult:
...
def test_mark_read_on_focus_change(client: kshr) -> TestResult:
...
def test_mark_read_on_app_active(client: kshr) -> TestResult:
...
def test_mark_read_on_tab_switch(client: kshr) -> TestResult:
...
def test_flash_on_tab_switch(client: kshr) -> TestResult:
...
def test_focus_on_notification_click(client: kshr) -> TestResult:
...
def test_restore_focus_on_tab_switch(client: kshr) -> TestResult:
...
def test_clear_on_tab_close(client: kshr) -> TestResult:

...
Requires:
  - kshr running with allowAll socket mode
  - bash shell integration sourced (kshr-bash-integration.bash)

Run with a tagged instance:
  KSHR_TAG=<tag> python3 tests/test_split_cwd_inheritance.py
"""
...
from kshr import kshr  # noqa: E402
...
def _focus_first_surface(client: kshr) -> str:
...
def main() -> int:
    ...
    socket_path = None
    if tag:
        socket_path = f"/tmp/kshr-debug-{tag}.sock"
    client = kshr(socket_path=socket_path)
    client.connect()

    # Use resolved paths to avoid /tmp -> /private/tmp symlink mismatch on macOS
    test_dir_a = str(Path("/tmp/kshr_split_cwd_test_a").resolve())
    test_dir_b = str(Path("/tmp/kshr_split_cwd_test_b").resolve())

...
OUTPUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/kshr-remote-assets-test.XXXXXX")"
trap 'rm -rf "$OUTPUT_DIR"' EXIT

"$ROOT_DIR/scripts/build_remote_daemon_release_assets.sh" \
  --version "0.62.0-test" \
  --release-tag "v0.62.0-test" \
  --repo "yosefkohan26/kshr" \
...
python3 - <<'PY' "$OUTPUT_DIR/kshrd-remote-manifest.json" "$OUTPUT_DIR/kshrd-remote-checksums.txt"
import json
import sys
...
if not manifest["checksumsURL"].endswith("/kshrd-remote-checksums.txt"):
...
# ------------------------------------------------------------------
SUFFIX_DIR="$(mktemp -d "${TMPDIR:-/tmp}/kshr-remote-assets-suffix-test.XXXXXX")"
...
for asset in \
  kshrd-remote-darwin-arm64-123456 \
  kshrd-remote-darwin-amd64-123456 \
  kshrd-remote-linux-arm64-123456 \
  kshrd-remote-linux-amd64-123456 \
  kshrd-remote-checksums-123456.txt \
  kshrd-remote-manifest-123456.json
do
...
python3 - <<'PY' "$SUFFIX_DIR/kshrd-remote-manifest-123456.json"
import json
import sys
from pathlib import Path

...
def wait_for_overlay_probe_ready(client: kshr, timeout_s: float = 8.0) -> None:
    start = time.time()
    last_error = None
    while time.time() - start < timeout_s:
        try:
            ...
            last_error = e
            time.sleep(0.1)
    raise kshrError(f"overlay_hit_gate probe unavailable: {last_error}")


def assert_gate(client: kshr, event_type: str, expected: bool, reason: str) -> None:
...
def assert_drop_gate(client: kshr, source: str, expected: bool, reason: str) -> None:
...
def assert_portal_gate(client: kshr, event_type: str, expected: bool, reason: str) -> None:
...
def assert_sidebar_gate(client: kshr, state: str, expected: bool, reason: str) -> None:
...
def main() -> int:
    ...
    if not os.path.exists(socket_path):
        ...
        print("Tip: start kshr first (or set KSHR_TAG / KSHR_SOCKET_PATH).")
        return 0

#!/usr/bin/env python3
"""Regression test: CLI should auto-discover tagged debug sockets from KSHR_TAG."""
...
def main() -> int:
    ...
    env = os.environ.copy()
    env["KSHR_SOCKET_PATH"] = "/tmp/kshr.sock"
    env["KSHR_TAG"] = tag
    env["KSHR_CLI_SENTRY_DISABLED"] = "1"
    env["KSHR_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    try:
        ...
        print(f"FAIL: invoking kshr ping failed: {exc}")
        return 1
    finally:
    ...
    if proc.returncode != 0:
        print("FAIL: kshr ping returned non-zero status")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return 1

    if proc.stdout.strip() != "PONG":
        print("FAIL: kshr ping did not use auto-discovered socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return 1

    print("PASS: kshr ping auto-discovers tagged socket from KSHR_TAG")
    return 0

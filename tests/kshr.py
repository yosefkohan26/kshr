...
A client library for programmatically controlling kshr via Unix socket.
...
_DEFAULT_DEBUG_BUNDLE_ID = "com.kshrterm.app.debug"
...
def _default_bundle_id() -> str:
    override = os.environ.get("KSHR_BUNDLE_ID")
    if override:
        return override

    tag = os.environ.get("KSHR_TAG")
    if tag:
...
def _default_socket_path() -> str:
    ...
    if tag:
        ...
        tagged_candidates = [
            f"/tmp/kshr-debug-{slug}.sock",
            f"/tmp/kshr-{slug}.sock",
        ]
    ...
    override = os.environ.get("KSHR_SOCKET_PATH")
    if override:
    ...
    tagged = glob.glob("/tmp/kshr-debug-*.sock")
    tagged.extend(glob.glob(os.path.join(_APP_SUPPORT_DIR, "kshr*.sock")))
...
class kshr:
    """Client for controlling kshr via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise kshrError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise kshrError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise kshrError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise kshrError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise kshrError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise kshrError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise kshrError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for kshr"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="kshr CLI")
    ...
    try:
        with kshr(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("kshr CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()

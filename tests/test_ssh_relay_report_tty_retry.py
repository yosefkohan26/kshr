    ...
    env.update(
        {
            ...
            "KSHR_TEST_LOG": str(log_path),
            "KSHR_TEST_STATE": str(state_path),
            "KSHR_TEST_BIN_DIR": str(bin_dir),
        }
    )
    command = f"""
source "{integration_path}"
PATH="$KSHR_TEST_BIN_DIR:$PATH"
hash -r 2>/dev/null || true
: > "{log_path}"
rm -f "{state_path}"
_KSHR_TTY_NAME={tty_name}
_KSHR_TTY_REPORTED=0
_kshr_report_tty_once || true
first="${{_KSHR_TTY_REPORTED}}:$(wc -l < "{log_path}" | tr -d ' ')"
_kshr_report_tty_once || true
second="${{_KSHR_TTY_REPORTED}}:$(wc -l < "{log_path}" | tr -d ' ')"
printf '%s\\n' "$first|$second"
""".strip()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="kshr-ssh-relay-report-tty-retry-") as td:
        tmp = Path(td)
        bin_dir = tmp / "bin"
        ...
        _write_executable(
            ...
            """#!/bin/sh
count=0
if [ -r "$KSHR_TEST_STATE" ]; then
    count=$(cat "$KSHR_TEST_STATE")
fi
count=$((count + 1))
printf '%s' "$count" > "$KSHR_TEST_STATE"
printf '%s\n' "$*" >> "$KSHR_TEST_LOG"
if [ "$count" -eq 1 ]; then
...
""",
        ...
        cases = [
            (
                "zsh",
                ["-f", "-c"],
                SHELL_DIR / "kshr-zsh-integration.zsh",
                "ttys777",
            ),
            (
                "bash",
                ["--noprofile", "--norc", "-c"],
                SHELL_DIR / "kshr-bash-integration.bash",
                "ttys888",
            ),
        ]

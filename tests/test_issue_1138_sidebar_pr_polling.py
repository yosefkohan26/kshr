...
def _git_stub() -> str:
    return textwrap.dedent(
        ...
        if [ "$1" = "remote" ] && [ "$2" = "get-url" ] && [ "$3" = "origin" ]; then
          printf 'https://github.com/yosefkohan26/kshr.git\\n'
          exit 0
        fi
...
def _gh_stub() -> str:
    return textwrap.dedent(
        """\
        #!/bin/sh
        args_log="${KSHR_TEST_GH_ARGS_LOG:?}"
        count_file="${KSHR_TEST_GH_COUNT_FILE:?}"
        pid_file="${KSHR_TEST_GH_PID_FILE:-}"
        scenario="${KSHR_TEST_SCENARIO:?}"
        head_file="${KSHR_TEST_HEAD_FILE:?}"
        ...
        case "$scenario" in
          prompt_helper_idle)
            printf '1138\\tOPEN\\thttps://github.com/yosefkohan26/kshr/pull/1138\\n'
            ;;
          ...
          branch_switch_clear)
            if [ "$branch" = "feature/old" ]; then
              printf '111\\tOPEN\\thttps://github.com/yosefkohan26/kshr/pull/111\\n'
              exit 0
            fi
          ...
          timeout_recovery)
            if [ "$count" -eq 1 ]; then
              ...
              sleep "${KSHR_TEST_HANG_SECONDS:-4}"
              exit 0
            fi
            printf '1138\\tOPEN\\thttps://github.com/yosefkohan26/kshr/pull/1138\\n'
            ;;
...
def _shell_command(kind: str, scenario: str) -> str:
    shared = {
        ...
        "nested_shell_stops_parent_poll": (
            ...
            '_KSHR_PR_POLL_INTERVAL=10\n'
            '_kshr_prompt_entry\n'
            'parent_pid="$_KSHR_PR_POLL_PID"\n'
            '[ -n "$parent_pid" ] || { echo "missing parent poll pid" >&2; exit 41; }\n'
            '_kshr_nested_shell_entry\n'
            'sleep 1\n'
            'if kill -0 "$parent_pid" 2>/dev/null; then\n'
            '  echo "parent poller still running" >&2\n'
            '  exit 42\n'
            'fi\n'
            'parent_watch_pid="${_KSHR_GIT_HEAD_WATCH_PID:-}"\n'
    ...
    if kind == "zsh":
        return textwrap.dedent(
            f"""\
            source "$KSHR_TEST_SCRIPT"
            _kshr_send() {{ print -r -- "$1" >> "$KSHR_TEST_SEND_LOG"; }}
            _kshr_prompt_entry() {{ _kshr_precmd; }}
            _kshr_nested_shell_entry() {{ _kshr_preexec zsh; }}
            _kshr_regular_command_entry() {{ _kshr_preexec "echo hi"; }}
            _kshr_cleanup() {{ _kshr_zshexit; }}
            {shared}"""
        )
    ...
    if kind == "bash":
        return textwrap.dedent(
            f"""\
            source "$KSHR_TEST_SCRIPT"
            _kshr_send() {{ printf '%s\\n' "$1" >> "$KSHR_TEST_SEND_LOG"; }}
            _kshr_prompt_entry() {{ _kshr_prompt_command; }}
            _kshr_nested_shell_entry() {{ _kshr_bash_preexec_hook bash; }}
            _kshr_regular_command_entry() {{ _kshr_bash_preexec_hook "echo hi"; }}
            _kshr_cleanup() {{ type _kshr_bash_cleanup >/dev/null 2>&1 && _kshr_bash_cleanup; }}
            {shared}"""
        )
...
def _report_line(number: int) -> str:
    return (
        f"report_pr {number} https://github.com/yosefkohan26/kshr/pull/{number} "
...
def _run_case(base: Path, *, shell: str, shell_args: list[str], script: Path, scenario: str) -> tuple[int, str]:
    ...
    env["KSHR_SOCKET_PATH"] = str(socket_path)
    env["KSHR_TAB_ID"] = "00000000-0000-0000-0000-000000000001"
    env["KSHR_PANEL_ID"] = "00000000-0000-0000-0000-000000000002"
    env["KSHR_TEST_SCRIPT"] = str(script)
    env["KSHR_TEST_REPO"] = str(repo)
    env["KSHR_TEST_SEND_LOG"] = str(send_log)
    env["KSHR_TEST_GH_COUNT_FILE"] = str(gh_count_file)
    env["KSHR_TEST_GH_ARGS_LOG"] = str(gh_args_log)
    env["KSHR_TEST_GH_PID_FILE"] = str(gh_pid_file)
    env["KSHR_TEST_SCENARIO"] = scenario
    env["KSHR_TEST_HEAD_FILE"] = str(head_file)
    env["KSHR_TEST_HANG_SECONDS"] = "4"
...
def main() -> int:
    ...
    cases = [
        ("zsh", ["-f", "-c"], root / "Resources" / "shell-integration" / "kshr-zsh-integration.zsh"),
        ("bash", ["--noprofile", "--norc", "-c"], root / "Resources" / "shell-integration" / "kshr-bash-integration.bash"),
    ]
    scenarios = [
        "prompt_helper_idle",
        "transient_same_context",
    ...
    base = Path("/tmp") / f"kshr_issue_1138_pr_poll_{os.getpid()}"
    try:

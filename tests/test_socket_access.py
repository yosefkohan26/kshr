...
In kshrOnly mode (default), only processes descended from the kshr
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: kshrOnly — external processes get rejected
  Phase 2: kshrOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("KSHR_TAG") or os.environ.get("KSHR_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/kshr DEV.app")
...
def _find_app():
    explicit = os.environ.get("KSHR_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/kshr-{preferred_slug}/Build/Products/Debug/kshr DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/kshr-{preferred_slug}/Build/Products/Debug/kshr DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/kshr DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/kshr-*/Build/Products/Debug/kshr DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/kshr-*/Build/Products/Debug/kshr DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "kshr")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/kshr"
    )))
    candidates.extend(glob.glob("/tmp/kshr-*/Build/Products/Debug/kshr"))
    candidates.extend(glob.glob("/private/tmp/kshr-*/Build/Products/Debug/kshr"))
...
def _kill_kshr(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/kshr DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "kshr DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_kshr(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"KSHR_SOCKET_MODE={mode}"]
    launch_env = {
        "KSHR_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a kshr-spawned terminal process CAN connect in kshrOnly mode.
    Inject a test via the shell rc file, then launch kshr in kshrOnly mode.
    The shell (a descendant of kshr) runs the test on startup.
    """
    result = TestResult("Internal process can connect (kshrOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"kshr_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"kshr_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing kshr, launch in kshrOnly mode (default)
        _kill_kshr(app_path)
        _launch_kshr(app_path, socket_path, mode="kshrOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in kshrOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify KSHR_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_kshr(app_path)
        _launch_kshr(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"kshr-pass-{os.getpid()}"
    try:
        _kill_kshr(app_path)
        _launch_kshr(
            app_path,
            socket_path,
            mode="password",
            extra_env={"KSHR_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find kshr CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("kshr Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find kshr DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/kshr-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: kshrOnly — external rejection ──
    print("Phase 1: kshrOnly mode — external rejection")
    print("-" * 50)

    # Ensure kshr is running in kshrOnly mode
    _kill_kshr(app_path)
    print("  Launching kshr in kshrOnly mode...")
    _launch_kshr(app_path, socket_path, mode="kshrOnly")
    ...
    # ── Phase 2: kshrOnly — internal process CAN connect ──
    print("Phase 2: kshrOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave kshr in kshrOnly mode ──
    _kill_kshr(app_path)
    _launch_kshr(app_path, socket_path, mode="kshrOnly")

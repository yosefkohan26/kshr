...
def run_wrapper(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="kshr-claude-wrapper-test-") as td:
        tmp = Path(td)
        ...
        hook_kshr_bin_log = tmp / "hook-kshr-bin.log"
        kshr_log = tmp / "kshr.log"
        socket_path = str(tmp / "kshr.sock")
...
printf '%s\\n' "${KSHR_CLAUDE_HOOK_KSHR_BIN-__UNSET__}" > "$FAKE_HOOK_KSHR_BIN_LOG"
for arg in "$@"; do
...
""",
        ...
        make_executable(
            ...
            """#!/usr/bin/env bash
set -euo pipefail
printf '%s timeout=%s\\n' "$*" "${KSHRTERM_CLI_RESPONSE_TIMEOUT_SEC-__UNSET__}" >> "$FAKE_KSHR_LOG"
if [[ "${1:-}" == "--socket" ]]; then
  shift 2
fi
if [[ "${1:-}" == "ping" ]]; then
  if [[ "${FAKE_KSHR_PING_OK:-0}" == "1" ]]; then
    exit 0
  fi
  exit 1
fi
...
""",
        ...
        bundled_cli_path = bundled_dir / "kshr"
        make_executable(
            bundled_cli_path,
            """#!/usr/bin/env bash
exit 0
""",
        ...
        env["KSHR_SURFACE_ID"] = "surface:test"
        env["KSHR_SOCKET_PATH"] = socket_path
        ...
        env["FAKE_HOOK_KSHR_BIN_LOG"] = str(hook_kshr_bin_log)
        env["FAKE_KSHR_LOG"] = str(kshr_log)
        env["FAKE_KSHR_PING_OK"] = "1" if socket_state == "live" else "0"
        env["KSHR_BUNDLED_CLI_PATH"] = str(bundled_cli_path)
        ...
        hook_kshr_bin_lines = read_lines(hook_kshr_bin_log)
        ...
        hook_kshr_bin_value = hook_kshr_bin_lines[0] if hook_kshr_bin_lines else ""
        return (
            proc.returncode,
...
def test_live_socket_injects_supported_hooks(failures: list[str]) -> None:
    code, real_argv, kshr_log, stderr, claudecode, node_options, runtime_node_options, child_node_options, hook_kshr_bin = run_wrapper(
        socket_state="live",
        argv=["hello"],
    )
    ...
    expect(any(" ping" in line for line in kshr_log), f"live socket: expected kshr ping, got {kshr_log}", failures)
    expect(
        any("timeout=0.75" in line for line in kshr_log),
        f"live socket: expected bounded ping timeout, got {kshr_log}",
        failures,
    )
    ...
    expect(hook_kshr_bin.endswith("/bundled cli/kshr"), f"live socket: expected bundled kshr pin, got {hook_kshr_bin!r}", failures)
    ...
    for hook_name, expected_subcommand in {
        ...
        expect(
            hook_command == f'"${{KSHR_CLAUDE_HOOK_KSHR_BIN:-kshr}}" claude-hook {expected_subcommand}',
            f"{hook_name} hook should pin bundled kshr, got {hook_command!r}",
            failures,
        )
...
def test_live_socket_tmpdir_failure_skips_node_options_injection(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="kshr-claude-wrapper-bad-tmp-") as td:
        bad_tmpdir = Path(td) / "not-a-directory"
        bad_tmpdir.write_text("occupied", encoding="utf-8")
        code, real_argv, kshr_log, stderr, claudecode, node_options, runtime_node_options, child_node_options, _ = run_wrapper(
            socket_state="live",
            argv=["hello"],
    ...
    expect(any(" ping" in line for line in kshr_log), f"tmpdir failure: expected kshr ping, got {kshr_log}", failures)
...
def test_missing_socket_skips_hook_injection(failures: list[str]) -> None:
    ...
    expect(kshr_log == [], f"missing socket: expected no kshr calls, got {kshr_log}", failures)
    ...
    expect(hook_kshr_bin == "__UNSET__", f"missing socket: expected hook kshr unset, got {hook_kshr_bin!r}", failures)
...
def test_stale_socket_skips_hook_injection(failures: list[str]) -> None:
    ...
    expect(any(" ping" in line for line in kshr_log), f"stale socket: expected kshr ping probe, got {kshr_log}", failures)
    expect(
        any("timeout=0.75" in line for line in kshr_log),
        f"stale socket: expected bounded ping timeout, got {kshr_log}",
        failures,
    )
    ...
    expect(hook_kshr_bin == "__UNSET__", f"stale socket: expected hook kshr unset, got {hook_kshr_bin!r}", failures)

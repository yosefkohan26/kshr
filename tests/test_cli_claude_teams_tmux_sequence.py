#!/usr/bin/env python3
"""
Regression test: `kshr claude-teams` supports Claude's tmux teammate flow.
"""
    ...
    def handle(self, method: str, params: dict[str, object]) -> dict[str, object]:
        with self.lock:
            ...
            if method == "surface.send_text":
                return {"ok": True}
            raise RuntimeError(f"Unsupported fake kshr method: {method}")
...
class FakeKshrUnixServer(socketserver.ThreadingUnixStreamServer):
    allow_reuse_address = True

    def __init__(self, socket_path: str, state: FakeKshrState) -> None:
        self.state = state
        super().__init__(socket_path, FakeKshrHandler)


class FakeKshrHandler(socketserver.StreamRequestHandler):
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="kshr-claude-teams-seq-") as td:
        tmp = Path(td)
        home = tmp / "home"
        home.mkdir(parents=True, exist_ok=True)

        socket_path = tmp / "fake-kshr.sock"
        state = FakeKshrState()
        server = FakeKshrUnixServer(str(socket_path), state)
...
printf '%s\\n' "${KSHR_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_LOG"
...
""",
        ...
        try:
            ...
            print("FAIL: `kshr claude-teams --version` timed out")
        ...
        if socket_value != str(socket_path):
            print(f"FAIL: expected KSHR_SOCKET_PATH={socket_path}, got {socket_value!r}")
            return 1

        window_target = read_text(window_target_log)
        if window_target != "kshr:1":
            print(f"FAIL: expected tmux window target 'kshr:1', got {window_target!r}")
            return 1
    ...
    print("PASS: kshr claude-teams supports Claude's tmux teammate flow")
    return 0

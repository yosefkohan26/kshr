...
Run with a tagged instance to avoid unix socket conflicts:
  KSHR_TAG=<tag> python3 tests/test_sidebar_ports.py
"""
...
from kshr import kshr, kshrError  # noqa: E402
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside kshr and ensure it is actually listening.
...
def _start_agent_server(base: Path, port: int, pid_file: Path, log_file: Path) -> subprocess.Popen:
    """
    Start a long-lived "agent" shell outside kshr. The shell owns a child
...
def _wait_for_port(client: kshr, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: kshr, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state())
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
def _assert_port_absent_for_duration(client: kshr, port: int, duration: float = 6.0, interval: float = 0.15) -> None:
    """
...
def main() -> int:
    tag = os.environ.get("KSHR_TAG") or ""
    if not tag:
        print("Tip: set KSHR_TAG=<tag> when running this test to avoid socket conflicts.")

    base = Path("/tmp") / f"kshr_ports_test_{os.getpid()}"
    ...
    try:
        ...
        # Start a listening server outside kshr. A fresh tab should NOT show this port,
        ...
        with kshr() as client:
            ...
            client.send("echo kshr_ports_test\n")
    ...
    except (kshrError, AssertionError, RuntimeError, ValueError) as e:

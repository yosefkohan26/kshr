...
Run with a tagged instance to avoid unix socket conflicts:
  KSHR_TAG=<tag> python3 tests/test_sidebar_cwd_git.py
"""
...
def _init_git_repo(repo: Path) -> None:
    ...
    _git(repo, "init")
    _git(repo, "config", "user.email", "kshr-test@example.com")
    _git(repo, "config", "user.name", "kshr-test")
...
def main() -> int:
    ...
    base = Path("/tmp") / f"kshr_sidebar_test_{os.getpid()}"
    repo = base / "repo"
    other = base / "other"
    ...
    except (kshrError, subprocess.CalledProcessError, AssertionError) as e:

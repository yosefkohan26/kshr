...
def run_wrapper(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="kshr-open-wrapper-test-") as td:
        tmp = Path(td)
        wrapper = tmp / "open"
...
key="${3:-}"
case "$key" in
  browserInterceptTerminalOpenCommandInKshrBrowser)
    ...
    exit 1
    ;;
  browserOpenTerminalLinksInKshrBrowser)
...
if [[ -n "${FAKE_KSHR_FAIL_URLS:-}" ]]; then
  IFS=',' read -r -a failures <<< "$FAKE_KSHR_FAIL_URLS"
...
""",
        ...
        env = os.environ.copy()
        env["KSHR_SOCKET_PATH"] = "/tmp/kshr-open-wrapper-test.sock"
        env["KSHR_BUNDLE_ID"] = "com.kshrterm.app.debug.test"
        env["KSHR_OPEN_WRAPPER_SYSTEM_OPEN"] = str(system_open)
        env["KSHR_OPEN_WRAPPER_DEFAULTS"] = str(defaults)
        env["FAKE_OPEN_LOG"] = str(open_log)
        env["FAKE_KSHR_LOG"] = str(kshr_log)
        if python_bin is None:
            env.pop("KSHR_OPEN_WRAPPER_PYTHON3", None)
        else:
            env["KSHR_OPEN_WRAPPER_PYTHON3"] = python_bin
        ...
        if fail_urls:
            env["FAKE_KSHR_FAIL_URLS"] = ",".join(fail_urls)
        else:
            env.pop("FAKE_KSHR_FAIL_URLS", None)
        ...
        return read_log(open_log), read_log(kshr_log), result.returncode, result.stderr.strip()
...
def test_toggle_disabled_passthrough(failures: list[str]) -> None:
    ...
    open_log, kshr_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="0",
        whitelist="",
    )
    expect(code == 0, f"toggle off: wrapper exited {code}: {stderr}", failures)
    expect(kshr_log == [], f"toggle off: kshr should not be called, got {kshr_log}", failures)
...
def test_toggle_disabled_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [],
        f"toggle off (case-insensitive): kshr should not be called, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_whitelist_miss_passthrough(failures: list[str]) -> None:
    ...
    expect(kshr_log == [], f"whitelist miss: kshr should not be called, got {kshr_log}", failures)
    expect(open_log == [url], f"whitelist miss: expected system open [{url}], got {open_log}", failures)


def test_whitelist_match_routes_to_kshr(failures: list[str]) -> None:
    ...
    expect(kshr_log == [f"browser open {url}"], f"whitelist match: unexpected kshr log {kshr_log}", failures)
...
def test_external_literal_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [f"browser open {url}"],
        f"external literal deferred: expected wrapper to pass URL to kshr, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [f"browser open {url}"],
        f"external regex deferred: expected wrapper to pass URL to kshr, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_with_icu_features_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [f"browser open {url}"],
        f"external regex icu deferred: expected wrapper to pass URL to kshr, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_invalid_regex_is_ignored_silently(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [f"browser open {url}"],
        f"external invalid regex: expected kshr open for {url}, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_partial_failures_only_fallback_failed_urls(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [f"browser open {good}", f"browser open {failed}"],
        f"partial failure: kshr log mismatch {kshr_log}",
        failures,
    )
    expect(
...
def test_legacy_toggle_fallback_passthrough(failures: list[str]) -> None:
    ...
    expect(kshr_log == [], f"legacy fallback: kshr should not be called, got {kshr_log}", failures)
...
def test_legacy_toggle_fallback_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [],
        f"legacy fallback (case-insensitive): kshr should not be called, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_uppercase_scheme_routes_to_kshr(failures: list[str]) -> None:
    url = "HTTPS://api.example.com/path?q=1"
    open_log, kshr_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(kshr_log == [f"browser open {url}"], f"uppercase scheme: unexpected kshr log {kshr_log}", failures)


def test_local_html_file_routes_to_kshr(failures: list[str]) -> None:
    ...
    expect(len(kshr_log) == 1, f"local html file: expected exactly one kshr call, got {kshr_log}", failures)
    if kshr_log:
        expect(
            kshr_log[0].startswith("browser open file://"),
            f"local html file: expected file:// target, got {kshr_log[0]}",
            failures,
        )
        expect(
            "hello%20page.HTML" in kshr_log[0],
            f"local html file: expected URL-encoded filename in kshr target, got {kshr_log[0]}",
            failures,
        )


def test_file_url_html_routes_to_kshr(failures: list[str]) -> None:
    url = "file:///tmp/kshr-open-wrapper-fixture.html"
    open_log, kshr_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
        whitelist="",
    ...
    expect(kshr_log == [f"browser open {url}"], f"file url html: unexpected kshr log {kshr_log}", failures)


def test_file_url_html_routes_to_kshr_without_python_binary(failures: list[str]) -> None:
    url = "file:///tmp/kshr-open-wrapper-fixture.html"
    open_log, kshr_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(
        kshr_log == [f"browser open {url}"],
        f"file url html no-python fallback: unexpected kshr log {kshr_log}",
        failures,
    )


def test_local_html_file_routes_to_kshr_without_python_binary(failures: list[str]) -> None:
    ...
    expect(
        len(kshr_log) == 1,
        f"local html no-python fallback: expected exactly one kshr call, got {kshr_log}",
        failures,
    )
    if kshr_log:
        expect(
            kshr_log[0].startswith("browser open file://"),
            f"local html no-python fallback: expected file:// target, got {kshr_log[0]}",
            failures,
        )
        expect(
            "no%20python%20fallback.html" in kshr_log[0],
            f"local html no-python fallback: expected URL-encoded filename, got {kshr_log[0]}",
            failures,
        )
...
def test_domain_like_html_argument_passthrough(failures: list[str]) -> None:
    ...
    expect(
        kshr_log == [],
        f"domain-like html argument: kshr should not be called, got {kshr_log}",
        failures,
    )
    expect(
        open_log == [arg],
...
def test_non_file_scheme_html_passthrough(failures: list[str]) -> None:
    ...
    expect(kshr_log == [], f"non-file scheme html: kshr should not be called, got {kshr_log}", failures)
...
def test_mailto_html_passthrough(failures: list[str]) -> None:
    ...
    expect(kshr_log == [], f"mailto html: kshr should not be called, got {kshr_log}", failures)
...
def test_local_non_html_file_passthrough(failures: list[str]) -> None:
    ...
    expect(kshr_log == [], f"local non-html file: kshr should not be called, got {kshr_log}", failures)
...
def test_unicode_whitelist_matches_punycode_url(failures: list[str]) -> None:
    ...
    expect(kshr_log == [f"browser open {url}"], f"unicode whitelist: unexpected kshr log {kshr_log}", failures)
...
def test_punycode_whitelist_matches_unicode_url(failures: list[str]) -> None:
    ...
    expect(kshr_log == [f"browser open {url}"], f"punycode whitelist: unexpected kshr log {kshr_log}", failures)
...
def main() -> int:
    ...
    test_whitelist_match_routes_to_kshr(failures)
    ...
    test_uppercase_scheme_routes_to_kshr(failures)
    test_local_html_file_routes_to_kshr(failures)
    test_file_url_html_routes_to_kshr(failures)
    test_file_url_html_routes_to_kshr_without_python_binary(failures)
    test_local_html_file_routes_to_kshr_without_python_binary(failures)

        ...
        try:
            ...
            with conn:
                ...
                if not self.commands:
                    raise RuntimeError("kshr CLI never sent a command to the teardown test socket")
...
def main() -> int:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="kshr-claude-hook-stop-")
    try:
        root = Path(temp_dir.name)
        socket_path = str(root / "kshr.sock")
        ...
        env["KSHR_WORKSPACE_ID"] = str(uuid.uuid4())
        env["KSHR_SURFACE_ID"] = str(uuid.uuid4())

...
- Summary:
  ...
  - Adapts the parser to upstream's newer capture API so the kshr OSC 99 hook survives the March 30 upstream sync.

### 4) kshr theme picker helper hooks

- Commits:
  - `1da7281fd` (Add kshr theme picker helper hooks)
  - `ea482b73e` (Fix kshr theme picker preview writes)
  - `c7ab66056` (Improve kshr theme picker footer contrast)
  - `c49f69f7b` (Respect system theme in kshr picker)
  - `599b0ff43` (Skip theme detection in kshr picker)
  - `b75388d95` (Match Ghostty theme picker startup)
  - `f985d2d04` (Harden kshr theme override writes)
- Files:
  - `build.zig`
  - `src/cli/list_themes.zig`
  - `src/main_ghostty.zig`
- Summary:
  - Adds a `zig build cli-helper` step so kshr can bundle Ghostty's CLI helper binary on macOS.
  - Lets `+list-themes` switch into a kshr-managed mode via env vars, writing the kshr theme override file and posting the existing kshr reload notification for live app-wide preview.
  - Keeps the preview UI readable in light mode, matches upstream picker startup behavior, and hardens writes to the kshr-managed theme override file.
...
- Summary:
  ...
  - Sends the initial DSR 997 report as soon as mode 2031 is enabled, which kshr relies on for immediate color-scheme awareness.

### 6) Keyboard copy mode selection C API

- Commit: `0b231db94` (Re-export kshr selection APIs removed from upstream)
- Files:
  - `include/ghostty.h`
  - `src/Surface.zig`
  - `src/apprt/embedded.zig`
- Summary:
  ...
  - Keeps kshr keyboard copy mode working against the refreshed Ghostty base after upstream removed those exports.
...
- Summary:
  ...
  - Replays the layer-background restore on top of the refreshed Ghostty base so kshr keeps the resize-coverage fix after the upstream sync.
...
- Dropped during the March 30, 2026 rebase because newer Ghostty prompt-marking changes on the refreshed base superseded these fork-only zsh redraw patches, so kshr no longer carries them separately.
...
- `src/cli/list_themes.zig`
  - kshr now relies on the upstream picker UI plus local env-driven hooks for live preview and restore.
    If upstream reorganizes the preview loop or key handling, re-check the kshr mode path and keep the
    stock Ghostty behavior unchanged when the kshr env vars are absent.

- `build.zig`
  - Upstream's new wasm/libghostty work touched the same build graph. Keep the kshr-only `cli-helper`
...
- `include/ghostty.h`, `src/Surface.zig`, `src/apprt/embedded.zig`
  - Upstream removed kshr-used selection exports. Preserve the re-exported

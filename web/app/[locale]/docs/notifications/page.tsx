  ...
  return (
    <>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><code>KSHR_NOTIFICATION_TITLE</code></td>
            <td>{t("envTitle")}</td>
          </tr>
          <tr>
            <td><code>KSHR_NOTIFICATION_SUBTITLE</code></td>
            <td>{t("envSubtitle")}</td>
          </tr>
          <tr>
            <td><code>KSHR_NOTIFICATION_BODY</code></td>
            <td>{t("envBody")}</td>
          </tr>
        </tbody>
      </table>
      <CodeBlock title="Examples" lang="bash">{`# Text-to-speech
say "$KSHR_NOTIFICATION_TITLE"

# Custom sound file
afplay /path/to/sound.aiff

# Log to file
echo "$KSHR_NOTIFICATION_TITLE: $KSHR_NOTIFICATION_BODY" >> ~/notifications.log`}</CodeBlock>
      <p>{t("customCommandNote")}</p>

      <h2>{t("sending")}</h2>

      <h3>{t("cli")}</h3>
      <CodeBlock lang="bash">{`kshr notify --title "Task Complete" --body "Your build finished"
kshr notify --title "Claude Code" --subtitle "Waiting" --body "Agent needs input"`}</CodeBlock>
...
printf '\\e]99;i=1;e=1;d=1;p=body:All tests passed\\e\\\\'`}</CodeBlock>
      ...
      <CodeBlock title="~/.claude/hooks/kshr-notify.sh" lang="bash">{`#!/bin/bash
# Skip if not in kshr
[ -S /tmp/kshr.sock ] || exit 0
...
case "$EVENT_TYPE" in
    "Stop")
        kshr notify --title "Claude Code" --body "Session complete"
        ;;
    "PostToolUse")
        [ "$TOOL" = "Task" ] && kshr notify --title "Claude Code" --body "Agent finished"
        ;;
esac`}</CodeBlock>
      <CodeBlock lang="bash">{`chmod +x ~/.claude/hooks/kshr-notify.sh`}</CodeBlock>
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/kshr-notify.sh"
          }
        ]
      }
    ],
  ...
  "hooks": {
    ...
    "agentStop": [
      {
        "type": "command",
        "bash": "if command -v kshr &>/dev/null; then kshr notify --title 'Copilot CLI' --body 'Done'; kshr set-status copilot_cli Idle; fi",
        "timeoutSec": 5
      }
    ],
    "errorOccurred": [
      {
        "type": "command",
        "bash": "if command -v kshr &>/dev/null; then kshr notify --title 'Copilot CLI' --subtitle 'Error' --body 'An error occurred'; kshr set-status copilot_cli Error; fi",
        "timeoutSec": 5
      }
    ],
    "sessionEnd": [
...
notify-after() {
  ...
  if [ $exit_code -eq 0 ]; then
    kshr notify --title "✓ Command Complete" --body "$1"
  else
    kshr notify --title "✗ Command Failed" --body "$1 (exit $exit_code)"
  fi
  return $exit_code
}

...
import settingsSchema from "../../../../data/kshr-settings.schema.json";
import { shortcutCategories, type LocalizedText } from "../../../../data/kshr-shortcuts";
...
const schemaUrl =
  ...
  "https://raw.githubusercontent.com/yosefkohan26/kshr/main/web/data/kshr-settings.schema.json";
const schemaSourceUrl =
  "https://github.com/yosefkohan26/kshr/blob/main/web/data/kshr-settings.schema.json";
const sectionOrder = [
  "app",
  "notifications",
  "sidebar",
...
const settingsFileExample = `{
  ...
  // "browser": {
  //   "openTerminalLinksInKshrBrowser": true,
...
working-directory = ~/code`}</CodeBlock>
      ...
      <h2>kshr settings.json</h2>
      <p>
        kshr keeps app-owned settings in a separate user file instead of mixing them into Ghostty
        config. On launch, if neither settings location exists, kshr writes a commented template to{" "}
        <code>~/.config/kshr/settings.json</code>.
      </p>
      <ol>
        <li>
          <code>~/.config/kshr/settings.json</code>
        ...
        <li>
          <code>~/Library/Application Support/com.kshrterm.app/settings.json</code>
        </li>
      </ol>
      <Callout type="info">
        <strong>Precedence:</strong> <code>~/.config/kshr/settings.json</code> wins over the
      ...
      <Callout type="info">
        ...
        <code>kshr reload-config</code> to re-read it without restarting the app.
      </Callout>
      <Callout type="warn">
        <strong>Migrations:</strong> keep <code>schemaVersion</code> at <code>1</code> for now.
        Future kshr versions will use that field for upgrades. If kshr sees a newer schema version,
      ...
      <CodeBlock title="~/.config/kshr/settings.json" lang="json">
        {settingsFileExample}
      </CodeBlock>
      ...
      <p>
        ...
        browser, sidebar, notifications, automation, and kshr-owned keyboard shortcuts all live
        here.
      </p>
  ...
  "workspaceColors": {
    "colors": {
      ...
      <p>
        The defaults below are the same kshr-owned actions listed on the{" "}

...
export default function GettingStartedPage() {
  ...
  return (
    <>
      ...
      <h3>{t("homebrew")}</h3>
      <CodeBlock lang="bash">{`brew tap yosefkohan26/kshr
brew install --cask kshr`}</CodeBlock>
      <p>{t("updateLater")}</p>
      <CodeBlock lang="bash">{`brew upgrade --cask kshr`}</CodeBlock>
      ...
      <h2>{t("cliSetup")}</h2>
      <p>{t("cliDesc")}</p>
      <CodeBlock lang="bash">{`sudo ln -sf "/Applications/kshr.app/Contents/Resources/bin/kshr" /usr/local/bin/kshr`}</CodeBlock>
      <p>{t("cliThen")}</p>
      <CodeBlock lang="bash">{`kshr list-workspaces
kshr notify --title "Build Complete" --body "Your build finished"`}</CodeBlock>

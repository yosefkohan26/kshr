...
const feedbackRecipient = "feedback@yosefkohan26.com";
...
function resolveFeedbackConfig() {
  ...
  const fromEmail = env.KSHR_FEEDBACK_FROM_EMAIL;
  const rateLimitId = env.KSHR_FEEDBACK_RATE_LIMIT_ID;
...
function buildSubject(email: string, message: string, appVersion: string) {
  ...
  return `kshr feedback from ${email}${versionSuffix}: ${summary}`;
}
...
function buildHtmlBody(input: {
  ...
  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#111827;line-height:1.5">
      <h1 style="font-size:18px;margin:0 0 16px">kshr feedback</h1>

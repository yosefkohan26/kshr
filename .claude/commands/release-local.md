...
If the script fails, run `say "kshr release failed"`.

### 7. Verify homebrew cask

- Run `bash tests/test_homebrew_sha.sh` to confirm the cask SHA matches the release DMG
- Update the homebrew-kshr submodule pointer: `git add homebrew-kshr && git commit -m "Update homebrew-kshr submodule to latest" && git push origin main`

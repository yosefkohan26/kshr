...
fileprivate func kshrVsyncIOSurfaceTimelineCallback(
    ...
    // Capture the current frame index under the lock to detect stale callbacks.
    // The tryBeginCapture() gate above already serializes; we snapshot the frame
    // number here so the async block can skip if another callback already advanced it.
    let capturedFrame = st.framesWritten

    // Dispatch to main asynchronously so the display-link thread is never blocked
    // waiting for main (which prevents frame stalls when SwiftUI layout is busy).
    // The tryBeginCapture/endCapture lock ensures at most one async block is in-flight
    // at a time, preserving frame ordering without synchronous blocking.
    DispatchQueue.main.async {
        defer { st.endCapture() }
        // Stale-frame guard: if framesWritten has moved past our snapshot, another
        // callback already handled this slot — skip.
        guard st.framesWritten == capturedFrame, st.framesWritten < st.frameCount else { return }
        ...
        // Check completion inside the async block where framesWritten is up to date.
...
class TabManager: ObservableObject {
    ...
    /// O(1) lookup of workspaces by ID. Rebuilt whenever `tabs` changes.
    private(set) var tabsById: [UUID: Workspace] = [:]

    /// Cached count of pinned workspaces. Rebuilt whenever `tabs` changes.
    private(set) var pinnedCount: Int = 0

    @Published private(set) var isWorkspaceCycleHot: Bool = false
    @Published private(set) var pendingBackgroundWorkspaceLoadIds: Set<UUID> = []
    @Published private(set) var debugPinnedWorkspaceLoadIds: Set<UUID> = []

    /// Global monotonically increasing counter for KSHR_PORT ordinal assignment.
    ...
    private let initialWorkspaceGitProbeQueue = DispatchQueue(
        label: "com.kshr.initial-workspace-git-probe",
        qos: .utility
    )
    ...
    /// Pause periodic background timers (agent PID sweep, git metadata polling,
    /// port scanning) to save CPU/battery when all windows are hidden or occluded.
    func suspendBackgroundWork() {
        ...
        PortScanner.shared.suspend()
    }

    /// Resume periodic background timers after a `suspendBackgroundWork()` call.
    ...
    func scheduleInitialWorkspaceGitMetadataRefreshIfPossible(
        ...
        guard let workspace = tabsById[workspaceId],
    ...
    /// Rebuild the `tabsById` dictionary and `pinnedCount` from the current `tabs` array.
    private func rebuildTabIndex() {
        var dict = [UUID: Workspace](minimumCapacity: tabs.count)
        var pinned = 0
        for tab in tabs {
            dict[tab.id] = tab
            if tab.isPinned { pinned += 1 }
        }
        tabsById = dict
        pinnedCount = pinned
    }
    ...
    func workspace(byId id: UUID) -> Workspace? {
        tabsById[id]
    }

    var selectedWorkspace: Workspace? {
        guard let selectedTabId else { return nil }
        return tabsById[selectedTabId]
    }
    ...
    private func maybeMutateSelectionDuringWorkspaceCreationForDev(
        ...
        let isEnabled: Bool = {
            if let raw = env["KSHR_DEV_MUTATE_WORKSPACE_SELECTION_DURING_CREATION"] {
                return raw == "1" || raw.caseInsensitiveCompare("true") == .orderedSame
            }
            return UserDefaults.standard.bool(forKey: "kshrDevMutateWorkspaceSelectionDuringCreation")
        }()
        guard isEnabled,
        ...
        if let terminalPanel = workspace.focusedTerminalPanel,
           terminalPanel.surface.surface != nil {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                terminalPanel.sendText("kshr welcome\n")
            }
            return
        }
    ...
    private func applyWorkspaceGitMetadataSnapshot(
        ...
        guard let workspace = tabsById[probeKey.workspaceId] else {
            clearWorkspaceGitProbe(probeKey)
            return
        }
    ...
    private func inheritedTerminalConfigForNewWorkspace() -> KshrSurfaceConfigTemplate? {
    ...
    func closeCurrentWorkspace() {
        guard let selectedId = selectedTabId,
              let workspace = tabsById[selectedId] else { return }
...
#endif
        guard let selectedId = selectedTabId,
              let tab = tabsById[selectedId] else { return }
    ...
    func closeWorkspaceWithConfirmation(tabId: UUID) -> Bool {
        guard let workspace = tabsById[tabId] else { return false }
...
#if DEBUG
        if ProcessInfo.processInfo.environment["KSHR_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] == "1" {
            return true
        }
#endif
    ...
    private func focusSelectedTabPanel(previousTabId: UUID?) {
        guard let selectedTabId,
              let tab = tabsById[selectedTabId] else { return }
        ...
        if let previousTabId,
           let previousTab = tabsById[previousTabId],
    ...
    private func updateWindowTitleForSelectedTab() {
        guard let selectedTabId,
              let tab = tabsById[selectedTabId] else {
    ...
    private func windowTitle(for tab: Workspace?) -> String {
        guard let tab else { return "kshr" }
        ...
        return trimmedDirectory.isEmpty ? "kshr" : trimmedDirectory
    }
    ...
    private func setupUITestFocusShortcutsIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_FOCUS_SHORTCUTS"] == "1" else { return }
    ...
    private func setupSplitCloseRightUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_SETUP"] == "1" else { return }
        guard let path = env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_PATH"], !path.isEmpty else { return }
        let visualMode = env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1"
        let shotsDir = (env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_SHOTS_DIR"] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        let visualIterations = Int((env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_ITERATIONS"] ?? "20").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 20
        let burstFrames = Int((env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_BURST_FRAMES"] ?? "6").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 6
        let closeDelayMs = Int((env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_CLOSE_DELAY_MS"] ?? "70").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 70
        let pattern = (env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_PATTERN"] ?? "close_right")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        for i in 1...iterations {
            ...
            sendText(topLeftId, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo KSHR_SPLIT_TOPLEFT_\(i); done; printf '\\033[HKSHR_MARKER_TOPLEFT\\n'\r")
            sendText(topRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo KSHR_SPLIT_TOPRIGHT_\(i); done; printf '\\033[HKSHR_MARKER_TOPRIGHT\\n'\r")
            if let bottomLeft {
                sendText(bottomLeft.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo KSHR_SPLIT_BOTTOMLEFT_\(i); done; printf '\\033[HKSHR_MARKER_BOTTOMLEFT\\n'\r")
            }
            if let bottomRight {
                sendText(bottomRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo KSHR_SPLIT_BOTTOMRIGHT_\(i); done; printf '\\033[HKSHR_MARKER_BOTTOMRIGHT\\n'\r")
            }
	    ...
	    private func captureVsyncIOSurfaceTimeline(
	        ...
	        await withCheckedContinuation { (cont: CheckedContinuation<Void, Never>) in
	            ...
	            CVDisplayLinkSetOutputCallback(link, kshrVsyncIOSurfaceTimelineCallback, ctx)
    ...
    private func setupChildExitSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_CHILD_EXIT_SPLIT_SETUP"] == "1" else { return }
        guard let path = env["KSHR_UI_TEST_CHILD_EXIT_SPLIT_PATH"], !path.isEmpty else { return }
        let requestedIterations = Int(env["KSHR_UI_TEST_CHILD_EXIT_SPLIT_ITERATIONS"] ?? "1") ?? 1
    ...
    private func setupChildExitKeyboardUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1" else { return }
        guard let path = env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"], !path.isEmpty else { return }
        let autoTrigger = env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] == "1"
        let strictKeyOnly = env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] == "1"
        let triggerMode = (env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_TRIGGER_MODE"] ?? "shell_input")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let layout = (env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] ?? "lr")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let expectedPanelsAfter = max(
            1,
            Int((env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] ?? "1")
                .trimmingCharacters(in: .whitespacesAndNewlines)
            ) ?? 1
        )
...
extension TabManager {
    func sessionAutosaveFingerprint() -> Int {
        ...
        for workspace in tabs.prefix(SessionPersistencePolicy.maxWorkspacesPerWindow) {
            ...
            hasher.combine(workspace.telemetry.surfaceListeningPorts.count)
...
extension Notification.Name {
    static let commandPaletteToggleRequested = Notification.Name("kshr.commandPaletteToggleRequested")
    static let commandPaletteRequested = Notification.Name("kshr.commandPaletteRequested")
    static let commandPaletteSwitcherRequested = Notification.Name("kshr.commandPaletteSwitcherRequested")
    static let commandPaletteSubmitRequested = Notification.Name("kshr.commandPaletteSubmitRequested")
    static let commandPaletteDismissRequested = Notification.Name("kshr.commandPaletteDismissRequested")
    static let commandPaletteRenameTabRequested = Notification.Name("kshr.commandPaletteRenameTabRequested")
    static let commandPaletteRenameWorkspaceRequested = Notification.Name("kshr.commandPaletteRenameWorkspaceRequested")
    static let commandPaletteEditWorkspaceDescriptionRequested = Notification.Name("kshr.commandPaletteEditWorkspaceDescriptionRequested")
    static let commandPaletteMoveSelection = Notification.Name("kshr.commandPaletteMoveSelection")
    static let commandPaletteRenameInputInteractionRequested = Notification.Name("kshr.commandPaletteRenameInputInteractionRequested")
    static let commandPaletteRenameInputDeleteBackwardRequested = Notification.Name("kshr.commandPaletteRenameInputDeleteBackwardRequested")
    static let feedbackComposerRequested = Notification.Name("kshr.feedbackComposerRequested")
    ...
    static let terminalPortalVisibilityDidChange = Notification.Name("kshr.terminalPortalVisibilityDidChange")
    static let browserPortalRegistryDidChange = Notification.Name("kshr.browserPortalRegistryDidChange")
}

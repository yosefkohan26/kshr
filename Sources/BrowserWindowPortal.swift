...
private var kshrWindowBrowserPortalKey: UInt8 = 0
private var kshrWindowBrowserPortalCloseObserverKey: UInt8 = 0
private var kshrBrowserSearchOverlayPanelIdAssociationKey: UInt8 = 0
private var kshrBrowserPortalNeedsRenderingStateReattachKey: UInt8 = 0
...
private extension WKWebView {
    private var browserPortalNeedsRenderingStateReattach: Bool {
        get {
            (objc_getAssociatedObject(self, &kshrBrowserPortalNeedsRenderingStateReattachKey) as? NSNumber)?
                .boolValue ?? false
        }
        set {
            objc_setAssociatedObject(
                self,
                &kshrBrowserPortalNeedsRenderingStateReattachKey,
...
final class WindowBrowserSlotView: NSView {
    ...
    func setSearchOverlay(_ configuration: BrowserPortalSearchOverlayConfiguration?) {
        guard let configuration else {
            ...
            if let overlay = searchOverlayHostingView {
                objc_setAssociatedObject(
                    overlay,
                    &kshrBrowserSearchOverlayPanelIdAssociationKey,
                    nil,
                    .OBJC_ASSOCIATION_RETAIN_NONATOMIC
                )
            }
    ...
    func searchOverlayPanelId(for responder: NSResponder) -> UUID? {
        ...
        let panelId = objc_getAssociatedObject(overlay, &kshrBrowserSearchOverlayPanelIdAssociationKey) as? UUID
...
final class WindowBrowserPortal: NSObject {
    ...
    private func synchronizeAllEntriesFromExternalGeometryChange() {
        ...
        // Only sync visible entries during geometry changes — hidden entries
        // will be synced when made visible via bind(). This avoids O(total entries)
        // main-thread work during divider drags and window resizes.
        synchronizeAllWebViews(excluding: nil, source: "externalGeometry", visibleOnly: true)
    ...
    func bind(webView: WKWebView, to anchorView: NSView, visibleInUI: Bool, zPriority: Int = 0) {
        ...
        let shouldPreserveExternalFullscreenHost =
            webView.kshrIsManagedByExternalFullscreenWindow(relativeTo: window)
    ...
    func synchronizeWebViewForAnchor(_ anchorView: NSView) {
        ...
        // Each panel's HostContainerView fires its own geometry callback, so
        // secondary web views will sync themselves when their own anchor changes.
        // No deferred full-sync needed here — that was O(total portal entries)
        // main-thread work on every divider drag frame, causing resize stutter.
    }
...
#endif
            self.synchronizeAllWebViews(excluding: nil, source: "deferredTick", visibleOnly: true)
        }
    }
    ...
    private func synchronizeAllWebViews(
        ...
        for webViewId in webViewIds {
            ...
            if visibleOnly, let entry = entriesByWebViewId[webViewId] {
                // Skip hidden entries — they'll be synced when made visible via bind().
                guard entry.visibleInUI,
                      let containerView = entry.containerView,
                      !containerView.isHidden else { continue }
            }
...
enum BrowserWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &kshrWindowBrowserPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &kshrWindowBrowserPortalCloseObserverKey,
            observer,
            .OBJC_ASSOCIATION_RETAIN_NONATOMIC
        )
    }
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        if let observer = objc_getAssociatedObject(window, &kshrWindowBrowserPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &kshrWindowBrowserPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &kshrWindowBrowserPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow) -> WindowBrowserPortal {
        if let existing = objc_getAssociatedObject(window, &kshrWindowBrowserPortalKey) as? WindowBrowserPortal {
        ...
        objc_setAssociatedObject(window, &kshrWindowBrowserPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
...
// MARK: - LRU WebView Eviction for Hidden Browser Panels

/// Manages LRU-based eviction of WKWebViews for browser panels that have been
/// hidden (e.g., workspace switches) for longer than a grace period.
/// This reduces memory usage by detaching and nilling out WKWebViews for
/// off-screen panels while preserving their URL/state for seamless restoration.
@MainActor
final class BrowserWebViewEvictionManager {
    static let shared = BrowserWebViewEvictionManager()

    /// Grace period before a hidden panel's webview is evicted (seconds).
    private let evictionGracePeriod: TimeInterval = 45

    /// How often the eviction sweep runs (seconds).
    private let sweepInterval: TimeInterval = 15

    /// Tracks when each panel was last hidden.
    private var hiddenTimestamps: [UUID: Date] = [:]

    /// Weak references to registered panels.
    private var registeredPanels: [UUID: Weak<BrowserPanel>] = [:]

    private var sweepTimer: DispatchSourceTimer?

    private init() {}

    /// Register a panel for eviction tracking.
    func register(_ panel: BrowserPanel) {
        registeredPanels[panel.id] = Weak(panel)
        startSweepTimerIfNeeded()
    }

    /// Unregister a panel (e.g., on close).
    func unregister(_ panel: BrowserPanel) {
        registeredPanels.removeValue(forKey: panel.id)
        hiddenTimestamps.removeValue(forKey: panel.id)
        if registeredPanels.isEmpty {
            stopSweepTimer()
        }
    }

    /// Mark a panel as hidden (starts the eviction countdown).
    func noteHidden(_ panel: BrowserPanel) {
        guard panel.webView != nil, !panel.isEvicted else { return }
        hiddenTimestamps[panel.id] = Date()
        startSweepTimerIfNeeded()
    }

    /// Mark a panel as visible (cancels the eviction countdown and restores if needed).
    func noteVisible(_ panel: BrowserPanel) {
        hiddenTimestamps.removeValue(forKey: panel.id)
        if panel.isEvicted {
            panel.ensureWebView()
        }
    ...
    private func sweep() {
        ...
        registeredPanels = registeredPanels.filter { $0.value.value != nil }
        if registeredPanels.isEmpty {
            stopSweepTimer()
            return
        }

        for (panelId, hiddenAt) in hiddenTimestamps {
            guard now.timeIntervalSince(hiddenAt) >= evictionGracePeriod else { continue }
            guard let panel = registeredPanels[panelId]?.value else {
                hiddenTimestamps.removeValue(forKey: panelId)
                continue
            }
            // Only evict if the panel still has a webview and is truly not visible.
            guard panel.webView != nil, !panel.isEvicted else {
                hiddenTimestamps.removeValue(forKey: panelId)
                continue
            }
...
#if DEBUG
            dlog(
                "browser.eviction.sweep panel=\(panelId.uuidString.prefix(5)) " +
                "hiddenFor=\(String(format: "%.1f", now.timeIntervalSince(hiddenAt)))s"
            )
#endif
        }

        if hiddenTimestamps.isEmpty {
            stopSweepTimer()
        }

#if DEBUG
        if evictedCount > 0 {
            dlog("browser.eviction.sweep.done evicted=\(evictedCount) remaining=\(hiddenTimestamps.count)")
        }
#endif
    }
}

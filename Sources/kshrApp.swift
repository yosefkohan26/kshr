...
enum UITestLaunchManifest {
    static let argumentName = "-kshrUITestLaunchManifest"
...
struct kshrApp: App {
    ...
    @StateObject private var kshrConfigStore = KshrConfigStore()
    ...
    init() {
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.kshrOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged kshr DEV; start with ./scripts/reload.sh --tag <name> (or set KSHR_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(updateViewModel: appDelegate.updateViewModel, windowId: primaryWindowId)
                ...
                .onAppear {
#if DEBUG
                    if ProcessInfo.processInfo.environment["KSHR_UI_TEST_MODE"] == "1" {
                        UpdateLogStore.shared.append("ui test: kshrApp onAppear")
                    }
#endif
                    ...
                    applyAppearance()
                    if ProcessInfo.processInfo.environment["KSHR_UI_TEST_SHOW_SETTINGS"] == "1" {
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openKshrSettingsFile", defaultValue: "Open settings.json")) {
                    openKshrSettingsFileInEditor()
                }
            ...
            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About kshr")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitKshr", defaultValue: "Quit kshr"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow ?? NSApp.mainWindow,
           kshrWindowShouldOwnCloseShortcut(window) {
...
private let kshrAuxiliaryWindowIdentifiers: Set<String> = [
    "kshr.settings",
    "kshr.about",
    "kshr.licenses",
    "kshr.browser-popup",
    "kshr.settingsAboutTitlebarDebug",
    "kshr.debugWindowControls",
    "kshr.browserImportHintDebug",
    "kshr.sidebarDebug",
    "kshr.menubarDebug",
    "kshr.backgroundDebug",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func kshrWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return kshrAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class SettingsAboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: SettingsAboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("kshr.debug.titlebar.\(kind.rawValue)")
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("kshr.settingsAboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, kshr uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("kshr.licenses")
        window.center()
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.settings")
        window.center()
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("kshr.settings.navigate")
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private let githubURL = URL(string: "https://github.com/yosefkohan26/kshr")
    private let docsURL = URL(string: "https://kshr.com/docs")

    private var version: String? { Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String }
    private var build: String? { Bundle.main.infoDictionary?["CFBundleVersion"] as? String }
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["KSHRCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["KSHR_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "kshr"))
                        .bold()
                        .font(.title)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/yosefkohan26/kshr/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.splitButtonLayoutDebug")
        window.center()
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("kshr.backgroundDebug")
        window.center()
    ...
    private func updateWindowGlassTint() {
        let window: NSWindow? = {
            if let key = NSApp.keyWindow,
               ...
               raw == "kshr.main" || raw.hasPrefix("kshr.main.") {
                return key
            }
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.kshrterm.appIconDidChange")
...
enum WelcomeSettings {
    static let shownKey = "kshrWelcomeShown"
}
...
struct SettingsView: View {
    ...
    private let shortcutChordsDocsURL = URL(string: "https://kshr.com/docs/keyboard-shortcuts#shortcut-chords")!
    ...
    @AppStorage("kshrPortBase") private var kshrPortBase = 9100
    @AppStorage("kshrPortRange") private var kshrPortRange = 10
    ...
    @AppStorage(BrowserLinkOpenSettings.openTerminalLinksInKshrBrowserKey) private var openTerminalLinksInKshrBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInKshrBrowser
    @AppStorage(BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowserKey)
    private var interceptTerminalOpenCommandInKshrBrowser = BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInKshrBrowserValue()
    ...
    @AppStorage(BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowserKey)
    private var openSidebarPullRequestLinksInKshrBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInKshrBrowser
    @AppStorage(BrowserLinkOpenSettings.openSidebarPortLinksInKshrBrowserKey)
    private var openSidebarPortLinksInKshrBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInKshrBrowser
    ...
    @State private var trustedDirectoriesDraft: String = KshrDirectoryTrust.shared.allTrustedPaths.joined(separator: "\n")
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When kshr is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When kshr is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
    ...
    private func saveTrustedDirectories() {
        let paths = trustedDirectoriesDraft
            ...
            .filter { !$0.isEmpty }
        KshrDirectoryTrust.shared.replaceAll(with: paths)
    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart kshr to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep kshr in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when kshr highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $KSHR_NOTIFICATION_TITLE, $KSHR_NOTIFICATION_SUBTITLE, $KSHR_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve kshr.")
                        ) {
                        ...
                        SettingsCardRow(
                            String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in kshr Browser"),
                            subtitle: openSidebarPullRequestLinksInKshrBrowser
                                ? String(localized: "settings.app.openSidebarPRLinks.subtitleOn", defaultValue: "Clicks open inside kshr browser.")
                                : String(localized: "settings.app.openSidebarPRLinks.subtitleOff", defaultValue: "Clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPullRequestLinksInKshrBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                        ...
                        SettingsCardRow(
                            String(localized: "settings.app.openSidebarPortLinks", defaultValue: "Open Sidebar Port Links in kshr Browser"),
                            subtitle: openSidebarPortLinksInKshrBrowser
                                ? String(localized: "settings.app.openSidebarPortLinks.subtitleOn", defaultValue: "Port clicks open inside kshr browser.")
                                : String(localized: "settings.app.openSidebarPortLinks.subtitleOff", defaultValue: "Port clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPortLinksInKshrBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: KSHR_SOCKET_ENABLE, KSHR_SOCKET_MODE, and KSHR_SOCKET_PATH (set KSHR_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without kshr integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, kshr wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for KSHR_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $kshrPortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $kshrPortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets KSHR_PORT and KSHR_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        VStack(alignment: .leading, spacing: 6) {
                            SettingsCardRow(
                                ...
                                subtitle: String(localized: "settings.customCommands.trustedDirectories.subtitle", defaultValue: "Commands from kshr.json in these directories run without confirmation. One path per line.")
                            ) {
                                EmptyView()
                            }
                        ...
                        SettingsCardDivider()
                        SettingsCardNote(String(localized: "settings.customCommands.trustedDirectories.note", defaultValue: "Place a kshr.json in your project root to define custom commands. Trust a directory from the confirmation dialog, or add paths here. For git repos, trusting the root covers all subdirectories."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in kshr Browser"),
                            ...
                            Toggle("", isOn: $openTerminalLinksInKshrBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        ...
                        SettingsCardRow(
                            ...
                            Toggle("", isOn: $interceptTerminalOpenCommandInKshrBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }

                        if openTerminalLinksInKshrBrowser || interceptTerminalOpenCommandInKshrBrowser {
                            SettingsCardDivider()

                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    String(localized: "settings.browser.hostWhitelist", defaultValue: "Hosts to Open in Embedded Browser"),
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in kshr. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in kshr.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in kshr without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        ...
                        SettingsCardRow(
                            ...
                            HStack(spacing: 8) {
                                ...
                                Button(String(localized: "settings.app.settingsFile.openButton", defaultValue: "Open settings.json")) {
                                    openKshrSettingsFileInTextEdit()
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.small)
                ...
                HStack {
                    ...
                    HStack(spacing: 6) {
                        SettingsHeaderActionButton(
                            ...
                            action: openKshrSettingsFileInTextEdit
                        )
                    }
                }
    ...
    private func resetAllSettings() {
        ...
        openTerminalLinksInKshrBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInKshrBrowser
        interceptTerminalOpenCommandInKshrBrowser = BrowserLinkOpenSettings.defaultInterceptTerminalOpenCommandInKshrBrowser
        ...
        openSidebarPullRequestLinksInKshrBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInKshrBrowser
        openSidebarPortLinksInKshrBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInKshrBrowser
...
private struct SettingsRootView: View {
    ...
    private func configureSettingsWindow(_ window: NSWindow) {
        ...
        for index in accessories.indices.reversed() {
            ...
            guard identifier.hasPrefix("kshr.") else { continue }

...
struct NotificationsPage: View {
    ...
    /// Pre-computed tab-title map: built once per render instead of O(n) per row.
    private var tabTitleMap: [UUID: String] {
        var map: [UUID: String] = [:]
        for tab in tabManager.tabs {
            map[tab.id] = tab.title
        }
        return map
    }

    var body: some View {
        let titleMap = tabTitleMap
        VStack(spacing: 0) {
            header
            Divider()
                    ...
                    LazyVStack(spacing: 8) {
                        ForEach(notificationStore.notifications) { notification in
                            NotificationRow(
                                ...
                                tabTitle: AppDelegate.shared?.tabTitle(for: notification.tabId) ?? titleMap[notification.tabId],
                                onOpen: {

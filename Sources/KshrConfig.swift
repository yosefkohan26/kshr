...
struct KshrConfigFile: Codable, Sendable {
    var commands: [KshrCommandDefinition]
}

struct KshrCommandDefinition: Codable, Sendable, Identifiable {
    var name: String
    var description: String?
    var keywords: [String]?
    var restart: KshrRestartBehavior?
    var workspace: KshrWorkspaceDefinition?
    var command: String?
    var confirm: Bool?

    var id: String {
        "kshr.config.command." + (name.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? name)
    }
    ...
    init(
        ...
        keywords: [String]? = nil,
        restart: KshrRestartBehavior? = nil,
        workspace: KshrWorkspaceDefinition? = nil,
        command: String? = nil,
        confirm: Bool? = nil
    ) {
        self.name = name
    ...
    init(from decoder: Decoder) throws {
        ...
        restart = try container.decodeIfPresent(KshrRestartBehavior.self, forKey: .restart)
        workspace = try container.decodeIfPresent(KshrWorkspaceDefinition.self, forKey: .workspace)
...
enum KshrRestartBehavior: String, Codable, Sendable {
    case recreate
    case ignore
    case confirm
}

struct KshrWorkspaceDefinition: Codable, Sendable {
    var name: String?
    var cwd: String?
    var color: String?
    var layout: KshrLayoutNode?

    init(name: String? = nil, cwd: String? = nil, color: String? = nil, layout: KshrLayoutNode? = nil) {
        self.name = name
        self.cwd = cwd
        self.color = color
        self.layout = layout
    ...
    init(from decoder: Decoder) throws {
        ...
        layout = try container.decodeIfPresent(KshrLayoutNode.self, forKey: .layout)
...
indirect enum KshrLayoutNode: Codable, Sendable {
    case pane(KshrPaneDefinition)
    case split(KshrSplitDefinition)
    ...
    init(from decoder: Decoder) throws {
        ...
        if hasPane && hasDirection {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "KshrLayoutNode must not contain both 'pane' and 'direction' keys"
                )
            )
        }

        if hasPane {
            let pane = try container.decode(KshrPaneDefinition.self, forKey: .pane)
            self = .pane(pane)
        } else if hasDirection {
            let splitDef = try KshrSplitDefinition(from: decoder)
            self = .split(splitDef)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "KshrLayoutNode must contain either a 'pane' key or a 'direction' key"
                )
            )
        }
    }
...
struct KshrSplitDefinition: Codable, Sendable {
    var direction: KshrSplitDirection
    var split: Double?
    var children: [KshrLayoutNode]

    init(direction: KshrSplitDirection, split: Double? = nil, children: [KshrLayoutNode]) {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        direction = try container.decode(KshrSplitDirection.self, forKey: .direction)
        split = try container.decodeIfPresent(Double.self, forKey: .split)
        children = try container.decode([KshrLayoutNode].self, forKey: .children)
        if children.count != 2 {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
enum KshrSplitDirection: String, Codable, Sendable {
    case horizontal
    case vertical
}

struct KshrPaneDefinition: Codable, Sendable {
    var surfaces: [KshrSurfaceDefinition]

    init(surfaces: [KshrSurfaceDefinition]) {
        self.surfaces = surfaces
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        surfaces = try container.decode([KshrSurfaceDefinition].self, forKey: .surfaces)
        if surfaces.isEmpty {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
struct KshrSurfaceDefinition: Codable, Sendable {
    var type: KshrSurfaceType
    var name: String?
    var command: String?
    var cwd: String?
...
enum KshrSurfaceType: String, Codable, Sendable {
    case terminal
    case browser
}

@MainActor
final class KshrConfigStore: ObservableObject {
    @Published private(set) var loadedCommands: [KshrCommandDefinition] = []
    ...
    let globalConfigPath: String = {
        ...
        return (home as NSString).appendingPathComponent(".config/kshr/kshr.json")
    }()
    ...
    private let watchQueue = DispatchQueue(label: "com.kshr.config-file-watch")
    ...
    func wireDirectoryTracking(tabManager: TabManager) {
        ...
        tabManager.$selectedTabId
            .compactMap { [weak tabManager] tabId -> Workspace? in
                ...
                return tabManager.tabsById[tabId]
            }
    ...
    private func updateLocalConfigPath(_ directory: String?) {
        ...
        if let directory, !directory.isEmpty {
            newPath = findKshrConfig(startingFrom: directory)
                ?? (directory as NSString).appendingPathComponent("kshr.json")
        } else {
            newPath = nil
        }
    ...
    private func findKshrConfig(startingFrom directory: String) -> String? {
        var current = directory
        let fs = FileManager.default
        while true {
            let candidate = (current as NSString).appendingPathComponent("kshr.json")
    ...
    private func parseConfig(at path: String) -> KshrConfigFile? {
        ...
        do {
            return try JSONDecoder().decode(KshrConfigFile.self, from: data)
        } catch {
            NSLog("[KshrConfig] parse error at %@: %@", path, String(describing: error))
            return nil
        }
    }

...
extension Notification.Name {
    static let kshrNotificationsPopoverVisibilityDidChange = Notification.Name("kshr.notificationsPopoverVisibilityDidChange")
}
...
private func postNotificationsPopoverVisibilityDidChange(isShown: Bool) {
    NotificationCenter.default.post(
        name: .kshrNotificationsPopoverVisibilityDidChange,
        object: nil,
...
struct TitlebarControlsView: View {
    ...
    @State private var showSSHPresetsPopover = false
    ...
    var body: some View {
        ...
        controlsGroup(config: config)
            ...
            .onReceive(NotificationCenter.default.publisher(for: .kshrNotificationsPopoverVisibilityDidChange)) { notification in
    ...
    private func controlsGroup(config: TitlebarControlsStyleConfig) -> some View {
        ...
        let content = HStack(spacing: config.spacing) {
            ...
            TitlebarControlButton(config: config, action: {
                #if DEBUG
                dlog("titlebar.sshPresets")
                #endif
                showSSHPresetsPopover.toggle()
            }) {
                iconLabel(systemName: "server.rack", config: config)
            }
            .accessibilityIdentifier("titlebarControl.sshPresets")
            .accessibilityLabel(String(localized: "titlebar.sshPresets.accessibilityLabel", defaultValue: "SSH Presets"))
            .safeHelp(String(localized: "titlebar.sshPresets.tooltip", defaultValue: "SSH presets"))
            .popover(isPresented: $showSSHPresetsPopover, arrowEdge: .bottom) {
                SSHPresetsPopoverView(
                    store: SSHPresetStore.shared,
                    dismiss: { showSSHPresetsPopover = false }
                )
            }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            Button(action: onOpen) {
                HStack(alignment: .top, spacing: 10) {
                    Circle()
                        .fill(notification.isRead ? Color.clear : kshrAccentColor())
                        .frame(width: 8, height: 8)
                        .overlay(
                            Circle()
                                .stroke(kshrAccentColor().opacity(notification.isRead ? 0.2 : 1), lineWidth: 1)
                        )
                        .padding(.top, 6)
...
final class UpdateTitlebarAccessoryController {
    ...
    private let controlsIdentifier = NSUserInterfaceItemIdentifier("kshr.titlebarControls")
    ...
    private func isSettingsWindow(_ window: NSWindow) -> Bool {
        if window.identifier?.rawValue == "kshr.settings" {
            return true
        }

...
enum UpdateSettings {
    ...
    static let migrationKey = "kshr.sparkle.automaticChecksMigration.v2"
...
/// Controller for managing Sparkle updates in kshr.
class UpdateController {
    ...
    func startUpdaterIfNeeded() {
        // Auto-update disabled for KSHR fork
        UpdateLogStore.shared.append("updater disabled (KSHR fork)")
        return
        ...
        if ProcessInfo.processInfo.environment["KSHR_UI_TEST_RESET_SPARKLE_PERMISSION"] == "1" {
            let defaults = UserDefaults.standard
    ...
    private func recordUITestTimestamp(key: String) {
        ...
        guard let path = env["KSHR_UI_TEST_TIMING_PATH"] else { return }

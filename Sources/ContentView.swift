...
func kshrAccentNSColor(for colorScheme: ColorScheme) -> NSColor {
    switch colorScheme {
    case .dark:
        return NSColor(
            srgbRed: 0,
...
func kshrAccentNSColor(for appAppearance: NSAppearance?) -> NSColor {
    let bestMatch = appAppearance?.bestMatch(from: [.darkAqua, .aqua])
    let scheme: ColorScheme = (bestMatch == .darkAqua) ? .dark : .light
    return kshrAccentNSColor(for: scheme)
}

func kshrAccentNSColor() -> NSColor {
    NSColor(name: nil) { appearance in
        kshrAccentNSColor(for: appearance)
    }
}

func kshrAccentColor() -> Color {
    Color(nsColor: kshrAccentNSColor())
}
...
func sidebarSelectedWorkspaceBackgroundNSColor(for colorScheme: ColorScheme) -> NSColor {
    ...
    return kshrAccentNSColor(for: colorScheme)
}
...
enum InternalTabDragConfigurationProvider {
    // These drags only make sense inside kshr. Outside the app, Finder should
...
let commandPaletteOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("kshr.commandPalette.overlay.container")
let tmuxWorkspacePaneOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("kshr.tmuxWorkspacePane.overlay.container")
...
enum WorkspaceMountPolicy {
    // Keep recent workspaces mounted so switching back is instant (no remount).
    static let maxMountedWorkspaces = 3
    // During workspace cycling, keep the handoff set plus one warm neighbor.
    static let maxMountedWorkspacesDuringCycle = 4
...
struct ContentView: View {
    ...
    @EnvironmentObject var kshrConfigStore: KshrConfigStore
    ...
    @State private var retiringWorkspaceIds: Set<UUID> = []
    ...
    private var terminalContent: some View {
        ...
        let retiringWorkspaceIds = self.retiringWorkspaceIds

        return ZStack {
            ZStack {
                ForEach(mountedWorkspaces) { tab in
                    let isSelectedWorkspace = selectedWorkspaceId == tab.id
                    let isRetiringWorkspace = retiringWorkspaceIds.contains(tab.id)
    ...
    private var windowIdentifier: String { "kshr.main.\(windowId.uuidString)" }
    ...
    // Split into helper to avoid "expression too complex" with long modifier chains.
    @ViewBuilder
    private var bodyCore: some View {
        contentAndSidebarLayout
            ...
            .onChange(of: retiringWorkspaceIds) { _ in
                reconcileMountedWorkspaceIds()
            }
    ...
    @ViewBuilder
    private var bodyCoreB: some View {
        bodyCore
            ...
            .onReceive(tabManager.$tabs) { tabs in
                ...
                let staleRetirees = retiringWorkspaceIds.subtracting(existingIds)
                if !staleRetirees.isEmpty {
                    retiringWorkspaceIds.subtract(staleRetirees)
                    if retiringWorkspaceIds.isEmpty {
    ...
    @ViewBuilder
    private var bodyCoreC: some View {
        bodyCoreB
    ...
    var body: some View {
        bodyCoreC
            ...
            let shouldApplyWindowGlass = kshrShouldApplyWindowGlass(
    ...
    private func reconcileMountedWorkspaceIds(tabs: [Workspace]? = nil, selectedId: UUID? = nil) {
        ...
        let handoffPinnedIds = retiringWorkspaceIds
    ...
    private func setTitlebarControlsHidden(_ hidden: Bool, in window: NSWindow) {
        let controlsId = NSUserInterfaceItemIdentifier("kshr.titlebarControls")
    ...
    private func startWorkspaceHandoffIfNeeded(newSelectedId: UUID?) {
        ...
        guard let oldSelectedId, let newSelectedId, oldSelectedId != newSelectedId else {
            ...
            // Hide portals for any accumulated retirees before clearing
            for retireeId in retiringWorkspaceIds {
                if let workspace = tabManager.tabs.first(where: { $0.id == retireeId }) {
                    workspace.hideAllTerminalPortalViews()
                    workspace.hideAllBrowserPortalViews()
                }
            }
            retiringWorkspaceIds.removeAll()
            workspaceHandoffFallbackTask?.cancel()
            workspaceHandoffFallbackTask = nil
            return
        }
        ...
        retiringWorkspaceIds.insert(oldSelectedId)
        workspaceHandoffFallbackTask?.cancel()
    ...
    private func completeWorkspaceHandoff(reason: String) {
        workspaceHandoffFallbackTask?.cancel()
        workspaceHandoffFallbackTask = nil
        let retirees = retiringWorkspaceIds

        // Hide portal-hosted views for ALL retiring workspaces BEFORE clearing
        // retiringWorkspaceIds. Once cleared, reconcileMountedWorkspaceIds unmounts
        // the workspaces — but dismantleNSView intentionally doesn't hide portal views
        // during transient rebuilds. Hiding here prevents stale terminal/browser
        // portals from covering the newly selected workspace.
        for retireeId in retirees {
            if let workspace = tabManager.tabs.first(where: { $0.id == retireeId }) {
                workspace.hideAllTerminalPortalViews()
                workspace.hideAllBrowserPortalViews()
            }
        ...
        if let snapshot = tabManager.debugCurrentWorkspaceSwitchSnapshot() {
            let dtMs = (CACurrentMediaTime() - snapshot.startedAt) * 1000
            dlog(
                "ws.handoff.complete id=\(snapshot.id) dt=\(debugMsText(dtMs)) reason=\(reason) retirees=\(retirees.map { debugShortWorkspaceId($0) })"
            )
        } else {
            dlog("ws.handoff.complete id=none reason=\(reason) retirees=\(retirees.map { debugShortWorkspaceId($0) })")
        }
#endif
    }
    ...
    private func commandPaletteCommandsFingerprint(commandsContext: CommandPaletteCommandsContext) -> Int {
        ...
        hasher.combine(kshrConfigStore.configRevision)
    ...
    private func commandPaletteSurfaceSearchMetadata(
        ...
        let ports = workspace.telemetry.surfaceListeningPorts[panelId] ?? []
        return CommandPaletteSwitcherSearchMetadata(
            directories: directories,
            branches: branches,
            ports: ports
    ...
    private func commandPaletteCommandsContext(
        ...
        let cliInstalledInPATH = AppDelegate.shared?.isKshrCLIInstalledInPATH() ?? false
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.installCLI.title", defaultValue: "Shell Command: Install 'kshr' in PATH")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.uninstallCLI.title", defaultValue: "Shell Command: Uninstall 'kshr' from PATH")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                keywords: ["restart", "socket", "listener", "cli", "kshr", "control"]
            )
        )
        ...
        let kshrConfigDefaultSubtitle = constant(String(localized: "command.kshrConfig.subtitle", defaultValue: "kshr.json"))
        for command in kshrConfigStore.loadedCommands {
            let commandName = sanitizeKshrConfigPaletteText(command.name)
            let subtitle = command.description
                .map { sanitizeKshrConfigPaletteText($0) }
                .flatMap { $0.isEmpty ? nil : constant($0) }
                ?? kshrConfigDefaultSubtitle
            contributions.append(
                CommandPaletteCommandContribution(
                    commandId: command.id,
                    title: constant(String(localized: "command.kshrConfig.customTitle", defaultValue: "Custom: \(commandName)")),
                    subtitle: subtitle,
    ...
    private func sanitizeKshrConfigPaletteText(_ text: String) -> String {
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        registry.register(commandId: "palette.installCLI") {
            AppDelegate.shared?.installKshrCLIInPath(nil)
        }
        registry.register(commandId: "palette.uninstallCLI") {
            AppDelegate.shared?.uninstallKshrCLIInPath(nil)
        }
        ...
        for command in kshrConfigStore.loadedCommands {
            ...
            let sourcePath = kshrConfigStore.commandSourcePaths[command.id]
            let globalPath = kshrConfigStore.globalConfigPath
    ...
    private func commandPaletteBackdropFocusTarget(for responder: NSResponder) -> CommandPaletteRestoreFocusTarget? {
        if let terminalView = kshrOwningGhosttyView(for: responder),
           let workspaceId = terminalView.tabId,
           let panelId = terminalView.terminalSurface?.id,
           tabManager.tabs.contains(where: { $0.id == workspaceId }) {
            return commandPaletteRestoreFocusTarget(
    ...
    private func applyUITestSidebarSelectionIfNeeded(tabs: [Workspace]) {
        ...
        guard let rawValue = env["KSHR_UI_TEST_SIDEBAR_SELECTED_WORKSPACE_INDICES"]?
    ...
    private func openWorkspacePullRequestsInConfiguredBrowser() -> Bool {
        ...
        var openedCount = 0
        if openSidebarPullRequestLinksInKshrBrowser {
...
private struct SidebarTabItemSettingsSnapshot: Equatable {
    ...
    let showsSSH: Bool
    let openPullRequestLinksInKshrBrowser: Bool
    let openPortLinksInKshrBrowser: Bool
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        openPullRequestLinksInKshrBrowser = BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowser(
            defaults: defaults
        )
        openPortLinksInKshrBrowser = BrowserLinkOpenSettings.openSidebarPortLinksInKshrBrowser(
            defaults: defaults
        )
...
private enum FeedbackComposerSettings {
    ...
    static let endpointEnvironmentKey = "KSHR_FEEDBACK_API_URL"
    static let defaultEndpoint = "https://kshr.com/api/feedback"
    static let foundersEmail = "founders@yosefkohan26.com"
...
private struct FeedbackComposerAppMetadata {
    ...
    static var current: FeedbackComposerAppMetadata {
        ...
        let env = ProcessInfo.processInfo.environment
        let commit = (infoDictionary["KSHRCommit"] as? String).flatMap { value in
...
enum SidebarDragLifecycleNotification {
    static let stateDidChange = Notification.Name("kshr.sidebarDragStateDidChange")
    static let requestClear = Notification.Name("kshr.sidebarDragRequestClear")
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(
                String(
                    ...
                    defaultValue: "You can also reach us at founders@yosefkohan26.com."
                )
            )
            .font(.system(size: 12))
            .foregroundStyle(.secondary)
        ...
        VStack(alignment: .leading, spacing: 14) {
            Text(
                String(
                    ...
                    defaultValue: "A human will read this! You can also reach us at founders@yosefkohan26.com."
                )
            )
            .font(.system(size: 12))
            .foregroundStyle(.secondary)
    ...
    private func userFacingErrorMessage(for error: Error) -> String {
        ...
        case .invalidEndpoint:
            return String(
                ...
                defaultValue: "Feedback is unavailable right now. Email founders@yosefkohan26.com instead."
            )
        case .invalidResponse:
            return String(
                localized: "sidebar.help.feedback.genericError",
...
enum FeedbackComposerBridge {
    ...
    private static func userFacingMessage(for error: Error) -> String {
        ...
        switch submissionError {
        case .invalidEndpoint:
            return "Feedback is unavailable right now. Email founders@yosefkohan26.com instead."
        case .invalidResponse:
            return "Couldn't send feedback. Please try again."
        case .attachmentReadFailed:
...
private struct SidebarHelpMenuButton: View {
    ...
    private let changelogURL = URL(string: "https://kshr.com/docs/changelog")
    private let githubURL = URL(string: "https://github.com/yosefkohan26/kshr")
    private let githubIssuesURL = URL(string: "https://github.com/yosefkohan26/kshr/issues")
    ...
    private var helpPopover: some View {
        VStack(alignment: .leading, spacing: 2) {
            helpOptionButton(
                title: String(localized: "sidebar.help.welcome", defaultValue: "Welcome to kshr!"),
                action: .welcome,
...
private struct TabItemView: View, Equatable {
    ...
    @State private var lastSingleClickTime: CFTimeInterval = 0
    @State private var lastSingleClickTabId: UUID?
    ...
    private var openSidebarPullRequestLinksInKshrBrowser: Bool {
        settings.openPullRequestLinksInKshrBrowser
    }

    private var openSidebarPortLinksInKshrBrowser: Bool {
        settings.openPortLinksInKshrBrowser
    }
    ...
    private var activeUnreadBadgeFillColor: Color {
        ...
        return usesInvertedActiveForeground ? Color.white.opacity(0.25) : kshrAccentColor()
    }
    ...
    private var activeProgressFillColor: Color {
        usesInvertedActiveForeground ? Color.white.opacity(0.8) : kshrAccentColor()
    }
    ...
    private var remoteConnectionDotColor: Color {
        switch tab.remoteConnectionState {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 4) {
                ...
                ZStack {
                    if isEditingTitle {
                        SidebarInlineRenameField(
                            ...
                            font: .systemFont(ofSize: 12.5, weight: titleFontWeight == .semibold ? .semibold : .regular),
                            textColor: NSColor(activePrimaryTextColor),
                            onCommit: {
                                tabManager.setCustomTitle(tabId: tab.id, title: editingTitleText)
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 4) {
                    ...
                    Text(Self.timestampFormatter.string(from: tab.createdAt))
                        .font(.system(size: 9))
                ...
                HStack(spacing: 4) {
                    Text("Modified")
                        .font(.system(size: 9, weight: .medium))
                        .foregroundColor(activeSecondaryColor(0.45))
                    Text(Self.relativeTimestamp(for: tab.lastActivityAt))
                        .font(.system(size: 9))
                        .foregroundColor(activeSecondaryColor(0.6))
                }
            }
            ...
            if detailVisibility.showsLog, let latestLog = tab.logEntries.last {
                HStack(spacing: 4) {
                    ...
                    Text(latestLog.message)
                        ...
                        .truncationMode(.tail)
                    Spacer(minLength: 0)
                    if latestLog.level == .error, latestLog.source == "remote-daemon" || latestLog.source == "remote" {
                        Button {
                        ...
                        .buttonStyle(.plain)
                        .help(String(localized: "sidebar.reconnect", defaultValue: "Reconnect"))
                    }
                    Button {
                    ...
                    .buttonStyle(.plain)
                    .help(String(localized: "sidebar.dismissLog", defaultValue: "Dismiss"))
                }
                .transition(.opacity.combined(with: .move(edge: .top)))
            }
        ...
        .onTapGesture(count: 1) {
            ...
            if now - lastSingleClickTime < NSEvent.doubleClickInterval,
               lastSingleClickTabId == tab.id {
                // Double-click detected — start rename
                lastSingleClickTime = 0
                lastSingleClickTabId = nil
                editingTitleText = tab.customTitle ?? tab.title
                isEditingTitle = true
            } else {
                // Single click — switch immediately, track for potential double
    ...
    private var backgroundColor: Color {
        ...
        case .leftRail:
            ...
            if isMultiSelected { return kshrAccentColor().opacity(0.25) }
            return Color.clear
        case .solidFill:
            if isActive { return Color(nsColor: selectionBackgroundColor) }
    ...
    private func openPortLink(_ port: Int) {
        ...
        updateSelection()
        if openSidebarPortLinksInKshrBrowser {
            if tabManager.openBrowser(
                inWorkspace: tab.id,
                url: url,
                preferSplitRight: true,
...
/// An NSViewRepresentable that wraps NSTextField for inline workspace renaming.
/// Uses a custom NSTextField subclass that refuses to resign first responder
/// while editing, preventing the terminal surface from stealing focus on cursor blink.
private struct SidebarInlineRenameField: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> SidebarRenameTextField {
        let field = SidebarRenameTextField()
        ...
        // Delay focus so SwiftUI settles first
    ...
    func updateNSView(_ nsView: SidebarRenameTextField, context: Context) {
        // Re-grab focus if the terminal surface stole it
        if context.coordinator.isReady,
           !context.coordinator.didCommitOrCancel,
    ...
    final class Coordinator: NSObject, NSTextFieldDelegate {
        let parent: SidebarInlineRenameField
        var didCommitOrCancel = false
        var isReady = false
        ...
        func controlTextDidEndEditing(_ obj: Notification) {
            ...
            guard !didCommitOrCancel else { return }
...
/// Custom NSTextField that refuses to resign first responder while rename is active.
/// This prevents the terminal surface (GhosttySurfaceScrollView) from stealing focus
/// on cursor blink redraws.
private final class SidebarRenameTextField: NSTextField {
    weak var coordinator: SidebarInlineRenameField.Coordinator?

    override var acceptsFirstResponder: Bool { true }

    override func resignFirstResponder() -> Bool {
        // If we're actively editing and haven't committed/cancelled, refuse to resign
        if let coord = coordinator, coord.isReady, !coord.didCommitOrCancel {
            return false
        }
...
private enum SidebarTabDragPayload {
    static let typeIdentifier = "com.kshr.sidebar-tab-reorder"
    static let dropContentType = UTType(exportedAs: typeIdentifier)
    static let dropContentTypes: [UTType] = [dropContentType]
    private static let prefix = "kshr.sidebar-tab."

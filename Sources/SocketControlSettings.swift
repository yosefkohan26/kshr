...
enum SocketControlMode: String, CaseIterable, Identifiable {
    ...
    static var uiCases: [SocketControlMode] { [.off, .kshrOnly, .automation, .password, .allowAll] }
    ...
    var displayName: String {
        ...
        case .kshrOnly:
            return String(localized: "socketControl.kshrOnly.name", defaultValue: "kshr processes only")
        case .automation:
    ...
    var description: String {
        ...
        case .kshrOnly:
            return String(localized: "socketControl.kshrOnly.description", defaultValue: "Only processes started inside kshr terminals can send commands.")
        case .automation:
    ...
    var socketFilePermissions: UInt16 {
        ...
        case .allowAll:
            return 0o666
        case .off, .kshrOnly, .automation, .password:
            return 0o600
        }
    }
...
enum SocketControlPasswordStore {
    static let directoryName = "kshr"
    static let fileName = "socket-control-password"
    static let didChangeNotification = Notification.Name("kshr.socketControlPasswordDidChange")
    ...
    private static let legacyKeychainService = "com.kshrterm.app.socket-control"
...
struct SocketControlSettings {
    ...
    static let allowSocketPathOverrideKey = "KSHR_ALLOW_SOCKET_OVERRIDE"
    static let socketPasswordEnvKey = "KSHR_SOCKET_PASSWORD"
    static let launchTagEnvKey = "KSHR_TAG"
    static let baseDebugBundleIdentifier = "com.kshrterm.app.debug"
    private static let socketDirectoryName = "kshr"
    private static let stableSocketFileName = "kshr.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyStableDefaultSocketPath = "/tmp/kshr.sock"
    static let legacyLastSocketPathFile = "/tmp/kshr-last-socket-path"
    ...
    static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any KSHR_UI_TEST_ env var.
        if environment.keys.contains(where: { $0.hasPrefix("KSHR_UI_TEST_") }) {
            return false
        }
    ...
    static func socketPath(
        ...
        if let taggedDebugPath = taggedDebugSocketPath(
            ...
            if isTruthy(environment[allowSocketPathOverrideKey]),
               let override = environment["KSHR_SOCKET_PATH"],
               !override.isEmpty {
                return override
            }
        ...
        guard let override = environment["KSHR_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    static func defaultSocketPath(
        ...
        if bundleIdentifier == "com.kshrterm.app.nightly" {
            return "/tmp/kshr-nightly.sock"
        }
    ...
    static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            .appendingPathComponent("kshr-\(currentUserID).sock", isDirectory: false)
            .path ?? "/tmp/kshr-\(currentUserID).sock"
    }
    ...
    static func isDebugLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.kshrterm.app.debug"
            || bundleIdentifier.hasPrefix("com.kshrterm.app.debug.")
    }

    /// Tagged DEV builds have bundle IDs like `com.kshrterm.app.debug.<tag>`.
    ...
    static func taggedDebugSocketPath(
        ...
        if bundleId.hasPrefix("\(baseDebugBundleIdentifier).") {
            ...
            if !slug.isEmpty {
                return "/tmp/kshr-debug-\(slug).sock"
            }
        }
        ...
        return "/tmp/kshr-debug-\(tag).sock"
    }

    static func isStagingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        guard let bundleIdentifier else { return false }
        return bundleIdentifier == "com.kshrterm.app.staging"
            || bundleIdentifier.hasPrefix("com.kshrterm.app.staging.")
    }
    ...
    static func envOverrideEnabled(
        ...
        guard let raw = environment["KSHR_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    static func envOverrideMode(
        ...
        guard let raw = environment["KSHR_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }
    ...
    static func effectiveMode(
        ...
        if let overrideEnabled = envOverrideEnabled(environment: environment) {
            ...
            return userMode == .off ? .kshrOnly : userMode
        }

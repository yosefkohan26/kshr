...
/// Manages trusted directories for kshr.json command execution.
/// When a directory (or its git repo root) is trusted, `confirm: true` commands
/// from that directory's kshr.json skip the confirmation dialog.
/// Global config (~/.config/kshr/kshr.json) is always trusted.
final class KshrDirectoryTrust {
    static let shared = KshrDirectoryTrust()
    static let didChangeNotification = Notification.Name("kshr.directoryTrustDidChange")
    ...
    private init() {
        ...
        ).first!.appendingPathComponent("kshr")
    ...
    /// Check if a kshr.json path is trusted.
    /// Global config is always trusted. For local configs, check the git repo root
    /// (or the kshr.json parent directory if not in a git repo).
    ...
    /// Trust the directory containing a kshr.json. If the kshr.json is inside a git
    ...
    /// Resolve the trust key for a kshr.json path: git repo root if inside a repo,
    /// otherwise the kshr.json's parent directory.

...
extension WKWebView {
    var kshrIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func kshrIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard kshrIsElementFullscreenActiveOrTransitioning else { return false }
...
/// family before kshr falls back to its own browser find overlay.
final class KshrWebView: WKWebView {
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? KshrWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: KshrWebView) {
    ...
    var contextMenuLinkURLProvider: ((KshrWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.kshr.sidebar-tab-reorder"),
    ]

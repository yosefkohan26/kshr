...
final class BrowserProfileStore: ObservableObject {
    ...
    func historyFileURL(for profileID: UUID) -> URL? {
        ...
        let bundleId = Bundle.main.bundleIdentifier ?? "kshr"
...
enum BrowserLinkOpenSettings {
    static let openTerminalLinksInKshrBrowserKey = "browserOpenTerminalLinksInKshrBrowser"
    static let defaultOpenTerminalLinksInKshrBrowser: Bool = true

    static let openSidebarPullRequestLinksInKshrBrowserKey = "browserOpenSidebarPullRequestLinksInKshrBrowser"
    static let defaultOpenSidebarPullRequestLinksInKshrBrowser: Bool = true

    static let openSidebarPortLinksInKshrBrowserKey = "browserOpenSidebarPortLinksInKshrBrowser"
    static let defaultOpenSidebarPortLinksInKshrBrowser: Bool = true

    static let interceptTerminalOpenCommandInKshrBrowserKey = "browserInterceptTerminalOpenCommandInKshrBrowser"
    static let defaultInterceptTerminalOpenCommandInKshrBrowser: Bool = true
    ...
    static func openTerminalLinksInKshrBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: openTerminalLinksInKshrBrowserKey) == nil {
            return defaultOpenTerminalLinksInKshrBrowser
        }
        return defaults.bool(forKey: openTerminalLinksInKshrBrowserKey)
    }

    static func openSidebarPullRequestLinksInKshrBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: openSidebarPullRequestLinksInKshrBrowserKey) == nil {
            return defaultOpenSidebarPullRequestLinksInKshrBrowser
        }
        return defaults.bool(forKey: openSidebarPullRequestLinksInKshrBrowserKey)
    }

    static func openSidebarPortLinksInKshrBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: openSidebarPortLinksInKshrBrowserKey) == nil {
            return defaultOpenSidebarPortLinksInKshrBrowser
        }
        return defaults.bool(forKey: openSidebarPortLinksInKshrBrowserKey)
    }

    static func interceptTerminalOpenCommandInKshrBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: interceptTerminalOpenCommandInKshrBrowserKey) != nil {
            return defaults.bool(forKey: interceptTerminalOpenCommandInKshrBrowserKey)
        }
        ...
        return defaultInterceptTerminalOpenCommandInKshrBrowser
    }

    static func initialInterceptTerminalOpenCommandInKshrBrowserValue(defaults: UserDefaults = .standard) -> Bool {
        interceptTerminalOpenCommandInKshrBrowser(defaults: defaults)
    }
...
func normalizedBrowserHistoryNamespace(bundleIdentifier: String) -> String {
    if bundleIdentifier.hasPrefix("com.kshrterm.app.debug.") {
        return "com.kshrterm.app.debug"
    }
    if bundleIdentifier.hasPrefix("com.kshrterm.app.staging.") {
        return "com.kshrterm.app.staging"
    }
...
actor BrowserSearchSuggestionService {
    ...
    func suggestions(engine: BrowserSearchEngine, query: String) async -> [String] {
        ...
        let forced = ProcessInfo.processInfo.environment["KSHR_UI_TEST_REMOTE_SUGGESTIONS_JSON"]
            ?? UserDefaults.standard.string(forKey: "KSHR_UI_TEST_REMOTE_SUGGESTIONS_JSON")
        if let forced,
...
final class BrowserPanel: Panel, ObservableObject {
    ...
    (() => {
      if (window.__kshrHooksInstalled) return true;
      window.__kshrHooksInstalled = true;

      window.__kshrConsoleLog = window.__kshrConsoleLog || [];
      const __pushConsole = (level, args) => {
        try {
          ...
          window.__kshrConsoleLog.push({ level, text, timestamp_ms: Date.now() });
          if (window.__kshrConsoleLog.length > 512) {
            window.__kshrConsoleLog.splice(0, window.__kshrConsoleLog.length - 512);
          }
        } catch (_) {}
      };
      ...
      window.__kshrErrorLog = window.__kshrErrorLog || [];
      window.addEventListener('error', (ev) => {
        try {
          ...
          window.__kshrErrorLog.push({ message, source, line, column: col, timestamp_ms: Date.now() });
          if (window.__kshrErrorLog.length > 512) {
            window.__kshrErrorLog.splice(0, window.__kshrErrorLog.length - 512);
          }
        } catch (_) {}
      });
      window.addEventListener('unhandledrejection', (ev) => {
        try {
          ...
          window.__kshrErrorLog.push({ message, source: 'unhandledrejection', line: 0, column: 0, timestamp_ms: Date.now() });
          if (window.__kshrErrorLog.length > 512) {
            window.__kshrErrorLog.splice(0, window.__kshrErrorLog.length - 512);
          }
        } catch (_) {}
    ...
    (() => {
      if (window.__kshrDialogHooksInstalled) return true;
      window.__kshrDialogHooksInstalled = true;

      window.__kshrDialogQueue = window.__kshrDialogQueue || [];
      window.__kshrDialogDefaults = window.__kshrDialogDefaults || { confirm: false, prompt: null };
      const __pushDialog = (type, message, defaultText) => {
        window.__kshrDialogQueue.push({
          type,
          message: String(message || ''),
          default_text: defaultText == null ? null : String(defaultText),
          timestamp_ms: Date.now()
        });
        if (window.__kshrDialogQueue.length > 128) {
          window.__kshrDialogQueue.splice(0, window.__kshrDialogQueue.length - 128);
        }
      };
      ...
      window.confirm = function(message) {
        ...
        return !!window.__kshrDialogDefaults.confirm;
      };
      window.prompt = function(message, defaultValue) {
        __pushDialog('prompt', message, defaultValue == null ? null : defaultValue);
        const v = window.__kshrDialogDefaults.prompt;
    ...
    /// The underlying web view. Nil for blank tabs that have never navigated,
    /// and for hidden panels whose WKWebView has been evicted to save memory.
    ...
    // MARK: - Lazy/eviction state

    /// URL saved when the webview is evicted so it can be restored on demand.
    private var evictedURL: URL?
    /// Scroll Y position saved at eviction time for seamless restoration.
    private var evictedScrollY: CGFloat?
    /// Page zoom level saved at eviction time.
    private var evictedPageZoom: CGFloat?
    /// Session navigation history saved at eviction time.
    private var evictedBackHistoryURLStrings: [String]?
    private var evictedForwardHistoryURLStrings: [String]?
    /// Whether this panel's webview was evicted (hidden + timed out).
    private(set) var isEvicted: Bool = false

    /// Creates the WKWebView on demand if it does not already exist.
    /// Call this before any operation that requires a live webview (navigation, JS eval, etc.).
    @discardableResult
    func ensureWebView() -> WKWebView {
        ...
        webView = newWebView
        bindWebView(newWebView)
        applyRemoteProxyConfigurationIfAvailable()
        applyBrowserThemeModeIfNeeded()

        // Restore evicted state if applicable.
        if isEvicted {
            isEvicted = false
            if let zoom = evictedPageZoom {
                newWebView.pageZoom = zoom
                evictedPageZoom = nil
            }
            if let backHistory = evictedBackHistoryURLStrings,
               let forwardHistory = evictedForwardHistoryURLStrings {
                restoreSessionNavigationHistory(
                    backHistoryURLStrings: backHistory,
                    forwardHistoryURLStrings: forwardHistory,
                    currentURLString: evictedURL?.absoluteString
                )
                evictedBackHistoryURLStrings = nil
                evictedForwardHistoryURLStrings = nil
            }
            if let url = evictedURL {
                evictedURL = nil
            ...
            evictedScrollY = nil // scroll restore handled by page reload
        }

#if DEBUG
        dlog(
            "browser.webview.ensured panel=\(id.uuidString.prefix(5)) " +
    ...
    /// Evict the WKWebView to free memory. Saves URL, scroll position, zoom, and session
    /// history so the panel can be seamlessly restored later.
    func evictWebView() {
        guard let wv = webView else { return }
        guard shouldRenderWebView else {
            // Blank tab: just nil it out, nothing to save.
            tearDownWebView(wv)
            webView = nil
            return
        }
        ...
        evictedURL = Self.remoteProxyDisplayURL(for: wv.url) ?? currentURL
        evictedScrollY = nil // WKWebView doesn't expose scroll position easily; rely on URL reload
        evictedPageZoom = wv.pageZoom
        evictedBackHistoryURLStrings = history.backHistoryURLStrings
        evictedForwardHistoryURLStrings = history.forwardHistoryURLStrings
        isEvicted = true
...
#if DEBUG
        dlog(
            "browser.webview.evicted panel=\(id.uuidString.prefix(5)) " +
            "url=\(evictedURL?.absoluteString ?? "nil")"
        )
#endif
    }

    /// Shared teardown for removing a WKWebView (used by eviction and close).
    private func tearDownWebView(_ wv: WKWebView) {
        webViewObservers.removeAll()
        webViewCancellables.removeAll()
        faviconTask?.cancel()
        faviconTask = nil
        faviconRefreshGeneration &+= 1
        BrowserWindowPortalRegistry.detach(webView: wv)
        wv.stopLoading()
        wv.navigationDelegate = nil
        wv.uiDelegate = nil
        if let kshrWV = wv as? KshrWebView {
            kshrWV.onContextMenuDownloadStateChanged = nil
        }
    }
    ...
    (() => {
      try {
        const syncState = (state) => {
          window.__kshrAddressBarFocusState = state;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ kshrAddressBarFocusState: state }, "*");
            } else if (window.top) {
              window.top.__kshrAddressBarFocusState = state;
            }
          } catch (_) {}
        };
        ...
        let id = active.getAttribute("data-kshr-addressbar-focus-id");
        if (!id) {
          id = "kshr-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
          active.setAttribute("data-kshr-addressbar-focus-id", id);
        }
    ...
    (() => {
      try {
        if (window.__kshrAddressBarFocusTrackerInstalled) return true;
        ...
        if (window.top === window && !window.__kshrAddressBarFocusMessageBridgeInstalled) {
          window.__kshrAddressBarFocusMessageBridgeInstalled = true;
          window.addEventListener("message", (ev) => {
            try {
              const data = ev ? ev.data : null;
              if (!data || !Object.prototype.hasOwnProperty.call(data, "kshrAddressBarFocusState")) return;
              window.__kshrAddressBarFocusState = data.kshrAddressBarFocusState || null;
            } catch (_) {}
          }, true);
        }
        ...
        const ensureFocusId = (el) => {
          let id = el.getAttribute("data-kshr-addressbar-focus-id");
          if (!id) {
            id = "kshr-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
            el.setAttribute("data-kshr-addressbar-focus-id", id);
          }
          return id;
        };
    ...
    (() => {
      try {
        const readState = () => {
          let state = window.__kshrAddressBarFocusState;
          try {
            if ((!state || typeof state.id !== "string" || !state.id) &&
                window.top && window.top.__kshrAddressBarFocusState) {
              state = window.top.__kshrAddressBarFocusState;
            }
          } catch (_) {}
          return state;
        };
        ...
        const clearState = () => {
          window.__kshrAddressBarFocusState = null;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ kshrAddressBarFocusState: null }, "*");
            } else if (window.top) {
              window.top.__kshrAddressBarFocusState = null;
            }
          } catch (_) {}
        };
        ...
        const selector = '[data-kshr-addressbar-focus-id="' + state.id + '"]';
    ...
    private static func makeWebView(
        ...
        let webView = KshrWebView(frame: .zero, configuration: config)
    ...
    static func configureWebViewConfiguration(
        ...
        // methods and __kshr* globals as environment tampering, failing the challenge.
    ...
    private func bindWebView(_ webView: KshrWebView) {
    ...
    init(
        ...
        // Lazy webview: only create the WKWebView when we have a URL to load.
        // Blank tabs stay nil until first navigation, saving memory.
        self.webView = nil
        ...
        // Register for LRU eviction tracking.
        BrowserWebViewEvictionManager.shared.register(self)

        // Navigate to initial URL if provided - this triggers lazy webview creation.
    ...
    func reattachToWorkspace(
        ...
        if needsStoreSwap, let wv = webView {
    ...
    func switchToProfile(_ requestedProfileID: UUID) -> Bool {
        ...
        let desiredZoom = max(minPageZoom, min(maxPageZoom, previousWebView?.pageZoom ?? 1.0))
        ...
        if let previousWebView {
            tearDownWebView(previousWebView)
        }
    ...
    private func setupObservers(for webView: WKWebView) {
        ...
        let fullscreenObserver = webView.observe(\.fullscreenState, options: [.initial, .new]) { [weak self] webView, _ in
            let isElementFullscreenActive = webView.kshrIsElementFullscreenActiveOrTransitioning
    ...
    func debugSimulateWebContentProcessTermination() {
        guard let wv = webView else { return }
        replaceWebViewAfterContentProcessTermination(for: wv)
    }
#endif
    ...
    func focus() {
        ...
        guard let wv = webView,
              let window = wv.window, !wv.isHiddenOrHasHiddenAncestor else { return }

        // If nothing meaningful is loaded yet, prefer letting the omnibar take focus.
        if !wv.isLoading {
            let urlString = Self.remoteProxyDisplayURL(for: wv.url)?.absoluteString ?? currentURL?.absoluteString
            if urlString == nil || urlString == "about:blank" {
                return
            }
        }

        if Self.responderChainContains(window.firstResponder, target: wv) {
            noteWebViewFocused()
            return
        }
        if window.makeFirstResponder(wv) {
            noteWebViewFocused()
        }
    }
    ...
    func unfocus() {
        ...
        guard let wv = webView, let window = wv.window else { return }
        if Self.responderChainContains(window.firstResponder, target: wv) {
    ...
    func close() {
        BrowserWebViewEvictionManager.shared.unregister(self)
    ...
    private func performNavigation(
        ...
        // Ensure webview exists before navigation (lazy creation for blank tabs / eviction restore).
        let wv = ensureWebView()
        // Some installs can end up with a legacy Chrome UA override; keep this pinned.
        wv.customUserAgent = BrowserUserAgentSettings.safariUserAgent
        shouldRenderWebView = true
        if recordTypedNavigation {
            historyStore.recordTypedNavigation(url: originalURL)
        }
        navigationDelegate?.lastAttemptedURL = originalURL
        browserLoadRequest(effectiveRequest, in: wv)
    }
...
extension BrowserPanel {
    ...
    func resetForWorkspaceContextChange(reason: String) {
        ...
        // Nil out the webview; a fresh one will be lazily created on next navigation.
        webView = nil
        webViewInstanceID = UUID()
...
extension BrowserPanel {
    ...
    func reload() {
        guard let wv = webView else { return }
        wv.customUserAgent = BrowserUserAgentSettings.safariUserAgent
        if Self.serializableSessionHistoryURLString(Self.remoteProxyDisplayURL(for: wv.url)) == nil {
    ...
    private func prepareDeveloperToolsForRevealIfNeeded(_ inspector: NSObject) {
        ...
        inspector.kshrCallVoid(selector: attachSelector)
    }

    @discardableResult
    private func revealDeveloperTools(_ inspector: NSObject) -> Bool {
        let isVisibleSelector = NSSelectorFromString("isVisible")
        if inspector.kshrCallBool(selector: isVisibleSelector) ?? false {
            developerToolsDetachedOpenGraceDeadline = nil
        ...
        inspector.kshrCallVoid(selector: showSelector)
        let visibleAfterShow = inspector.kshrCallBool(selector: isVisibleSelector) ?? false
        if visibleAfterShow {
            developerToolsLastKnownVisibleAt = Date()
        }
    ...
    private func concealDeveloperTools(_ inspector: NSObject) -> Bool {
        let isVisibleSelector = NSSelectorFromString("isVisible")
        guard inspector.kshrCallBool(selector: isVisibleSelector) ?? false else { return true }
        ...
        for rawSelector in ["hide", "close"] {
            ...
            inspector.kshrCallVoid(selector: selector)
            if !(inspector.kshrCallBool(selector: isVisibleSelector) ?? false) {
                return true
            }
        }
        ...
        return !(inspector.kshrCallBool(selector: isVisibleSelector) ?? false)
    }
    ...
    private func performDeveloperToolsVisibilityTransition(
        ...
        guard let inspector = webView?.kshrInspectorObject() else { return false }

        let isVisibleSelector = NSSelectorFromString("isVisible")
        let visible = inspector.kshrCallBool(selector: isVisibleSelector) ?? false
        setPreferredDeveloperToolsVisible(targetVisible)
        ...
        if targetVisible {
            let visibleAfterTransition = inspector.kshrCallBool(selector: isVisibleSelector) ?? false
    ...
    func showDeveloperToolsConsole() -> Bool {
        ...
        guard let inspector = webView?.kshrInspectorObject() else { return true }
    ...
    func syncDeveloperToolsPreferenceFromInspector(preserveVisibleIntent: Bool = false) {
        guard let inspector = webView?.kshrInspectorObject() else { return }
        guard let visible = inspector.kshrCallBool(selector: NSSelectorFromString("isVisible")) else { return }
        if isDeveloperToolsTransitionInFlight {
    ...
    func consumeAttachedDeveloperToolsManualCloseIfNeeded(inspector: NSObject? = nil) -> Bool {
        ...
        guard let wv = webView, wv.superview != nil, wv.window != nil else { return false }
        ...
        guard let inspector = inspector ?? wv.kshrInspectorObject() else { return false }
        guard let visible = inspector.kshrCallBool(selector: NSSelectorFromString("isVisible")) else { return false }
        guard !visible else {
            developerToolsLastKnownVisibleAt = Date()
            return false
        }
    ...
    func restoreDeveloperToolsAfterAttachIfNeeded() {
        ...
        guard let inspector = webView?.kshrInspectorObject() else {
            scheduleDeveloperToolsRestoreRetry()
            return
        }
        ...
        let visible = inspector.kshrCallBool(selector: NSSelectorFromString("isVisible")) ?? false
        if visible {
            developerToolsDetachedOpenGraceDeadline = nil
            syncDeveloperToolsPresentationPreferenceFromUI()
            developerToolsLastKnownVisibleAt = Date()
        ...
        kshrWithWindowFirstResponderBypass {
            _ = revealDeveloperTools(inspector)
        }
        setPreferredDeveloperToolsVisible(true)
        let visibleAfterShow = inspector.kshrCallBool(selector: NSSelectorFromString("isVisible")) ?? false
        if visibleAfterShow {
            syncDeveloperToolsPresentationPreferenceFromUI()
            developerToolsLastKnownVisibleAt = Date()
            cancelDeveloperToolsRestoreRetry()
    ...
    func isDeveloperToolsVisible() -> Bool {
        guard let inspector = webView?.kshrInspectorObject() else { return false }
        return inspector.kshrCallBool(selector: NSSelectorFromString("isVisible")) ?? false
    }
    ...
    @discardableResult
    func zoomIn() -> Bool {
        applyPageZoom((webView?.pageZoom ?? 1.0) + pageZoomStep)
    }
    ...
    func evaluateJavaScript(_ script: String) async throws -> Any? {
        ...
        return try await wv.evaluateJavaScript(script)
    }
    ...
    func findNext() {
        Task { @MainActor [weak self] in
            ...
            let result = try? await wv.evaluateJavaScript(BrowserFindJavaScript.nextScript())
    ...
    func findPrevious() {
        Task { @MainActor [weak self] in
            guard let self, let wv = self.webView else { return }
            let result = try? await wv.evaluateJavaScript(BrowserFindJavaScript.previousScript())
            self.parseFindResult(result)
        }
    }
    ...
    private func executeFindClear() {
        Task { @MainActor [weak self] in
            guard let self, let wv = self.webView else { return }
            do {
                _ = try await wv.evaluateJavaScript(BrowserFindJavaScript.clearScript())
            } catch {
    ...
    /// Called when this panel becomes visible in the UI (e.g., selected workspace/tab).
    /// Cancels any pending eviction and restores the webview if it was evicted.
    func notePanelBecameVisible() {
        BrowserWebViewEvictionManager.shared.noteVisible(self)
    }

    /// Called when this panel becomes hidden in the UI (e.g., workspace switch).
    /// Starts the eviction countdown.
    func notePanelBecameHidden() {
        BrowserWebViewEvictionManager.shared.noteHidden(self)
    }
    ...
    func captureFocusIntent(in window: NSWindow?) -> PanelFocusIntent {
        ...
        if let window, let wv = webView,
           Self.responderChainContains(window.firstResponder, target: wv) {
    ...
    func ownedFocusIntent(for responder: NSResponder, in window: NSWindow) -> PanelFocusIntent? {
        ...
        if let wv = webView, Self.responderChainContains(responder, target: wv) {
            return .browser(.webView)
        }
    ...
    func yieldFocusIntent(_ intent: PanelFocusIntent, in window: NSWindow) -> Bool {
        ...
        case .webView:
            guard let wv = webView,
                  Self.responderChainContains(window.firstResponder, target: wv) else { return false }
    ...
    private func restoreAddressBarPageFocusAttemptIfNeeded(
        ...
        wv.evaluateJavaScript(Self.addressBarFocusRestoreScript) { [weak self] result, error in
            guard let self else {
                completion(false)
                return
            }
...
private extension BrowserPanel {
    func applyBrowserThemeModeIfNeeded() {
        guard let wv = webView else { return }
        BrowserThemeSettings.apply(browserThemeMode, to: wv)
    }
...
extension BrowserPanel {
    ...
    func debugDeveloperToolsStateSummary() -> String {
        ...
        let inspector = webView?.kshrInspectorObject() == nil ? 0 : 1
    ...
    func debugDeveloperToolsGeometrySummary() -> String {
        ...
        return "webFrame=\(Self.debugRectDescription(webFrame)) webBounds=\(Self.debugRectDescription(webView?.bounds ?? .zero)) webWin=\(webView?.window?.windowNumber ?? -1) super=\(Self.debugObjectToken(container)) superType=\(containerType) superBounds=\(Self.debugRectDescription(containerBounds)) inspectorHApprox=\(String(format: "%.1f", inspectorHeightApprox)) inspectorInsets=\(String(format: "%.1f", inspectorInsets)) inspectorOverflow=\(String(format: "%.1f", inspectorOverflow)) inspectorSubviews=\(inspectorSubviews)"
    }
...
private extension BrowserPanel {
    ...
    func applyPageZoom(_ candidate: CGFloat) -> Bool {
        ...
        if abs(wv.pageZoom - clamped) < 0.0001 {
            return false
        }
        wv.pageZoom = clamped
        return true
...
extension WKWebView {
    func kshrInspectorObject() -> NSObject? {
    ...
    func kshrInspectorFrontendWebView() -> WKWebView? {
        guard let inspector = kshrInspectorObject() else { return nil }
...
private extension NSObject {
    func kshrCallBool(selector: Selector) -> Bool? {
    ...
    func kshrCallVoid(selector: Selector) {
...
class BrowserDownloadDelegate: NSObject, WKDownloadDelegate {
    ...
    private static let tempDir: URL = {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("kshr-downloads", isDirectory: true)
...
private class BrowserNavigationDelegate: NSObject, WKNavigationDelegate {
    ...
    func webView(
        ...
        let hasRecentMiddleClickIntent = KshrWebView.hasRecentMiddleClickIntent(for: webView)
...
private class BrowserUIDelegate: NSObject, WKUIDelegate {
    ...
    func webView(
        ...
        let isScriptedPopup = browserNavigationShouldCreatePopup(
            ...
            modifierFlags: navigationAction.modifierFlags,
            buttonNumber: navigationAction.buttonNumber,
            hasRecentMiddleClickIntent: KshrWebView.hasRecentMiddleClickIntent(for: webView)
        )
    ...
    var errorDescription: String? {
        ...
        case .missingDestinationProfile:
            return String(
                ...
                defaultValue: "The selected kshr browser profile no longer exists. Pick a destination profile again."
            )
        case .profileCreationFailed(let name):
            return String(
                format: String(
                    localized: "browser.import.error.destinationCreateFailed",
                    defaultValue: "kshr could not create the destination profile \"%@\"."
                ),
                name
            )
        }
            ...
            lines.append(
                String(
                    format: String(
                        ...
                        defaultValue: "Created kshr profiles: %@"
                    ),
...
enum BrowserDataImporter {
    ...
    private static func querySQLiteRows(
        ...
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent(
            "kshr-browser-import-\(UUID().uuidString)",
            isDirectory: true
        )
...
enum BrowserImportUITestFixtureLoader {
    ...
    static func browsers(from environment: [String: String]) -> [InstalledBrowserCandidate]? {
        guard let rawFixture = environment["KSHR_UI_TEST_BROWSER_IMPORT_FIXTURE"],
        ...
        let resolvedProfiles = fixture.profiles.enumerated().map { index, name in
            InstalledBrowserProfile(
                ...
                rootURL: FileManager.default.temporaryDirectory
                    .appendingPathComponent("kshr-ui-test-browser-import")
    ...
    static func destinationProfiles(from environment: [String: String]) -> [BrowserProfileDefinition]? {
        guard let rawDestinations = environment["KSHR_UI_TEST_BROWSER_IMPORT_DESTINATIONS"],
...
final class BrowserDataImportCoordinator {
    ...
    private func presentImportDialog(
        ...
        guard !browsers.isEmpty else {
            ...
            alert.informativeText = String(
                ...
                defaultValue: "kshr could not find browser profiles to import from on this Mac."
            )
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK"))
            alert.runModal()
            return
    ...
    private func captureSelectionIfRequested(
        ...
        let environment = ProcessInfo.processInfo.environment
        guard environment["KSHR_UI_TEST_BROWSER_IMPORT_MODE"] == "capture-only" else { return false }
        guard let path = environment["KSHR_UI_TEST_BROWSER_IMPORT_CAPTURE_PATH"], !path.isEmpty else {
            return true
        }
    ...
    private final class ImportWizardWindowController: NSObject, @preconcurrency NSWindowDelegate {
        ...
        private func setupSourceProfilesContainer() {
            ...
            sourceProfilesHelpLabel.stringValue = String(
                ...
                defaultValue: "Choose one or more source profiles. Step 3 lets you keep them separate or merge them into one kshr profile."
            )
        ...
        private func setupDataTypesContainer() {
            ...
            mergeProfilesRadio.title = String(
                ...
                defaultValue: "Merge all into one kshr profile"
            )
            ...
            let destinationTitleLabel = NSTextField(
                labelWithString: String(
                    localized: "browser.import.destination.kshr",
                    defaultValue: "kshr destination"
                )
            )
        ...
        private func rebuildStep3DestinationUI() {
            ...
            if presentation.showsSeparateRows {
                destinationHelpLabel.stringValue = String(
                    ...
                    defaultValue: "Missing kshr profiles are created when import starts."
                )
                destinationHelpLabel.isHidden = false
            } else if plan.entries.count > 1 {
                destinationHelpLabel.stringValue = String(
                    localized: "browser.import.destinationProfile.mergeHelp",
                    defaultValue: "All selected source profiles will be merged into the chosen kshr browser profile."
                )
                destinationHelpLabel.isHidden = false
            } else {

...
/// Hosts a popup `KshrWebView` in a standalone `NSPanel`, created when a page
...
/// `kshr_performKeyEquivalent` can dispatch it to the main menu's
...
final class BrowserPopupWindowController: NSObject, NSWindowDelegate {
    ...
    init(
        ...
        // overlaying the opener's browser context so OAuth popups keep kshr's
        // shared cookie/storage scope and opener linkage.
        let webView = KshrWebView(frame: .zero, configuration: configuration)
        ...
        panel.identifier = NSUserInterfaceItemIdentifier("kshr.browser-popup")

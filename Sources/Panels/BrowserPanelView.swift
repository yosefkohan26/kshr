...
private extension View {
    func kshrFlatSymbolColorRendering() -> some View {
...
struct BrowserPanelView: View {
    ...
    private var remoteSuggestionsEnabled: Bool {
        ...
        if ProcessInfo.processInfo.environment["KSHR_UI_TEST_REMOTE_SUGGESTIONS_JSON"] != nil ||
            UserDefaults.standard.string(forKey: "KSHR_UI_TEST_REMOTE_SUGGESTIONS_JSON") != nil {
            return true
        }
        // Keep UI tests deterministic by disabling network suggestions when requested.
        if ProcessInfo.processInfo.environment["KSHR_UI_TEST_DISABLE_REMOTE_SUGGESTIONS"] == "1" {
            return false
        }
    ...
    var body: some View {
        ...
        .overlay {
            RoundedRectangle(cornerRadius: FocusFlashPattern.ringCornerRadius)
                .stroke(kshrAccentColor().opacity(focusFlashOpacity), lineWidth: 3)
                .shadow(color: kshrAccentColor().opacity(focusFlashOpacity * 0.35), radius: 10)
        ...
        .onReceive(NotificationCenter.default.publisher(for: .webViewDidReceiveClick).filter { [weak panel] note in
            ...
            guard let webView = note.object as? KshrWebView else { return false }
        ...
        .onChange(of: isVisibleInUI) { visibleInUI in
            if visibleInUI {
                panel.notePanelBecameVisible()
                panel.cancelPendingDeveloperToolsVisibilityLossCheck()
                return
            }
            panel.notePanelBecameHidden()
    ...
    private var omnibarField: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: omnibarPillCornerRadius, style: .continuous)
                .stroke(addressBarFocused ? kshrAccentColor() : Color.clear, lineWidth: 1)
        )
    ...
    private func syncWebViewResponderPolicyWithViewState(
        ...
        guard let kshrWebView = panel.webView as? KshrWebView else { return }
        let isPanelFocused = isPanelFocusedOverride ?? isFocused
        let next = isPanelFocused && !panel.shouldSuppressWebViewFocus()
        if kshrWebView.allowsFirstResponderAcquisition != next {
#if DEBUG
            dlog(
                "browser.focus.policy.resync panel=\(panel.id.uuidString.prefix(5)) " +
                "web=\(ObjectIdentifier(kshrWebView)) old=\(kshrWebView.allowsFirstResponderAcquisition ? 1 : 0) " +
                "new=\(next ? 1 : 0) reason=\(reason) " +
                "panelFocusedUsed=\(isPanelFocused ? 1 : 0)"
            )
#endif
        }
        kshrWebView.allowsFirstResponderAcquisition = next
    }
    ...
    private func logBrowserFocusState(event: String, detail: String = "") {
        ...
        let webResponder: Int = {
            guard let wv = panel.webView else { return 0 }
            return browserFocusResponderChainContains(firstResponder, target: wv) ? 1 : 0
        }()
        var line =
    ...
    private func autoFocusOmnibarIfBlank() {
        ...
        guard !(panel.webView?.isLoading ?? false) else {
#if DEBUG
    ...
    private func forcedRemoteSuggestionsForUITest() -> [String]? {
        let raw = ProcessInfo.processInfo.environment["KSHR_UI_TEST_REMOTE_SUGGESTIONS_JSON"]
    ...
    private func applyOmnibarEffects(_ effects: OmnibarEffects) {
        ...
        if effects.shouldBlurToWebView {
            ...
            DispatchQueue.main.async {
                guard let wv = panel.webView,
                      let window = wv.window,
                      !wv.isHiddenOrHasHiddenAncestor else { return }
                ...
                let focusedWebView = window.makeFirstResponder(wv)
                if focusedWebView {
                ...
                panel.restoreAddressBarPageFocusIfNeeded { restored in
                    ...
                    var hasWebViewResponder =
                        browserFocusResponderChainContains(window.firstResponder, target: wv)
                    if !hasWebViewResponder {
                        let fallbackFocusedWebView = window.makeFirstResponder(wv)
...
struct WebViewRepresentable: NSViewRepresentable {
        ...
        private func updateHostedInspectorDockControlAvailabilityIfNeeded(reason: String) {
            ...
            hostedInspectorFrontendWebView.evaluateJavaScript(
                ...
                (() => {
                    ...
                    if (!WI.__kshrOriginalUpdateDockNavigationItems && typeof WI._updateDockNavigationItems === "function")
                        WI.__kshrOriginalUpdateDockNavigationItems = WI._updateDockNavigationItems;
                    if (!WI.__kshrOriginalDockLeft && typeof WI._dockLeft === "function")
                        WI.__kshrOriginalDockLeft = WI._dockLeft;
                    if (!WI.__kshrOriginalDockRight && typeof WI._dockRight === "function")
                        WI.__kshrOriginalDockRight = WI._dockRight;
                    if (!WI.__kshrOriginalTogglePreviousDockConfiguration && typeof WI._togglePreviousDockConfiguration === "function")
                        WI.__kshrOriginalTogglePreviousDockConfiguration = WI._togglePreviousDockConfiguration;
                    ...
                    function enforceDockControls() {
                        const disallowSideDock = !WI.__kshrAllowSideDock;
                    ...
                    WI.__kshrAllowSideDock = allowSideDock;
                    WI._dockLeft = function(event) {
                        if (!WI.__kshrAllowSideDock)
                            return callOriginal(WI._dockBottom, event);
                        return callOriginal(WI.__kshrOriginalDockLeft, event);
                    };
                    WI._dockRight = function(event) {
                        if (!WI.__kshrAllowSideDock)
                            return callOriginal(WI._dockBottom, event);
                        return callOriginal(WI.__kshrOriginalDockRight, event);
                    };
                    WI._togglePreviousDockConfiguration = function(event) {
                        ...
                        if (!WI.__kshrAllowSideDock && previousSideDock)
                            return callOriginal(WI._dockBottom, event);
                        return callOriginal(WI.__kshrOriginalTogglePreviousDockConfiguration, event);
                    };
                    WI._updateDockNavigationItems = function(...args) {
                        if (typeof WI.__kshrOriginalUpdateDockNavigationItems === "function")
                            WI.__kshrOriginalUpdateDockNavigationItems.apply(WI, args);
                        enforceDockControls();
                    };
                    WI._updateDockNavigationItems();
                    return WI.__kshrAllowSideDock;
                })();
                """,
    ...
    private static func shouldPreserveExternalFullscreenHost(
        ...
        webView.kshrIsManagedByExternalFullscreenWindow(relativeTo: expectedWindow)
    }
    ...
    private func updateUsingLocalInlineHosting(_ nsView: NSView, context: Context, webView: WKWebView) -> Bool {
        ...
        host.setHostedInspectorFrontendWebView(webView.kshrInspectorFrontendWebView())
    ...
    func updateNSView(_ nsView: NSView, context: Context) {
        // Ensure webview exists when rendering is expected (e.g., restoring from eviction).
        guard let webView = panel.webView ?? (panel.shouldRenderWebView ? panel.ensureWebView() : nil) else {
            return
        }
        let coordinator = context.coordinator
    ...
    private static func applyWebViewFirstResponderPolicy(
        ...
        guard let kshrWebView = webView as? KshrWebView else { return }
        let next = isPanelFocused && !panel.shouldSuppressWebViewFocus()
        if kshrWebView.allowsFirstResponderAcquisition != next {
#if DEBUG
            dlog(

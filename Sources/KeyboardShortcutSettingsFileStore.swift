...
final class KshrSettingsFileStore {
    static let shared = KshrSettingsFileStore()

    static let currentSchemaVersion = 1
    static let schemaURLString = "https://raw.githubusercontent.com/yosefkohan26/kshr/main/web/data/kshr-settings.schema.json"

    private static let releaseBundleIdentifier = "com.kshrterm.app"
    private static let backupsDefaultsKey = "kshr.settingsFile.backups.v1"
    ...
    static var defaultPrimaryPath: String {
        ...
        return (home as NSString).appendingPathComponent(".config/kshr/settings.json")
    }
    ...
    init(
        primaryPath: String = KshrSettingsFileStore.defaultPrimaryPath,
        fallbackPath: String? = KshrSettingsFileStore.defaultFallbackPath,
        ...
        trustObserver = notificationCenter.addObserver(
            forName: KshrDirectoryTrust.didChangeNotification,
            object: nil,
            queue: nil
        ) { [weak self] _ in
            self?.reapplyManagedSettingsIfNeeded()
    ...
    private func bootstrapPrimaryTemplateIfNeeded() {
        ...
        do {
            ...
            NSLog("[KshrSettingsFileStore] failed to bootstrap %@: %@", primaryPath, String(describing: error))
        }
    }
    ...
    private func loadSettings(at path: String) -> LoadResult {
        ...
        do {
            ...
            NSLog("[KshrSettingsFileStore] parse error at %@: %@", path, String(describing: error))
            return .invalid
        }
    }
    ...
    private func parseSettingsFile(root: [String: Any], sourcePath: String) -> ResolvedSettingsSnapshot {
        ...
        if schemaVersion > Self.currentSchemaVersion {
            NSLog(
                "[KshrSettingsFileStore] %@ uses future schemaVersion %d; parsing known fields only",
                sourcePath,
                schemaVersion
            )
        }
    ...
    private func parseSidebarSection(
        ...
        if let value = jsonBool(section["openPullRequestLinksInKshrBrowser"]) {
            snapshot.managedUserDefaults[BrowserLinkOpenSettings.openSidebarPullRequestLinksInKshrBrowserKey] = .bool(value)
        }
        if let value = jsonBool(section["openPortLinksInKshrBrowser"]) {
            snapshot.managedUserDefaults[BrowserLinkOpenSettings.openSidebarPortLinksInKshrBrowserKey] = .bool(value)
        }
        ...
        if section.keys.contains("colors") {
            ...
            for (rawName, rawValue) in rawColors {
                ...
                guard !name.isEmpty else {
                    NSLog("[KshrSettingsFileStore] ignoring empty workspace color name in %@", sourcePath)
                    continue
                }
                guard let hex = jsonString(rawValue),
                      let normalizedHex = WorkspaceTabColorSettings.normalizedHex(hex) else {
                    NSLog("[KshrSettingsFileStore] ignoring invalid workspace color '%@' in %@", name, sourcePath)
                    continue
                }
        ...
        if let rawOverrides = section["paletteOverrides"] as? [String: Any] {
            ...
            for (name, rawValue) in rawOverrides {
                guard validNames.contains(name) else {
                    NSLog("[KshrSettingsFileStore] ignoring unknown workspace color '%@' in %@", name, sourcePath)
                    continue
                }
                guard let hex = jsonString(rawValue),
                      let normalizedHex = WorkspaceTabColorSettings.normalizedHex(hex) else {
                    NSLog("[KshrSettingsFileStore] ignoring invalid workspace color override '%@' in %@", name, sourcePath)
                    continue
                }
    ...
    private func parseAutomationSection(
        ...
        if let raw = jsonString(section["socketControlMode"]) {
            let knownModes = Set([
                "off", "kshronly", "automation", "password", "allowall", "openaccess", "fullopenaccess",
                "notifications", "full",
            ])
        ...
        if let value = jsonInt(section["portBase"]) {
            ...
            snapshot.managedUserDefaults["kshrPortBase"] = .int(value)
        }
        if let value = jsonInt(section["portRange"]) {
            ...
            snapshot.managedUserDefaults["kshrPortRange"] = .int(value)
        }
    }
    ...
    private func parseBrowserSection(
        ...
        if let value = jsonBool(section["openTerminalLinksInKshrBrowser"]) {
            snapshot.managedUserDefaults[BrowserLinkOpenSettings.openTerminalLinksInKshrBrowserKey] = .bool(value)
        }
        if let value = jsonBool(section["interceptTerminalOpenCommandInKshrBrowser"]) {
            snapshot.managedUserDefaults[BrowserLinkOpenSettings.interceptTerminalOpenCommandInKshrBrowserKey] = .bool(value)
        }
    ...
    private func parseShortcutsSection(
        ...
        for (rawAction, rawBinding) in bindings {
            guard let action = KeyboardShortcutSettings.Action(rawValue: rawAction) else {
                NSLog("[KshrSettingsFileStore] ignoring unknown shortcut action '%@' in %@", rawAction, sourcePath)
                continue
            }
            guard let shortcut = parseShortcutBindingValue(rawBinding, action: action) else {
                NSLog(
                    "[KshrSettingsFileStore] ignoring invalid shortcut binding for '%@' in %@",
                    rawAction,
                    sourcePath
                )
                continue
        ...
        if updateBackups {
            ...
            if snapshot.managedCustomSettings.trustedDirectories != nil,
               backups[Self.trustedDirectoriesBackupIdentifier] == nil {
                backups[Self.trustedDirectoriesBackupIdentifier] = .stringArray(KshrDirectoryTrust.shared.allTrustedPaths)
            }
    ...
    private func applyManagedCustomSettings(_ settings: ManagedCustomSettings) {
        if let trustedDirectories = settings.trustedDirectories,
           KshrDirectoryTrust.shared.allTrustedPaths != trustedDirectories {
            KshrDirectoryTrust.shared.replaceAll(with: trustedDirectories)
        }
    ...
    private func restoreBackup(_ backup: BackupValue, for identifier: String) {
        ...
        case Self.trustedDirectoriesBackupIdentifier:
            if case .stringArray(let values) = backup {
                KshrDirectoryTrust.shared.replaceAll(with: values)
            } else {
                KshrDirectoryTrust.shared.replaceAll(with: [])
            }
    ...
    private func logInvalid(_ path: String, sourcePath: String) {
        NSLog("[KshrSettingsFileStore] ignoring invalid setting '%@' in %@", path, sourcePath)
    }
    ...
    static func defaultTemplate() -> String {
        var lines: [String] = [
            ...
            "  // kshr creates this template on launch when both settings file locations are missing.",
            "  // ~/.config/kshr/settings.json takes precedence over the Application Support fallback.",
            "",
        ]
        ...
        return [
            ...
            [
                "sidebar": [
                    ...
                    "openPullRequestLinksInKshrBrowser": BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInKshrBrowser,
                    "openPortLinksInKshrBrowser": BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInKshrBrowser,
                    "showSSH": true,
                    "showPorts": true,
                    "showLog": true,
                    "showProgress": true,
...
typealias KeyboardShortcutSettingsFileStore = KshrSettingsFileStore
...
private struct ManagedCustomSettings: Equatable {
    ...
    var managedIdentifiers: Set<String> {
        ...
        if trustedDirectories != nil {
            identifiers.insert(KshrSettingsFileStore.trustedDirectoriesBackupIdentifier)
        }
        if socketPassword != nil {
            identifiers.insert(KshrSettingsFileStore.socketPasswordBackupIdentifier)
        }
        return identifiers
    }
}
...
private final class ShortcutSettingsFileWatcher {
    ...
    private let watchQueue = DispatchQueue(label: "com.kshr.shortcut-settings-file-watch")

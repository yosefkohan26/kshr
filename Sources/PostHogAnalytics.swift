...
final class PostHogAnalytics {
    ...
    // PostHog Cloud US default (matches other kshr properties).
    private let host = "https://us.i.posthog.com"

    private let dailyActiveEvent = "kshr_daily_active"
    private let hourlyActiveEvent = "kshr_hourly_active"
    ...
    private init() {
        workQueue = DispatchQueue(label: "com.kshr.posthog.analytics", qos: .utility)
    ...
    private var isEnabled: Bool {
        ...
        return ProcessInfo.processInfo.environment["KSHR_POSTHOG_ENABLE"] == "1"
#else
...
#if DEBUG
        config.debug = ProcessInfo.processInfo.environment["KSHR_POSTHOG_DEBUG"] == "1"
#endif
    ...
    nonisolated static func superProperties(infoDictionary: [String: Any]) -> [String: Any] {
        var properties: [String: Any] = ["platform": "kshrterm"]
    ...
    nonisolated static func shouldFlushAfterCapture(event: String) -> Bool {
        switch event {
        case "kshr_daily_active", "kshr_hourly_active":
            return true
        default:
            return false
        }

...
struct KshrSurfaceConfigTemplate {
...
func kshrSurfaceContextName(_ context: ghostty_surface_context_e) -> String {
    switch context {
...
private func kshrPointerAppearsLive(_ pointer: UnsafeMutableRawPointer?) -> Bool {
    guard let pointer,
...
func kshrSurfacePointerAppearsLive(_ surface: ghostty_surface_t) -> Bool {
    ...
    kshrPointerAppearsLive(surface)
}

func kshrCurrentSurfaceFontSizePoints(_ surface: ghostty_surface_t) -> Float? {
    guard kshrSurfacePointerAppearsLive(surface) else {
        return nil
    }
...
func kshrInheritedSurfaceConfig(
    sourceSurface: ghostty_surface_t,
    context: ghostty_surface_context_e
) -> KshrSurfaceConfigTemplate {
    let inherited = ghostty_surface_inherited_config(sourceSurface, context)
    var config = KshrSurfaceConfigTemplate(cConfig: inherited)

    // Make runtime zoom inheritance explicit, even when Ghostty's
    // inherit-font-size config is disabled.
    let runtimePoints = kshrCurrentSurfaceFontSizePoints(sourceSurface)
    ...
    dlog(
        "zoom.inherit context=\(kshrSurfaceContextName(context)) " +
...
extension Workspace {
    ...
    private func sessionPanelSnapshot(panelId: UUID, includeScrollback: Bool) -> SessionPanelSnapshot? {
        ...
        if remoteDetectedSurfaceIds.contains(panelId) || isRemoteTerminalSurface(panelId) {
            ...
            listeningPorts = (telemetry.surfaceListeningPorts[panelId] ?? []).sorted()
        }
    ...
    private func applySessionPanelMetadata(_ snapshot: SessionPanelSnapshot, toPanelId panelId: UUID) {
        ...
        telemetry.surfaceListeningPorts[panelId] = Array(Set(snapshot.listeningPorts)).sorted()
...
// MARK: - kshr.json custom layout

extension Workspace {

    func applyCustomLayout(_ layout: KshrLayoutNode, baseCwd: String) {
        guard let rootPaneId = bonsplitController.allPaneIds.first else { return }

        var leaves: [(paneId: PaneID, surfaces: [KshrSurfaceDefinition])] = []
    ...
    private func buildCustomLayoutTree(
        _ node: KshrLayoutNode,
        inPane paneId: PaneID,
        leaves: inout [(paneId: PaneID, surfaces: [KshrSurfaceDefinition])]
    ) {
        switch node {
        case .pane(let pane):
        ...
        case .split(let split):
            guard split.children.count == 2 else {
                NSLog("[KshrConfig] split node requires exactly 2 children, got %d", split.children.count)
    ...
    private func populateCustomPane(
        _ paneId: PaneID,
        surfaces: [KshrSurfaceDefinition],
        baseCwd: String,
    ...
    private func configureExistingSurface(
        ...
        case .terminal where surface.cwd != nil || surface.env != nil:
            ...
            let resolvedCwd = KshrConfigStore.resolveCwd(surface.cwd, relativeTo: baseCwd)
    ...
    private func sendInputWhenReady(_ text: String, to panel: TerminalPanel) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            ...
            resolved = true
            if let observer { NotificationCenter.default.removeObserver(observer) }
            NSLog("[KshrConfig] surface not ready after 3s, dropping command (%d chars)", text.count)
        }
    }
}
...
final class WorkspaceRemoteDaemonPendingCallRegistry {
    ...
    private let queue = DispatchQueue(label: "com.kshr.remote-ssh.daemon-rpc.pending.\(UUID().uuidString)")
...
private final class WorkspaceRemoteDaemonRPCClient {
    ...
    private let writeQueue = DispatchQueue(label: "com.kshr.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    private let stateQueue = DispatchQueue(label: "com.kshr.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
    ...
    func start() throws {
        ...
        do {
            ...
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 1, userInfo: [
        ...
        do {
            ...
            guard capabilities.contains(Self.requiredProxyStreamCapability) else {
                throw NSError(domain: "kshr.remote.daemon.rpc", code: 2, userInfo: [
    ...
    func openStream(host: String, port: Int, timeoutMs: Int = 10000) throws -> String {
        ...
        guard !streamID.isEmpty else {
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 3, userInfo: [
    ...
    func attachStream(
        ...
        guard !trimmedStreamID.isEmpty else {
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 17, userInfo: [
    ...
    private func call(method: String, params: [String: Any], timeout: TimeInterval) throws -> [String: Any] {
        ...
        do {
            ...
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 10, userInfo: [
        ...
        case .timedOut:
            ...
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 11, userInfo: [
                NSLocalizedDescriptionKey: "daemon RPC timeout waiting for \(method) response",
            ])
        case .failure(let failure):
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 12, userInfo: [
                NSLocalizedDescriptionKey: failure,
            ])
        case .missing:
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 13, userInfo: [
        ...
        throw NSError(domain: "kshr.remote.daemon.rpc", code: 14, userInfo: [
    ...
    private func writePayload(_ payload: Data) throws {
        ...
        if stdinHandle === FileHandle.nullDevice {
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 15, userInfo: [
        ...
        do {
            ...
            stop(suppressTerminationCallback: false)
            throw NSError(domain: "kshr.remote.daemon.rpc", code: 16, userInfo: [
...
private final class WorkspaceRemoteDaemonProxyTunnel {
    private final class ProxySession {
        ...
        private static let remoteLoopbackProxyAliasHost = "kshr-loopback.localtest.me"
        ...
        private func parseSocksRequest(from data: Data) throws -> SocksRequest? {
            ...
            guard bytes[0] == 0x05 else {
                throw NSError(domain: "kshr.remote.proxy", code: 1, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS version"])
            }
            ...
            default:
                throw NSError(domain: "kshr.remote.proxy", code: 2, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS address type"])
            }

            guard !host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                throw NSError(domain: "kshr.remote.proxy", code: 3, userInfo: [NSLocalizedDescriptionKey: "empty SOCKS host"])
            }
            ...
            guard port > 0 && port <= 65535 else {
                throw NSError(domain: "kshr.remote.proxy", code: 4, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS port"])
            }
    ...
    private let queue = DispatchQueue(label: "com.kshr.remote-ssh.daemon-tunnel.\(UUID().uuidString)", qos: .utility)
    ...
    func start() throws {
        ...
        queue.sync {
            guard !isStopped else {
                capturedError = NSError(domain: "kshr.remote.proxy", code: 20, userInfo: [
    ...
    private static func makeLoopbackListener(port: Int) throws -> NWListener {
        guard let localPort = NWEndpoint.Port(rawValue: UInt16(port)) else {
            throw NSError(domain: "kshr.remote.proxy", code: 21, userInfo: [
...
private final class WorkspaceRemoteProxyBroker {
    ...
    private let queue = DispatchQueue(label: "com.kshr.remote-ssh.proxy-broker", qos: .utility)
...
private final class WorkspaceRemoteCLIRelayServer {
    private final class Session {
        ...
        private let challengeProtocol = "kshr-relay-auth"
        ...
        private static func roundTripUnixSocket(socketPath: String, request: Data) throws -> Data {
            ...
            guard fd >= 0 else {
                throw NSError(domain: "kshr.remote.relay", code: 1, userInfo: [
            ...
            guard pathBytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
                throw NSError(domain: "kshr.remote.relay", code: 2, userInfo: [
            ...
            guard connectResult == 0 else {
                throw NSError(domain: "kshr.remote.relay", code: 3, userInfo: [
                    NSLocalizedDescriptionKey: "failed to connect to local kshr socket",
                ])
            }
            ...
            try request.withUnsafeBytes { rawBuffer in
                ...
                while bytesRemaining > 0 {
                    ...
                    if written <= 0 {
                        throw NSError(domain: "kshr.remote.relay", code: 4, userInfo: [
            ...
            while true {
                ...
                if errno == EAGAIN || errno == EWOULDBLOCK {
                    ...
                    throw NSError(domain: "kshr.remote.relay", code: 5, userInfo: [
                        NSLocalizedDescriptionKey: "timed out waiting for local kshr response",
                    ])
                }
                throw NSError(domain: "kshr.remote.relay", code: 6, userInfo: [
                    NSLocalizedDescriptionKey: "failed to read local kshr response",
                ])
            }
            return response
        }
    ...
    private let relayID: String
    private let relayToken: Data
    private let queue = DispatchQueue(label: "com.kshr.remote-ssh.cli-relay.\(UUID().uuidString)", qos: .utility)
    ...
    init(localSocketPath: String, relayID: String, relayTokenHex: String) throws {
        guard let relayToken = Session.hexData(from: relayTokenHex), !relayToken.isEmpty else {
            throw NSError(domain: "kshr.remote.relay", code: 7, userInfo: [
    ...
    func start() throws -> Int {
        ...
        if waitResult != .success {
            ...
            listener.stateUpdateHandler = nil
            listener.cancel()
            throw NSError(domain: "kshr.remote.relay", code: 8, userInfo: [
...
final class WorkspaceRemoteSessionController {
    ...
    private let queue = DispatchQueue(label: "com.kshr.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    private func beginConnectionAttemptLocked() {
        ...
        do {
            ...
            guard hello.capabilities.contains(WorkspaceRemoteDaemonRPCClient.requiredProxyStreamCapability) else {
                throw NSError(domain: "kshr.remote.daemon", code: 43, userInfo: [
    ...
    private func requestBootstrapRemoteTTYIfNeededLocked() {
        ...
        let command = "sh -c \(Self.shellSingleQuoted("tty_path=\"$HOME/.kshr/relay/\(relayPort).tty\"; if [ -r \"$tty_path\" ]; then cat \"$tty_path\"; fi"))"
        do {
    ...
    private static let remotePlatformProbeOSMarker = "__KSHR_REMOTE_OS__="
    private static let remotePlatformProbeArchMarker = "__KSHR_REMOTE_ARCH__="
    private static let remotePlatformProbeExistsMarker = "__KSHR_REMOTE_EXISTS__="
    ...
    private func runProcess(
        ...
        let captureQueue = DispatchQueue(label: "kshr.remote.process.capture")
        ...
        do {
            ...
            throw NSError(domain: "kshr.remote.process", code: 1, userInfo: [
        ...
        if !didExitBeforeTimeout, process.isRunning {
            ...
            throw NSError(domain: "kshr.remote.process", code: 2, userInfo: [
    ...
    private func installRemoteRelayMetadataLocked(
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "kshr.remote.relay", code: 70, userInfo: [
    ...
    static func remoteRelayMetadataCleanupScript(relayPort: Int) -> String {
        ...
        socket_addr_file="$HOME/.kshr/socket_addr"
        if [ -r "$socket_addr_file" ] && [ "$(tr -d '\\r\\n' < "$socket_addr_file")" = "$relay_socket" ]; then
          rm -f "$socket_addr_file"
        fi
        rm -f "$HOME/.kshr/relay/\(relayPort).auth" "$HOME/.kshr/relay/\(relayPort).daemon_path" "$HOME/.kshr/relay/\(relayPort).tty"
        """
    }

    private func probeRemoteBootstrapStateLocked(version: String) throws -> RemoteBootstrapState {
        let script = """
        kshr_uname_os="$(uname -s)"
        kshr_uname_arch="$(uname -m)"
        printf '%s%s\\n' '\(Self.remotePlatformProbeOSMarker)' "$kshr_uname_os"
        printf '%s%s\\n' '\(Self.remotePlatformProbeArchMarker)' "$kshr_uname_arch"
        case "$(printf '%s' "$kshr_uname_os" | tr '[:upper:]' '[:lower:]')" in
          linux|darwin|freebsd) kshr_go_os="$(printf '%s' "$kshr_uname_os" | tr '[:upper:]' '[:lower:]')" ;;
          *) exit 70 ;;
        esac
        case "$(printf '%s' "$kshr_uname_arch" | tr '[:upper:]' '[:lower:]')" in
          x86_64|amd64) kshr_go_arch=amd64 ;;
          aarch64|arm64) kshr_go_arch=arm64 ;;
          armv7l) kshr_go_arch=arm ;;
          *) exit 71 ;;
        esac
        kshr_remote_path="$HOME/.kshr/bin/kshrd-remote/\(version)/${kshr_go_os}-${kshr_go_arch}/kshrd-remote"
        if [ -x "$kshr_remote_path" ]; then
        ...
        guard let unameOS, let unameArch else {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "kshr.remote.daemon", code: 11, userInfo: [
        ...
        guard let goOS = Self.mapUnameOS(unameOS),
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 12, userInfo: [
        ...
        if result.status != 0, binaryExists == nil {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "kshr.remote.daemon", code: 13, userInfo: [
    ...
    static let remoteDaemonManifestInfoKey = "KSHRRemoteDaemonManifestJSON"
    ...
    private static func allowLocalDaemonBuildFallback(environment: [String: String] = ProcessInfo.processInfo.environment) -> Bool {
        // Always allow local build fallback for KSHR fork (no release infrastructure)
        return true
    }
    ...
    private static func explicitRemoteDaemonBinaryURL(environment: [String: String] = ProcessInfo.processInfo.environment) -> URL? {
        ...
        guard let path = environment["KSHR_REMOTE_DAEMON_BINARY"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !path.isEmpty else {
            return nil
        }
    ...
    private static func versionedRemoteDaemonBuildURL(goOS: String, goArch: String, version: String) -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("kshr-remote-daemon-build", isDirectory: true)
            .appendingPathComponent(version, isDirectory: true)
            .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
            .appendingPathComponent("kshrd-remote", isDirectory: false)
    }
    ...
    private static func fetchRemoteManifestLocked(releaseURL: String, version: String) -> WorkspaceRemoteDaemonManifest? {
        guard let manifestURL = URL(string: "\(releaseURL)/kshrd-remote-manifest.json") else { return nil }
        let request = NSMutableURLRequest(url: manifestURL)
        request.timeoutInterval = 15
        request.setValue("kshr/\(version)", forHTTPHeaderField: "User-Agent")
    ...
    private func downloadRemoteDaemonBinaryLocked(entry: WorkspaceRemoteDaemonManifest.Entry, version: String, releaseURL: String? = nil) throws -> URL {
        guard let url = URL(string: entry.downloadURL) else {
            throw NSError(domain: "kshr.remote.daemon", code: 25, userInfo: [
        ...
        session.downloadTask(with: request as URLRequest) { localURL, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse,
               !(200...299).contains(httpResponse.statusCode) {
                downloadError = NSError(domain: "kshr.remote.daemon", code: 26, userInfo: [
        ...
        guard let downloadedURL else {
            throw NSError(domain: "kshr.remote.daemon", code: 27, userInfo: [
        ...
        if downloadedSHA != entry.sha256.lowercased() {
            ...
            if let releaseURL,
               ...
               downloadedSHA == liveEntry.sha256.lowercased() {
                ...
                throw NSError(domain: "kshr.remote.daemon", code: 28, userInfo: [
    ...
    private func buildLocalDaemonBinary(goOS: String, goArch: String, version: String) throws -> URL {
        ...
        guard Self.allowLocalDaemonBuildFallback() else {
            throw NSError(domain: "kshr.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "this build does not include a verified kshrd-remote manifest for \(goOS)-\(goArch). Use a release/nightly build, or set KSHR_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 for a dev-only fallback.",
            ])
        }

        guard let repoRoot = Self.findRepoRoot() else {
            throw NSError(domain: "kshr.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "cannot locate kshr repo root for dev-only kshrd-remote build fallback",
            ])
        }
        ...
        guard FileManager.default.fileExists(atPath: goModPath) else {
            throw NSError(domain: "kshr.remote.daemon", code: 21, userInfo: [
                NSLocalizedDescriptionKey: "missing daemon module at \(goModPath)",
            ])
        }
        guard let goBinary = Self.which("go") else {
            throw NSError(domain: "kshr.remote.daemon", code: 22, userInfo: [
                NSLocalizedDescriptionKey: "go is required for the dev-only kshrd-remote build fallback",
            ])
        }
        ...
        let result = try runProcess(
            executable: goBinary,
            arguments: ["build", "-trimpath", "-buildvcs=false", "-ldflags", ldflags, "-o", output.path, "./cmd/kshrd-remote"],
            environment: env,
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 23, userInfo: [
                NSLocalizedDescriptionKey: "failed to build kshrd-remote: \(detail)",
            ])
        }
        guard FileManager.default.isExecutableFile(atPath: output.path) else {
            throw NSError(domain: "kshr.remote.daemon", code: 24, userInfo: [
                NSLocalizedDescriptionKey: "kshrd-remote build output is not executable",
            ])
        }
    ...
    private func uploadRemoteDaemonBinaryLocked(localBinary: URL, remotePath: String) throws {
        ...
        guard mkdirResult.status == 0 else {
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 30, userInfo: [
        ...
        guard scpResult.status == 0 else {
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 31, userInfo: [
                NSLocalizedDescriptionKey: "failed to upload kshrd-remote: \(detail)",
            ])
        }
        ...
        guard finalizeResult.status == 0 else {
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 32, userInfo: [
    ...
    static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/kshr-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
    ...
    private func helloRemoteDaemonLocked(remotePath: String) throws -> DaemonHello {
        ...
        guard result.status == 0 else {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "kshr.remote.daemon", code: 40, userInfo: [
        ...
        guard !responseLine.isEmpty,
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 41, userInfo: [
        ...
        if let ok = payload["ok"] as? Bool, !ok {
            ...
            throw NSError(domain: "kshr.remote.daemon", code: 42, userInfo: [
        ...
        return DaemonHello(
            name: (name?.isEmpty == false ? name! : "kshrd-remote"),
    ...
    static func remoteCLIWrapperScript() -> String {
        ...
        daemon="$HOME/.kshr/bin/kshrd-remote-current"
        socket_path="${KSHR_SOCKET_PATH:-}"
        if [ -z "$socket_path" ] && [ -r "$HOME/.kshr/socket_addr" ]; then
          socket_path="$(tr -d '\\r\\n' < "$HOME/.kshr/socket_addr")"
        fi

        if [ -n "$socket_path" ] && [ "${socket_path#/}" = "$socket_path" ] && [ "${socket_path#*:}" != "$socket_path" ]; then
          relay_port="${socket_path##*:}"
          relay_map="$HOME/.kshr/relay/${relay_port}.daemon_path"
          if [ -r "$relay_map" ]; then
            mapped_daemon="$(tr -d '\\r\\n' < "$relay_map")"
    ...
    static func remoteCLIWrapperInstallScript(daemonRemotePath: String) -> String {
        ...
        return """
        mkdir -p "$HOME/.kshr/bin" "$HOME/.kshr/relay"
        ln -sf "$HOME/\(trimmedRemotePath)" "$HOME/.kshr/bin/kshrd-remote-current"
        wrapper_tmp="$HOME/.kshr/bin/.kshr-wrapper.tmp.$$"
        cat > "$wrapper_tmp" <<'KSHRWRAPPER'
        \(remoteCLIWrapperScript())
        KSHRWRAPPER
        chmod 755 "$wrapper_tmp"
        mv -f "$wrapper_tmp" "$HOME/.kshr/bin/kshr"
        """
    }
    ...
    static func remoteRelayMetadataInstallScript(
        ...
        return """
        umask 077
        mkdir -p "$HOME/.kshr" "$HOME/.kshr/relay"
        chmod 700 "$HOME/.kshr/relay"
        \(remoteCLIWrapperInstallScript(daemonRemotePath: trimmedRemotePath))
        printf '%s' "$HOME/\(trimmedRemotePath)" > "$HOME/.kshr/relay/\(relayPort).daemon_path"
        cat > "$HOME/.kshr/relay/\(relayPort).auth" <<'KSHRRELAYAUTH'
        \(authPayload)
        KSHRRELAYAUTH
        chmod 600 "$HOME/.kshr/relay/\(relayPort).auth"
        printf '%s' '127.0.0.1:\(relayPort)' > "$HOME/.kshr/socket_addr"
        """
    }
    ...
    private static func remoteDaemonPath(version: String, goOS: String, goArch: String) -> String {
        ".kshr/bin/kshrd-remote/\(version)/\(goOS)-\(goArch)/kshrd-remote"
    }

    static func orphanedKSHRRemoteSSHPIDs(
        psOutput: String,
        destination: String,
        relayPort: Int? = nil
    ) -> [Int] {
        ...
        return psOutput
            ...
            .compactMap { line -> Int? in
                ...
                guard isOrphanedKSHRRemoteSSHCommand(
                    parsed.command,
    ...
    private static func killOrphanedRemoteSSHProcesses(destination: String, relayPort: Int? = nil) {
        ...
        for pid in orphanedKSHRRemoteSSHPIDs(
            psOutput: output,
    ...
    private static func isOrphanedKSHRRemoteSSHCommand(
        _ command: String,
        destination: String,
        relayPort: Int?
    ) -> Bool {
        ...
        if trimmed.contains("kshrd-remote") && trimmed.contains(" serve --stdio") {
            return true
        }
        return false
    }
    ...
    private static func findRepoRoot() -> URL? {
        ...
        if let envRoot = environment["KSHR_REMOTE_DAEMON_SOURCE_ROOT"],
        ...
        if let envRoot = environment["KSHRTERM_REPO_ROOT"],
           !envRoot.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            candidates.append(URL(fileURLWithPath: envRoot, isDirectory: true))
        }
    ...
    private func scanRemotePortsByPanelLocked(ttyNamesByPanel: [UUID: String]) throws -> [UUID: [Int]] {
        ...
        guard result.status == 0 else {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "kshr.remote.ports", code: 90, userInfo: [
    ...
    private func shouldUseFallbackRemotePortPollingLocked() -> Bool {
        // `kshr ssh` owns the remote shell bootstrap and can report the remote
    ...
    private func shouldUseTTYFallbackRemotePortPollingLocked() -> Bool {
        // `kshr ssh` can still land in shells without our command hooks, such as
    ...
    private static func remotePortScanScript(ttyNames: [String], excluding ports: Set<Int>) -> String {
        ...
        return """
        set -eu
        kshr_tracked_ttys=" \(ttySet) "
        kshr_tty_csv='\(ttyCSV)'
        kshr_excluded_ports=" \(excludedPorts) "

        kshr_emit_port() {
          kshr_tty="$1"
          kshr_port="$2"
          case "$kshr_tracked_ttys" in
            *" $kshr_tty "*) ;;
            *) return 0 ;;
          esac
          case "$kshr_excluded_ports" in
            *" $kshr_port "*) return 0 ;;
          esac
          [ "$kshr_port" -ge 1024 ] && [ "$kshr_port" -le 65535 ] || return 0
          printf '%s\\t%s\\n' "$kshr_tty" "$kshr_port"
        }

        kshr_used_ss=0
        if [ -d /proc ] && command -v ss >/dev/null 2>&1; then
          kshr_ss_output="$(ss -ltnpH 2>/dev/null || true)"
          case "$kshr_ss_output" in
            *pid=*)
              kshr_used_ss=1
              printf '%s\\n' "$kshr_ss_output" | while IFS= read -r kshr_line; do
                [ -n "$kshr_line" ] || continue
                kshr_port="$(printf '%s\\n' "$kshr_line" | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ { print $1; exit }')"
                [ -n "$kshr_port" ] || continue
                printf '%s\\n' "$kshr_line" | awk '
                  {
                    line = $0
                ...
                ' | while IFS= read -r kshr_pid; do
                  [ -n "$kshr_pid" ] || continue
                  kshr_tty_path="$(readlink "/proc/$kshr_pid/fd/0" 2>/dev/null || true)"
                  [ -n "$kshr_tty_path" ] || continue
                  kshr_tty="${kshr_tty_path##*/}"
                  [ -n "$kshr_tty" ] || continue
                  kshr_emit_port "$kshr_tty" "$kshr_port"
                done
              done
              ;;
          esac
        ...
        if [ "$kshr_used_ss" -eq 0 ] && command -v lsof >/dev/null 2>&1 && [ -n "$kshr_tty_csv" ]; then
          kshr_tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t kshr-ports)"
          trap 'rm -rf "$kshr_tmpdir"' EXIT INT TERM
          kshr_pid_tty_map="$kshr_tmpdir/pid_tty"
          ps -t "$kshr_tty_csv" -o pid=,tty= 2>/dev/null | awk '
            NF >= 2 {
              tty = $2
              sub(/^.*\\//, "", tty)
              print $1 "\\t" tty
          ...
          ' > "$kshr_pid_tty_map"
          [ -s "$kshr_pid_tty_map" ] || exit 0
          kshr_pid_csv="$(awk '{print $1}' "$kshr_pid_tty_map" | paste -sd, -)"
          [ -n "$kshr_pid_csv" ] || exit 0
          lsof -nP -a -p "$kshr_pid_csv" -iTCP -sTCP:LISTEN -Fpn 2>/dev/null | awk -v map="$kshr_pid_tty_map" '
            BEGIN {
              while ((getline < map) > 0) {
                pid_to_tty[$1] = $2
              }
          ...
          ' | while IFS=$'\\t' read -r kshr_tty kshr_port; do
            [ -n "$kshr_tty" ] || continue
            [ -n "$kshr_port" ] || continue
            kshr_emit_port "$kshr_tty" "$kshr_port"
          done
    ...
    private static func remoteAllPortsScanScript(excluding ports: Set<Int>) -> String {
        ...
        if command -v ss >/dev/null 2>&1; then
          ss -ltnH 2>/dev/null | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r kshr_port; do
            [ -n "$kshr_port" ] || continue
            kshr_emit_port "$kshr_port"
          done
        elif command -v netstat >/dev/null 2>&1; then
          netstat -lnt 2>/dev/null | awk 'NR > 2 {print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r kshr_port; do
            [ -n "$kshr_port" ] || continue
            kshr_emit_port "$kshr_port"
          done
        elif command -v lsof >/dev/null 2>&1; then
          lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null | awk 'NR > 1 {print $9}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r kshr_port; do
            [ -n "$kshr_port" ] || continue
            kshr_emit_port "$kshr_port"
          done
        fi
...
/// Holds high-frequency telemetry state that does NOT need to trigger Workspace's
/// objectWillChange (and therefore does not cause sidebar re-diff).
/// Mutated by socket commands on the main actor; read by detail views as needed.
@MainActor
final class WorkspaceTelemetryState {
...
final class Workspace: Identifiable, ObservableObject {
    let id: UUID
    /// High-frequency telemetry state isolated from objectWillChange to avoid sidebar invalidation.
    let telemetry = WorkspaceTelemetryState()
    /// When this workspace was created.
    let createdAt: Date
    /// Last time meaningful activity occurred (command run, notification, agent update).
    /// Not updated by mere navigation/selection.
    ...
    /// Ordinal for KSHR_PORT range assignment (monotonically increasing per app session)
    ...
    private static let sshControlMasterCleanupQueue = DispatchQueue(
        label: "com.kshr.remote-ssh.control-master-cleanup",
        qos: .utility
    )
    ...
    /// Record meaningful activity in this workspace (command execution, notification, agent update).
    /// Does NOT fire for mere navigation/selection.
    func recordActivity() {
        lastActivityAt = Date()
    }
    ...
    init(
        ...
        portOrdinal: Int = 0,
        configTemplate: KshrSurfaceConfigTemplate? = nil,
    ...
    private func browserRemoteWorkspaceStatusSnapshot() -> BrowserRemoteWorkspaceStatus? {
        ...
        return BrowserRemoteWorkspaceStatus(
            ...
            heartbeatCount: telemetry.remoteHeartbeatCount,
            lastHeartbeatAt: telemetry.remoteLastHeartbeatAt
        )
    }
    ...
    func resetSidebarContext(reason: String = "unspecified") {
        ...
        telemetry.surfaceListeningPorts.removeAll()
    ...
    func pruneSurfaceMetadata(validSurfaceIds: Set<UUID>) {
        ...
        telemetry.surfaceListeningPorts = telemetry.surfaceListeningPorts.filter { validSurfaceIds.contains($0.key) }
    ...
    func recomputeListeningPorts() {
        let unique = Set(telemetry.surfaceListeningPorts.values.flatMap { $0 })
            .union(agentListeningPorts)
            .union(telemetry.remoteDetectedPorts)
            .union(telemetry.remoteForwardedPorts)
    ...
    func remoteStatusPayload() -> [String: Any] {
        let heartbeatAgeSeconds: Any = {
            guard let last = telemetry.remoteLastHeartbeatAt else { return NSNull() }
        ...
        var payload: [String: Any] = [
            ...
            "daemon": telemetry.remoteDaemonStatus.payload(),
            "detected_ports": telemetry.remoteDetectedPorts,
            "forwarded_ports": telemetry.remoteForwardedPorts,
            "conflicted_ports": telemetry.remotePortConflicts,
            "detail": remoteConnectionDetail ?? NSNull(),
            "heartbeat": [
                "count": telemetry.remoteHeartbeatCount,
                "last_seen_at": heartbeatTimestamp,
                "age_seconds": heartbeatAgeSeconds,
            ],
        ]
        if let endpoint = telemetry.remoteProxyEndpoint {
            payload["proxy"] = [
                "state": "ready",
                "host": endpoint.host,
                "port": endpoint.port,
    ...
    func configureRemoteConnection(_ configuration: WorkspaceRemoteConfiguration, autoConnect: Bool = true) {
        ...
        telemetry.remoteDetectedPorts = []
        telemetry.remoteForwardedPorts = []
        telemetry.remotePortConflicts = []
        telemetry.remoteProxyEndpoint = nil
        telemetry.remoteHeartbeatCount = 0
        telemetry.remoteLastHeartbeatAt = nil
        remoteConnectionDetail = nil
        telemetry.remoteDaemonStatus = WorkspaceRemoteDaemonStatus()
    ...
    private func maybeDemoteRemoteWorkspaceAfterSSHSessionEnded() {
        ...
        if !hasBrowserPanels {
            if remoteConnectionState == .error || telemetry.remoteDaemonStatus.state == .error || remoteConnectionState == .connecting {
                return
            }
            disconnectRemoteConnection(clearConfiguration: true)
        }
    ...
    fileprivate func applyRemoteDaemonStatusUpdate(_ status: WorkspaceRemoteDaemonStatus, target: String) {
        telemetry.remoteDaemonStatus = status
        applyBrowserRemoteWorkspaceStatusToPanels()
    ...
    fileprivate func applyRemoteProxyEndpointUpdate(_ endpoint: BrowserProxyEndpoint?) {
        telemetry.remoteProxyEndpoint = endpoint
        for panel in panels.values {
            guard let browserPanel = panel as? BrowserPanel else { continue }
    ...
    fileprivate func applyRemoteHeartbeatUpdate(count: Int, lastSeenAt: Date?) {
        telemetry.remoteHeartbeatCount = max(0, count)
        telemetry.remoteLastHeartbeatAt = lastSeenAt
        applyBrowserRemoteWorkspaceStatusToPanels()
    }
    ...
    fileprivate func applyRemoteDetectedSurfacePortsSnapshot(
        ...
        for panelId in remoteDetectedSurfaceIds.subtracting(trackedSurfaceIds) {
            telemetry.surfaceListeningPorts.removeValue(forKey: panelId)
        }
        ...
        for (panelId, ports) in detectedByPanel {
            if ports.isEmpty {
                ...
                telemetry.surfaceListeningPorts[panelId] = ports
            }
        }

        telemetry.remoteDetectedPorts = detected
        telemetry.remoteForwardedPorts = forwarded
        telemetry.remotePortConflicts = conflicts
        recordActivity()
        recomputeListeningPorts()
    ...
    private func seedTerminalInheritanceFontPoints(
        panelId: UUID,
        configTemplate: KshrSurfaceConfigTemplate?
    ) {
    ...
    private func resolvedTerminalInheritanceFontPoints(
        ...
        sourceSurface: ghostty_surface_t,
        inheritedConfig: KshrSurfaceConfigTemplate
    ) -> Float? {
        let runtimePoints = kshrCurrentSurfaceFontSizePoints(sourceSurface)
    ...
    private func inheritedTerminalConfig(
        ...
        for terminalPanel in terminalPanelConfigInheritanceCandidates(
            ...
            // ghostty_surface_inherited_config or kshrCurrentSurfaceFontSizePoints
            // is still reading through the pointer.
            let surface = terminalPanel.surface
            guard let sourceSurface = surface.surface else { continue }
            var config = kshrInheritedSurfaceConfig(
        ...
        if let fallbackFontPoints = lastTerminalConfigInheritanceFontPoints {
            var config = KshrSurfaceConfigTemplate()
    ...
    func closePanel(_ panelId: UUID, force: Bool = false) -> Bool {
        ...
        let firstResponderPanelId = kshrOwningGhosttyView(
    ...
    private func browserPortalReady(for browserPanel: BrowserPanel) -> Bool {
        guard let wv = browserPanel.webView else { return false }
        return browserPortalAnchorReady(for: browserPanel) &&
            wv.window != nil &&
            wv.superview != nil &&
            BrowserWindowPortalRegistry.isWebView(wv, boundTo: browserPanel.portalAnchorView)
    }
    ...
    private func attemptEventDrivenLayoutFollowUp() {
        ...
        if let browserPanelId = layoutFollowUpBrowserPanelId {
            if let browserPanel = browserPanel(for: browserPanelId) {
                ...
                if isReady, let wv = browserPanel.webView,
                   (!wasReady || BrowserWindowPortalRegistry.debugSnapshot(for: wv)?.containerHidden == true) {
    ...
    private func reconcileTerminalPortalVisibilityForCurrentRenderedLayout() -> Bool {
        ...
        for panel in panels.values {
            ...
            // If the terminal should be visible but the portal view is still hidden or
            // detached, force a portal synchronization + refresh. This mirrors the browser
            // portal reconciliation path and recovers from stale portal state that can
            // accumulate during workspace handoff races or transient geometry issues.
            if shouldBeVisible {
                let hostedView = terminalPanel.hostedView
                let portalStale =
                    hostedView.isHidden ||
                ...
                if portalStale {
                    hostedView.reconcileGeometryNow()
                    hostedView.refreshSurfaceNow(reason: "reconcile.terminalPortalRecovery")
                    hostedView.retryFocusAfterPortalDisplayActivation(reason: "reconcile.terminalPortalRecovery")
                    didChange = true
                }
            }
        }
    ...
    private func reconcileBrowserPortalVisibilityForCurrentRenderedLayout(reason: String) -> Bool {
        ...
        for panel in panels.values {
            guard let browserPanel = panel as? BrowserPanel else { continue }
            guard let webView = browserPanel.webView else { continue }
    ...
    private func browserPortalVisibilityNeedsFollowUp() -> Bool {
        ...
        for panel in panels.values {
            ...
            let anchorReady =
                ...
                anchorView.bounds.width > 1 &&
                anchorView.bounds.height > 1
            guard let webView = browserPanel.webView else { return true }
            if !anchorReady ||
                webView.window == nil ||
                webView.superview == nil ||
                !BrowserWindowPortalRegistry.isWebView(webView, boundTo: anchorView) {
                return true
            }
        }
    ...
    private func promptMovePanel(tabId: TabID) {
        ...
        if !moved {
            ...
            failure.informativeText = String(localized: "alert.moveTab.failed.message", defaultValue: "kshr could not move this tab to the selected destination.")
...
extension Workspace: BonsplitDelegate {
    ...
    func splitTabBar(_ controller: BonsplitController, didSplitPane originalPane: PaneID, newPane: PaneID, orientation: SplitOrientation) {
        ...
        // tab in the source pane to avoid leaving it tabless. In kshr, this is undesirable:

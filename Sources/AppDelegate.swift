...
private enum KshrThemeNotifications {
    static let reloadConfig = Notification.Name("com.kshrterm.themes.reload-config")
}
...
#if DEBUG
enum KshrTypingTiming {
    static let isEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["KSHR_TYPING_TIMING_LOGS"] == "1" || environment["KSHR_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        let defaults = UserDefaults.standard
        return defaults.bool(forKey: "kshrTypingTimingLogs") || defaults.bool(forKey: "kshrKeyLatencyProbe")
    }()
    static let isVerboseProbeEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["KSHR_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        return UserDefaults.standard.bool(forKey: "kshrKeyLatencyProbe")
    }()
    ...
    static func logDuration(path: String, startedAt: TimeInterval?, event: NSEvent? = nil, extra: String? = nil) {
        KshrMainThreadTurnProfiler.endMeasure(path, startedAt: startedAt)
...
final class KshrMainRunLoopStallMonitor {
    static let shared = KshrMainRunLoopStallMonitor()
    ...
    func installIfNeeded() {
        guard KshrTypingTiming.isEnabled else { return }
        ...
        observer = CFRunLoopObserverCreate(
            ...
            { _, activity, info in
                ...
                let monitor = Unmanaged<KshrMainRunLoopStallMonitor>.fromOpaque(info).takeUnretainedValue()
...
final class KshrMainThreadTurnProfiler {
    static let shared = KshrMainThreadTurnProfiler()
    ...
    static func endMeasure(_ bucket: String, startedAt: TimeInterval?) {
        guard let startedAt, KshrTypingTiming.isEnabled, Thread.isMainThread else { return }
        let elapsedMs = max(0, (ProcessInfo.processInfo.systemUptime - startedAt) * 1000.0)
    ...
    func installIfNeeded() {
        ...
        observer = CFRunLoopObserverCreate(
            ...
            { _, activity, info in
                guard let info else { return }
                let profiler = Unmanaged<KshrMainThreadTurnProfiler>.fromOpaque(info).takeUnretainedValue()
...
final class VSCodeServeWebController {
    ...
    private let queue = DispatchQueue(label: "kshr.vscode.serveWeb")
    private let launchQueue = DispatchQueue(label: "kshr.vscode.serveWeb.launch")
    ...
    private static func makeConnectionTokenFile() -> URL? {
        ...
        let tokenFileName = "kshr-vscode-token-\(UUID().uuidString)"
...
struct KshrCLIPathInstaller {
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundledCLIMissing(let expectedPath):
                return "Bundled kshr CLI was not found at \(expectedPath)."
            ...
            case .installVerificationFailed(let path):
                return "Installed symlink at \(path) did not point to the bundled kshr CLI."
    ...
    init(
        ...
        destinationURL: URL = URL(fileURLWithPath: "/usr/local/bin/kshr"),
        bundledCLIURLProvider: @escaping () -> URL? = {
            KshrCLIPathInstaller.defaultBundledCLIURL()
        },
        expectedBundledCLIPath: String = KshrCLIPathInstaller.defaultBundledCLIExpectedPath(),
    ...
    private static func defaultBundledCLIURL(bundle: Bundle = .main) -> URL? {
        bundle.resourceURL?.appendingPathComponent("bin/kshr", isDirectory: false)
    }

    private static func defaultBundledCLIExpectedPath(bundle: Bundle = .main) -> String {
        bundle.bundleURL
            .appendingPathComponent("Contents/Resources/bin/kshr", isDirectory: false)
            .path
    }
...
private enum BrowserFindCommandEquivalent {
    ...
    var keepsKshrBrowserFindBarOwnershipWhenVisible: Bool {
        switch self {
...
private func kshrIsLikelyWebInspectorResponder(_ responder: NSResponder?) -> Bool {
    guard let responder else { return false }
...
/// For browser content, let the page try the Find command family before kshr's menu fallback.
/// This preserves native web-app shortcuts like VS Code's Cmd+F while still allowing kshr's
...
func shouldRouteBrowserFindCommandEquivalentThroughWebContentFirst(
    ...
    owningWebView: KshrWebView? = nil
) -> Bool {
    guard let shortcut = browserFindCommandEquivalent(for: event) else {
        return false
    }

    if kshrIsLikelyWebInspectorResponder(responder) {
        return false
    }

    if shortcut.keepsKshrBrowserFindBarOwnershipWhenVisible,
       let owningWebView {
...
func kshrOwningGhosttyView(for responder: NSResponder?) -> GhosttyNSView? {
    ...
    if let view = responder as? NSView,
       let ghosttyView = kshrOwningGhosttyView(for: view) {
        return ghosttyView
    }

    if let textView = responder as? NSTextView,
       let delegateView = textView.delegate as? NSView,
       let ghosttyView = kshrOwningGhosttyView(for: delegateView) {
        return ghosttyView
    }
...
private func kshrOwningGhosttyView(for view: NSView) -> GhosttyNSView? {
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private static func detectRunningUnderXCTest(_ env: [String: String]) -> Bool {
        ...
        if env.keys.contains(where: { $0.hasPrefix("KSHR_UI_TEST_") }) { return true }
        return false
    }
    ...
    private nonisolated static let persistedWindowGeometryDefaultsKey = "kshr.session.lastWindowGeometry.v2"
    private nonisolated static let legacyPersistedWindowGeometryDefaultsKeys = [
        "kshr.session.lastWindowGeometry.v1"
    ]
    ...
    private var isBackgroundWorkSuspended = false
    ...
    private static let didInstallWindowKeyEquivalentSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.kshr_performKeyEquivalent(with:))
    ...
    private static let didInstallWindowFirstResponderSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.kshr_makeFirstResponder(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallWindowSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.kshr_sendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.kshr_applicationSendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private func childExitKeyboardProbePath() -> String? {
        ...
        guard env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
              let path = env["KSHR_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private let sessionPersistenceQueue = DispatchQueue(
        label: "com.kshrterm.app.sessionPersistence",
        qos: .utility
    )
    private nonisolated static let launchServicesRegistrationQueue = DispatchQueue(
        label: "com.kshrterm.app.launchServicesRegistration",
        qos: .utility
    )
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: KshrThemeNotifications.reloadConfig,
            object: nil,
        ...
        KshrMainRunLoopStallMonitor.shared.installIfNeeded()
        KshrMainThreadTurnProfiler.shared.installIfNeeded()
        ...
        let forceDuplicateLaunchObserver = env["KSHR_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] == "1"
        ...
        QuickTerminalController.shared.installGlobalHotkey()
        NSApp.servicesProvider = self
#if DEBUG
        UpdateTestSupport.applyIfNeeded(to: updateController.viewModel)
        if env["KSHR_UI_TEST_MODE"] == "1" {
            let trigger = env["KSHR_UI_TEST_TRIGGER_UPDATE_CHECK"] ?? "<nil>"
            let feed = env["KSHR_UI_TEST_FEED_URL"] ?? "<nil>"
            UpdateLogStore.shared.append("ui test env: trigger=\(trigger) feed=\(feed)")
        }
        if env["KSHR_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" {
        ...
        if isRunningUnderXCTest {
            if let rawVariant = env["KSHR_UI_TEST_BROWSER_IMPORT_HINT_VARIANT"] {
                UserDefaults.standard.set(
                    BrowserImportHintSettings.variant(for: rawVariant).rawValue,
                    forKey: BrowserImportHintSettings.variantKey
                )
            }
            if let rawShow = env["KSHR_UI_TEST_BROWSER_IMPORT_HINT_SHOW"] {
                UserDefaults.standard.set(
                    rawShow == "1",
                    forKey: BrowserImportHintSettings.showOnBlankTabsKey
                )
            }
            if let rawDismissed = env["KSHR_UI_TEST_BROWSER_IMPORT_HINT_DISMISSED"] {
                UserDefaults.standard.set(
                    rawDismissed == "1",
            ...
            if env["KSHR_UI_TEST_BROWSER_IMPORT_HINT_OPEN_BLANK_BROWSER"] == "1" {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { [weak self] in
                    guard let self else { return }
                    _ = self.openBrowserAndFocusAddressBar(insertAtEnd: true)
                }
            }
            if env["KSHR_UI_TEST_BROWSER_IMPORT_HINT_OPEN_SETTINGS"] == "1" {
            ...
            if env["KSHR_UI_TEST_BROWSER_IMPORT_AUTO_OPEN"] == "1" {
    ...
    private func writeUITestDiagnosticsIfNeeded(stage: String) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["KSHR_UI_TEST_DIAGNOSTICS_PATH"], !path.isEmpty else { return }
        ...
        let screenIDs = windows.map { $0.screen?.kshrDisplayID.map(String.init) ?? "" }.joined(separator: ",")
        let targetDisplayID = env["KSHR_UI_TEST_TARGET_DISPLAY_ID"] ?? ""
        ...
        if let rawDisplayID = UInt32(targetDisplayID) {
            let screenPresent = NSScreen.screens.contains(where: { $0.kshrDisplayID == rawDisplayID })
            let movedWindow = windows.contains(where: { $0.screen?.kshrDisplayID == rawDisplayID })
    ...
    private func appendUITestSocketDiagnosticsIfNeeded(
        ...
        guard env["KSHR_UI_TEST_SOCKET_SANITY"] == "1" else { return }

        guard let config = socketListenerConfigurationIfEnabled() else {
            payload["socketExpectedPath"] = env["KSHR_SOCKET_PATH"] ?? ""
    ...
    private func appendUITestRenderDiagnosticsIfNeeded(
        ...
        guard env["KSHR_UI_TEST_DISPLAY_RENDER_STATS"] == "1" else { return }
    ...
    private func currentUITestRenderDiagnostics() -> UITestRenderDiagnosticsSnapshot? {
        guard let tabManager,
              ...
              let workspace = tabManager.tabsById[tabId] else {
            return nil
        }
    ...
    private func moveUITestWindowToTargetDisplayIfNeeded(attempt: Int = 0) {
        let env = ProcessInfo.processInfo.environment
        guard let rawDisplayID = env["KSHR_UI_TEST_TARGET_DISPLAY_ID"],
              let targetDisplayID = UInt32(rawDisplayID) else {
            return
        }

        guard let screen = NSScreen.screens.first(where: { $0.kshrDisplayID == targetDisplayID }) else {
            if attempt < 20 {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
        ...
        window.orderFrontRegardless()
        if window.screen?.kshrDisplayID != targetDisplayID, attempt < 20 {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
                self?.moveUITestWindowToTargetDisplayIfNeeded(attempt: attempt + 1)
            }
            return
    ...
    func applicationDidBecomeActive(_ notification: Notification) {
        ...
        // App activation means at least one window is coming to the foreground.
        ...
        if let surfaceId,
           let tab = tabManager.tabsById[tabId] {
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        DispatchQueue.main.async {
            ...
            alert.messageText = String(localized: "dialog.quitKshr.title", defaultValue: "Quit kshr?")
            alert.informativeText = String(localized: "dialog.quitKshr.message", defaultValue: "This will close all windows and workspaces.")
            alert.addButton(withTitle: String(localized: "dialog.quitKshr.quit", defaultValue: "Quit"))
    ...
    // MARK: - Background Work Suspend/Resume

    /// Install observers for window occlusion state changes so we can suspend
    /// heavyweight background work (port scanning, git polling, agent PID sweeps)
    /// when all windows are hidden, miniaturized, or fully occluded.
    func installWindowOcclusionObservers() {
        let nc = NotificationCenter.default
        windowOcclusionObservers.append(nc.addObserver(
            forName: NSWindow.didChangeOcclusionStateNotification,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            self?.reevaluateBackgroundWorkSuspension()
        })
        windowOcclusionObservers.append(nc.addObserver(
    ...
    /// Check whether any app window is visible and not fully occluded.
    /// If none are, suspend background work; otherwise resume.
    private func reevaluateBackgroundWorkSuspension() {
        // If the app is hidden (Cmd+H), applicationDidHide already suspended.
        guard !NSApp.isHidden else { return }

        let hasVisibleWindow = NSApp.windows.contains { window in
            // Skip panels and utility windows — only count main document windows.
    ...
    private func suspendBackgroundWorkIfNeeded() {
        guard !isBackgroundWorkSuspended else { return }
        isBackgroundWorkSuspended = true
        tabManager?.suspendBackgroundWork()
    }

    private func resumeBackgroundWorkIfNeeded() {
        guard isBackgroundWorkSuspended else { return }
        isBackgroundWorkSuspended = false
        tabManager?.resumeBackgroundWork()
    }
    ...
    func configure(tabManager: TabManager, notificationStore: TerminalNotificationStore, sidebarState: SidebarState) {
        ...
        installWindowOcclusionObservers()
    ...
    private func currentDisplayGeometries() -> (
        ...
        let available = NSScreen.screens.map { screen in
            SessionDisplayGeometry(
                displayID: screen.kshrDisplayID,
                frame: screen.frame,
...
#if DEBUG
        let timingStart = KshrTypingTiming.start()
        defer {
            KshrTypingTiming.logDuration(
    ...
    func moveSurface(
        ...
        guard let sourceWorkspace = source.tabManager.tabsById[source.workspaceId] else {
#if DEBUG
    ...
    func workspaceContainingPanel(
        ...
        if let located = locateSurface(surfaceId: panelId),
           let workspace = located.tabManager.tabsById[located.workspaceId],
           workspace.panels[panelId] != nil {
    ...
    private func windowForMainWindowId(_ windowId: UUID) -> NSWindow? {
        ...
        let expectedIdentifier = "kshr.main.\(windowId.uuidString)"
    ...
    private func mainWindowId(for window: NSWindow) -> UUID? {
        ...
        guard let rawIdentifier = window.identifier?.rawValue,
              rawIdentifier.hasPrefix("kshr.main.") else { return nil }
        let idPart = String(rawIdentifier.dropFirst("kshr.main.".count))
    ...
    private func focusedTerminalShortcutContext(preferredWindow: NSWindow? = nil) -> FocusedTerminalShortcutContext? {
        ...
        guard let ghosttyView = kshrOwningGhosttyView(for: responder),
    ...
    private func preferredMainWindowContextForShortcutRouting(event: NSEvent) -> MainWindowContext? {
        ...
        if shortcutEventHasAddressableWindow(event) {
            if let eventWindow = resolvedShortcutEventWindow(event),
               kshrWindowShouldOwnCloseShortcut(eventWindow) {
                // Auxiliary kshr windows do not own a terminal tab manager. Let them fall back
    ...
    func createMainWindow(
        ...
        kshrConfigStore.wireDirectoryTracking(tabManager: tabManager)
    ...
    func sendWelcomeCommandWhenReady(to workspace: Workspace, markShownOnSend: Bool = false) {
        sendTextWhenReady("kshr welcome\n", to: workspace) {
            if markShownOnSend {
    ...
    func isKshrCLIInstalledInPATH() -> Bool {
        KshrCLIPathInstaller().isInstalled()
    }

    @objc func installKshrCLIInPath(_ sender: Any?) {
        let installer = KshrCLIPathInstaller()
        do {
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.installed", defaultValue: "kshr CLI Installed"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.installFailed", defaultValue: "Couldn't Install kshr CLI"),
    ...
    @objc func uninstallKshrCLIInPath(_ sender: Any?) {
        let installer = KshrCLIPathInstaller()
        do {
            ...
            let prefix = outcome.removedExistingEntry
                ...
                : String(localized: "cli.uninstall.notFound", defaultValue: "No kshr CLI symlink was found at \(outcome.destinationURL.path).")
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.uninstalled", defaultValue: "kshr CLI Uninstalled"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.uninstallFailed", defaultValue: "Couldn't Uninstall kshr CLI"),
                informativeText: error.localizedDescription,
                style: .warning
            )
        }
    ...
    private func setupJumpUnreadUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_JUMP_UNREAD_SETUP"] == "1" else { return }
        guard let notificationStore else { return }
    ...
    func armJumpUnreadFocusRecord(tabId: UUID, surfaceId: UUID) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["KSHR_UI_TEST_JUMP_UNREAD_PATH"], !path.isEmpty else { return }
    ...
    private func setupGotoSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_GOTO_SPLIT_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }

        let useGhosttyConfig = env["KSHR_UI_TEST_GOTO_SPLIT_USE_GHOSTTY_CONFIG"] == "1"
        ...
        func runSetupWhenWindowReady() {
            ...
            let requestedBrowserURL = env["KSHR_UI_TEST_GOTO_SPLIT_BROWSER_URL"]?
                .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func setupBonsplitTabDragUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }
        let startWithHiddenSidebar = env["KSHR_UI_TEST_BONSPLIT_START_WITH_HIDDEN_SIDEBAR"] == "1"
    ...
    private func bonsplitTabDragUITestDataPath() -> String? {
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1",
              let path = env["KSHR_UI_TEST_BONSPLIT_TAB_DRAG_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private func isGotoSplitUITestRecordingEnabled() -> Bool {
        let env = ProcessInfo.processInfo.environment
        return env["KSHR_UI_TEST_GOTO_SPLIT_SETUP"] == "1" || env["KSHR_UI_TEST_GOTO_SPLIT_RECORD_ONLY"] == "1"
    }

    private func gotoSplitUITestDataPath() -> String? {
        guard isGotoSplitUITestRecordingEnabled() else { return nil }
        let env = ProcessInfo.processInfo.environment
        guard let path = env["KSHR_UI_TEST_GOTO_SPLIT_PATH"], !path.isEmpty else { return nil }
        return path
    }
    ...
    private func focusWebViewForGotoSplitUITest(tab: Workspace, browserPanelId: UUID) {
        ...
        func recordFocusedState() {
            ...
            if ProcessInfo.processInfo.environment["KSHR_UI_TEST_GOTO_SPLIT_INPUT_SETUP"] == "1" {
    ...
    private func isWebViewFocused(_ panel: BrowserPanel) -> Bool {
        guard let webView = panel.webView,
    ...
    private func setupFocusedInputForGotoSplitUITest(panel: BrowserPanel) {
        ...
        (() => {
          const snapshot = () => {
            ...
            return {
              ...
              trackerInstalled: window.__kshrAddressBarFocusTrackerInstalled === true,
              trackedStateId:
                window.__kshrAddressBarFocusState &&
                typeof window.__kshrAddressBarFocusState.id === "string"
                  ? window.__kshrAddressBarFocusState.id
                  : "",
          ...
          const seed = () => {
            ...
            let container = document.getElementById("kshr-ui-test-focus-container");
            if (!container || !container.tagName || container.tagName.toLowerCase() !== "div") {
              container = document.createElement("div");
              container.id = "kshr-ui-test-focus-container";
            ...
            const input = ensureInput("kshr-ui-test-focus-input", "kshr-ui-focus-primary");
            const secondaryInput = ensureInput("kshr-ui-test-focus-input-secondary", "kshr-ui-focus-secondary");
            ...
            let trackedFocusId = input.getAttribute("data-kshr-addressbar-focus-id");
            if (!trackedFocusId) {
              trackedFocusId = "kshr-ui-test-focus-input-tracked";
              input.setAttribute("data-kshr-addressbar-focus-id", trackedFocusId);
            }
            ...
            if (
              !window.__kshrAddressBarFocusState ||
              typeof window.__kshrAddressBarFocusState.id !== "string" ||
              window.__kshrAddressBarFocusState.id !== trackedFocusId
            ) {
              window.__kshrAddressBarFocusState = { id: trackedFocusId, selectionStart, selectionEnd };
            }
          ...
          const ready = () =>
            window.__kshrAddressBarFocusTrackerInstalled === true &&
        ...
        panel.webView?.evaluateJavaScript(script) { [weak self] result, _ in
            ...
            if let wv = panel.webView, let window = wv.window {
                let webFrame = wv.convert(wv.bounds, to: nil)
            ...
            try {
              ...
              if (!active) {
                return {
                  ...
                  editable: "false",
                  trackedFocusStateId: "",
                  focusTrackerInstalled: window.__kshrAddressBarFocusTrackerInstalled === true ? "true" : "false"
                };
              }
              ...
              return {
                ...
                focusTrackerInstalled:
                  window.__kshrAddressBarFocusTrackerInstalled === true ? "true" : "false"
              };
            } catch (_) {
              return {
                id: "",
    ...
    private func recordGotoSplitZoomIfNeeded() {
        ...
        func snapshot(for workspace: Workspace) -> ([String: String], Bool) {
            ...
            let browserSnapshot = browserPanel.flatMap { bp -> BrowserWindowPortalRegistry.DebugSnapshot? in
                guard let wv = bp.webView else { return nil }
                return BrowserWindowPortalRegistry.debugSnapshot(for: wv)
            }
    ...
    private func setupMultiWindowNotificationsUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] == "1" else { return }
        guard let path = env["KSHR_UI_TEST_MULTI_WINDOW_NOTIF_PATH"], !path.isEmpty else { return }
        ...
        func waitForSurfaceId(
            ...
            func resolvedSurfaceId() -> UUID? {
                ...
                guard let workspace = tabManager.tabsById[tabId] else {
                    return nil
                }
            ...
            func attemptResolve() {
                guard !resolved else { return }
                if let workspace = tabManager.tabsById[tabId],
    ...
    private func prepareMultiWindowNotificationSourceTerminalIfNeeded(
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] == "1" else { return }
        ...
        func attemptFocus() {
            ...
            if let tab = tabManager.tabsById[tabId] {
    ...
    private func publishMultiWindowNotificationSocketStateIfNeeded(at path: String) {
        ...
        guard let config = socketListenerConfigurationIfEnabled() else {
            writeMultiWindowNotificationTestData([
                "socketExpectedPath": env["KSHR_SOCKET_PATH"] ?? "",
                "socketMode": "off",
                "socketReady": "0",
                "socketPingResponse": "",
                "socketIsRunning": "0",
    ...
    func jumpToLatestUnread() {
        guard let notificationStore else { return }
#if DEBUG
        if ProcessInfo.processInfo.environment["KSHR_UI_TEST_JUMP_UNREAD_SETUP"] == "1" {
            writeJumpUnreadTestData([
                "jumpUnreadInvoked": "1",
    ...
    static func setWindowFirstResponderGuardTesting(currentEvent: NSEvent?, hitView: NSView?) {
        kshrFirstResponderGuardCurrentEventOverride = currentEvent
        kshrFirstResponderGuardHitViewOverride = hitView
    }

    static func clearWindowFirstResponderGuardTesting() {
        kshrFirstResponderGuardCurrentEventOverride = nil
        kshrFirstResponderGuardHitViewOverride = nil
    }
#endif
    ...
    private func installShortcutMonitor() {
        ...
        shortcutMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .keyUp, .flagsChanged]) { [weak self] event in
            ...
            if event.type == .keyDown {
                ...
                KshrTypingTiming.logEventDelay(path: "appMonitor", event: event)
                let shortcutMonitorTraceEnabled =
                    ProcessInfo.processInfo.environment["KSHR_SHORTCUT_MONITOR_TRACE"] == "1"
                    || UserDefaults.standard.bool(forKey: "kshrShortcutMonitorTrace")
                ...
                let shortcutTimingStart = KshrTypingTiming.start()
#endif
    ...
    func promptRenameSelectedWorkspace() -> Bool {
        guard let tabManager,
              let tabId = tabManager.selectedTabId,
              let tab = tabManager.tabsById[tabId] else {
            NSSound.beep()
            return false
        }
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        if browserAddressBarFocusedPanelId != nil,
           kshrOwningGhosttyView(for: NSApp.keyWindow?.firstResponder) != nil {
#if DEBUG
        ...
        if !normalizedFlags.contains(.command),
           let ghosttyView = kshrOwningGhosttyView(for: NSApp.keyWindow?.firstResponder),
        ...
        if matchConfiguredShortcut(event: event, action: .closeOtherTabsInPane) {
            if let targetWindow = event.window ?? NSApp.keyWindow ?? NSApp.mainWindow,
               targetWindow.identifier?.rawValue == "kshr.settings" {
        ...
        if matchConfiguredShortcut(event: event, action: .closeTab) {
            ...
            if let targetWindow = [targetWindow, NSApp.keyWindow]
                .compactMap({ $0 })
                .first(where: { $0.identifier?.rawValue == "kshr.browser-popup" }) {
#if DEBUG
               ...
               kshrWindowShouldOwnCloseShortcut(targetWindow) {
                targetWindow.performClose(nil)
            } else {
    ...
    private func isLikelyWebInspectorResponder(_ responder: NSResponder?) -> Bool {
        kshrIsLikelyWebInspectorResponder(responder)
    }
    ...
    fileprivate func browserFindBarIsVisible(for webView: KshrWebView) -> Bool {
    ...
    private func shouldLetFocusedBrowserOwnFindShortcut(_ event: NSEvent) -> Bool {
        ...
        let owningWebView = tabManager?.focusedBrowserPanel?.webView as? KshrWebView
    ...
    private func browserPanelOwning(_ webView: KshrWebView) -> BrowserPanel? {
    ...
    private func browserPanelOwning(_ webView: KshrWebView, in manager: TabManager) -> BrowserPanel? {
        for workspace in manager.tabs {
    ...
    private func workspaceForMainActor(tabId: UUID) -> Workspace? {
        contextContainingTabId(tabId)?.tabManager.tabsById[tabId]
    }
    ...
    func closeMainWindowContainingTabId(_ tabId: UUID) {
        ...
        let expectedIdentifier = "kshr.main.\(context.windowId.uuidString)"
...
#if DEBUG
        let isJumpUnreadUITest = ProcessInfo.processInfo.environment["KSHR_UI_TEST_JUMP_UNREAD_SETUP"] == "1"
    ...
    func tabTitle(for tabId: UUID) -> String? {
        if let context = contextContainingTabId(tabId) {
            return context.tabManager.tabsById[tabId]?.title
        }
...
final class MenuBarExtraController: NSObject, NSMenuDelegate {
    ...
    private let menu = NSMenu(title: "kshr")
    ...
    private let quitItem = NSMenuItem(title: String(localized: "menu.quitKshr", defaultValue: "Quit kshr"), action: nil, keyEquivalent: "")
        ...
        if let button = statusItem.button {
            ...
            button.toolTip = displayedUnreadCount == 0
                ...
                : displayedUnreadCount == 1
                    ? "kshr: " + String(localized: "statusMenu.tooltip.unread.one", defaultValue: "1 unread notification")
                    : "kshr: " + String(localized: "statusMenu.tooltip.unread.other", defaultValue: "\(displayedUnreadCount) unread notifications")
        }
    }
...
enum MenuBarIconRenderer {
    ...
    private static func drawGlyph(in rect: NSRect) {
        // Match the canonical kshr center-mark path from Icon Center Image Artwork.svg.
...
#if DEBUG
private var kshrFirstResponderGuardCurrentEventOverride: NSEvent?
private var kshrFirstResponderGuardHitViewOverride: NSView?
#endif
private var kshrFirstResponderGuardCurrentEventContext: NSEvent?
private var kshrFirstResponderGuardHitViewContext: NSView?
private var kshrFirstResponderGuardContextWindowNumber: Int?
private var kshrBrowserReturnForwardingDepth = 0
private var kshrWindowFirstResponderBypassDepth = 0
private var kshrFieldEditorOwningWebViewAssociationKey: UInt8 = 0

@discardableResult
func kshrWithWindowFirstResponderBypass<T>(_ body: () -> T) -> T {
    kshrWindowFirstResponderBypassDepth += 1
    defer {
        kshrWindowFirstResponderBypassDepth = max(0, kshrWindowFirstResponderBypassDepth - 1)
    }
    return body()
}

func kshrIsWindowFirstResponderBypassActive() -> Bool {
    kshrWindowFirstResponderBypassDepth > 0
}

private final class KshrFieldEditorOwningWebViewBox: NSObject {
    weak var webView: KshrWebView?

    init(webView: KshrWebView?) {
        self.webView = webView
    }
}

private extension NSApplication {
    @objc func kshr_applicationSendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? KshrTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        if event.type == .keyDown {
            KshrTypingTiming.logEventDelay(path: "app.sendEvent", event: event)
        }
        defer {
            if event.type == .keyDown {
                let totalMs = (ProcessInfo.processInfo.systemUptime - phaseTotalStart) * 1000.0
...
#endif
        kshr_applicationSendEvent(event)
    }
}
...
private extension AppDelegate {
    @objc func handleThemesReloadNotification(_ notification: Notification) {
        DispatchQueue.main.async {
            GhosttyApp.shared.reloadConfiguration(source: "distributed.kshr.themes")
        }
    }
}

private extension NSWindow {
    @objc func kshr_makeFirstResponder(_ responder: NSResponder?) -> Bool {
        if kshrIsWindowFirstResponderBypassActive() {
#if DEBUG
            dlog(
        ...
        let currentEvent = Self.kshrCurrentEvent(for: self)
        let responderWebView = responder.flatMap {
            Self.kshrOwningWebView(for: $0, in: self, event: currentEvent)
        }
        ...
        if let responder,
           ...
           !webView.allowsFirstResponderAcquisitionEffective {
            let pointerInitiatedFocus = Self.kshrShouldAllowPointerInitiatedWebViewFocus(
                window: self,
                webView: webView,
                event: currentEvent
            )
        ...
        if pointerInitiatedWebFocus, let webView = responderWebView {
            // `NSWindow.makeFirstResponder` may run before `KshrWebView.mouseDown(with:)`.
            // Preserve pointer intent during this synchronous responder change.
            result = webView.withPointerFocusAllowance {
                kshr_makeFirstResponder(responder)
            }
        } else {
            result = kshr_makeFirstResponder(responder)
        }
        if result {
            if let fieldEditor = responder as? NSTextView, fieldEditor.isFieldEditor {
                Self.kshrTrackFieldEditor(fieldEditor, owningWebView: responderWebView)
    ...
    @objc func kshr_sendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? KshrTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        ...
        let typingTimingExtra: String? = {
            ...
            let responderWebView = self.firstResponder.flatMap {
                Self.kshrOwningWebView(for: $0, in: self, event: event)
            }
            let hitWebView = Self.kshrHitViewForEventDispatch(in: self, event: event).flatMap {
                Self.kshrOwningWebView(for: $0)
            }
        ...
        if event.type == .keyDown {
            KshrTypingTiming.logEventDelay(path: "window.sendEvent", event: event)
        }
#endif
...
#endif
        let previousContextEvent = kshrFirstResponderGuardCurrentEventContext
        let previousContextHitView = kshrFirstResponderGuardHitViewContext
        let previousContextWindowNumber = kshrFirstResponderGuardContextWindowNumber
        kshrFirstResponderGuardCurrentEventContext = event
        kshrFirstResponderGuardHitViewContext = Self.kshrHitViewForEventDispatch(in: self, event: event)
        kshrFirstResponderGuardContextWindowNumber = self.windowNumber
#if DEBUG
        if event.type == .keyDown {
...
#endif
        defer {
            kshrFirstResponderGuardCurrentEventContext = previousContextEvent
            kshrFirstResponderGuardHitViewContext = previousContextHitView
            kshrFirstResponderGuardContextWindowNumber = previousContextWindowNumber
        }
    ...
    @objc func kshr_performKeyEquivalent(with event: NSEvent) -> Bool {
#if DEBUG
        let typingTimingStart = KshrTypingTiming.start()
        defer {
            KshrTypingTiming.logDuration(
        ...
        let firstResponderGhosttyView = kshrOwningGhosttyView(for: self.firstResponder)
        ...
        if let ghosttyView = firstResponderGhosttyView {
            ...
            if ghosttyView.hasMarkedText(), !event.modifierFlags.intersection(.deviceIndependentFlagsMask).contains(.command) {
                return kshr_performKeyEquivalent(with: event)
            }
        ...
        if shouldDispatchBrowserReturnViaFirstResponderKeyDown(
            ...
            if kshrBrowserReturnForwardingDepth > 0 {
#if DEBUG
                dlog("  → browser Return/Enter reentry; using normal dispatch")
#endif
                return false
            }
            kshrBrowserReturnForwardingDepth += 1
            defer { kshrBrowserReturnForwardingDepth = max(0, kshrBrowserReturnForwardingDepth - 1) }
#if DEBUG
        ...
        let result = kshr_performKeyEquivalent(with: event)
#if DEBUG
    ...
    private static func kshrOwningWebView(for responder: NSResponder) -> KshrWebView? {
        if let webView = responder as? KshrWebView {
            return webView
        }

        if let view = responder as? NSView,
           let webView = kshrOwningWebView(for: view) {
            return webView
        }
        ...
        var current = responder.nextResponder
        while let next = current {
            if let webView = next as? KshrWebView {
                return webView
            }
            if let view = next as? NSView,
               let webView = kshrOwningWebView(for: view) {
    ...
    private static func kshrOwningWebView(
        ...
        if let webView = kshrOwningWebView(for: responder) {
            return webView
        }
        ...
        if let event,
           let hitWebView = kshrPointerHitWebView(in: window, event: event) {
            kshrTrackFieldEditor(textView, owningWebView: hitWebView)
            return hitWebView
        }

        return kshrTrackedOwningWebView(for: textView)
    }

    private static func kshrOwningWebView(for view: NSView) -> KshrWebView? {
        if let webView = view as? KshrWebView {
            return webView
        }

        var current: NSView? = view.superview
        while let candidate = current {
            if let webView = candidate as? KshrWebView {
                return webView
            }
            if String(describing: type(of: candidate)).contains("WindowBrowserSlotView"),
               let portalWebView = kshrUniqueBrowserWebView(in: candidate) {
                ...
                if kshrAllowsPortalSlotTextEntryFocus(view) {
                    return nil
                }
                return portalWebView
            }
    ...
    private static func kshrAllowsPortalSlotTextEntryFocus(_ view: NSView) -> Bool {
        var current: NSView? = view
        while let candidate = current {
    ...
    private static func kshrUniqueBrowserWebView(in root: NSView) -> KshrWebView? {
        var stack: [NSView] = [root]
        var found: KshrWebView?
        while let current = stack.popLast() {
            if let webView = current as? KshrWebView {
                if found == nil {
                    found = webView
    ...
    private static func kshrCurrentEvent(for window: NSWindow) -> NSEvent? {
#if DEBUG
        if let override = kshrFirstResponderGuardCurrentEventOverride {
            return override
        }
#endif
        if kshrFirstResponderGuardContextWindowNumber == window.windowNumber {
            return kshrFirstResponderGuardCurrentEventContext
        }
        return NSApp.currentEvent
    }

    private static func kshrHitViewInThemeFrame(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func kshrHitViewInContentView(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func kshrTopHitViewForEvent(in window: NSWindow, event: NSEvent) -> NSView? {
        if let hitInThemeFrame = kshrHitViewInThemeFrame(in: window, event: event) {
            return hitInThemeFrame
        }
        return kshrHitViewInContentView(in: window, event: event)
    }

    private static func kshrHitViewForEventDispatch(in window: NSWindow, event: NSEvent) -> NSView? {
        ...
        return kshrTopHitViewForEvent(in: window, event: event)
    }

    private static func kshrHitViewForCurrentEvent(in window: NSWindow, event: NSEvent) -> NSView? {
#if DEBUG
        if let override = kshrFirstResponderGuardHitViewOverride {
            return override
        }
#endif
        if kshrFirstResponderGuardContextWindowNumber == window.windowNumber,
           let contextHitView = kshrFirstResponderGuardHitViewContext {
            return contextHitView
        }
        return kshrTopHitViewForEvent(in: window, event: event)
    }

    private static func kshrTrackFieldEditor(_ fieldEditor: NSTextView, owningWebView webView: KshrWebView?) {
        if let webView {
            objc_setAssociatedObject(
                fieldEditor,
                &kshrFieldEditorOwningWebViewAssociationKey,
                KshrFieldEditorOwningWebViewBox(webView: webView),
    ...
    private static func kshrTrackedOwningWebView(for fieldEditor: NSTextView) -> KshrWebView? {
        guard let box = objc_getAssociatedObject(
            fieldEditor,
            &kshrFieldEditorOwningWebViewAssociationKey
        ) as? KshrFieldEditorOwningWebViewBox else {
            return nil
        }
        guard let webView = box.webView else {
            kshrTrackFieldEditor(fieldEditor, owningWebView: nil)
            return nil
        }
        return webView
    }

    private static func kshrIsPointerDownEvent(_ event: NSEvent) -> Bool {
        switch event.type {
    ...
    private static func kshrPointerHitWebView(in window: NSWindow, event: NSEvent) -> KshrWebView? {
        guard kshrIsPointerDownEvent(event) else { return nil }
        if event.windowNumber != 0, event.windowNumber != window.windowNumber {
            return nil
        }
        if let eventWindow = event.window, eventWindow !== window {
        ...
        guard let hitView = kshrHitViewForCurrentEvent(in: window, event: event) else {
            return nil
        }
        return kshrOwningWebView(for: hitView)
    }

    private static func kshrShouldAllowPointerInitiatedWebViewFocus(
        window: NSWindow,
        webView: KshrWebView,
        event: NSEvent?
    ) -> Bool {
        guard let event,
              let hitWebView = kshrPointerHitWebView(in: window, event: event) else {
            return false
        }

...
#if os(macOS)
func kshrShouldApplyWindowGlass(
...
func kshrShouldUseTransparentBackgroundWindow() -> Bool {
    ...
    return kshrShouldApplyWindowGlass(
...
func kshrShouldUseClearWindowBackground(for opacity: Double) -> Bool {
    kshrShouldUseTransparentBackgroundWindow() || opacity < 0.999
}

private func kshrTransparentWindowBaseColor() -> NSColor {
...
private func kshrRuntimeReadClipboardCallback(
...
#if DEBUG
private func kshrChildExitProbePath() -> String? {
...
private func kshrLoadChildExitProbe(at path: String) -> [String: String] {
...
private func kshrWriteChildExitProbe(_ updates: [String: String], increments: [String: Int] = [:]) {
    guard let path = kshrChildExitProbePath() else { return }
    var payload = kshrLoadChildExitProbe(at: path)
...
private func kshrScalarHex(_ value: String?) -> String {
...
#if DEBUG
func kshrPasteboardStringContentsForTesting(_ pasteboard: NSPasteboard) -> String? {
    GhosttyPasteboardHelper.stringContents(from: pasteboard)
}

func kshrPasteboardImageFileURLForTesting(_ pasteboard: NSPasteboard) -> URL? {
    GhosttyPasteboardHelper.saveImageFileURLIfNeeded(from: pasteboard)
}

func kshrPasteboardImagePathForTesting(_ pasteboard: NSPasteboard) -> String? {
...
class GhosttyApp {
    ...
    private static let releaseBundleIdentifier = "com.kshrterm.app"
    ...
    private static func resolveBackgroundLogURL(
        ...
        if let explicitPath = environment["KSHR_DEBUG_BG_LOG"],
           !explicitPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return URL(fileURLWithPath: explicitPath)
        }

        if let debugLogPath = environment["KSHR_DEBUG_LOG"],
        ...
        return URL(fileURLWithPath: "/tmp/kshr-bg.log")
    }
                ...
                TerminalImageTransferPlanner.execute(
                    ...
                    uploadWorkspaceRemote: { fileURLs, operation, finish in
                        guard let workspace = MainActor.assumeIsolated({
                            ...
                            finish(.failure(NSError(domain: "kshr.remote.paste", code: 3)))
    ...
    let backgroundLogEnabled = {
        if ProcessInfo.processInfo.environment["KSHR_DEBUG_BG"] == "1" {
            return true
        }
        if ProcessInfo.processInfo.environment["KSHR_DEBUG_LOG"] != nil {
            return true
        }
        if ProcessInfo.processInfo.environment["GHOSTTYTABS_DEBUG_BG"] == "1" {
            return true
        }
        if UserDefaults.standard.bool(forKey: "kshrDebugBG") {
            return true
        }
    ...
    #if DEBUG
    private static let initLogPath = "/tmp/kshr-ghostty-init.log"
    ...
    private func initializeGhostty() {
        ...
        runtimeConfig.read_clipboard_cb = unsafeBitCast(
            kshrRuntimeReadClipboardCallback as @convention(c) (
        ...
        if let created = ghostty_app_new(&runtimeConfig, primaryConfig) {
            ...
            // kshr still launches with working terminals.
    ...
    private func loadDefaultConfigFilesWithLegacyFallback(_ config: ghostty_config_t) {
        ...
        loadKshrAppSupportGhosttyConfigIfNeeded(config)
        loadCJKFontFallbackIfNeeded(config)
        // kshr provides the terminal background via backgroundView (CALayer)
    ...
    ///
    /// See: https://github.com/yosefkohan26/kshr/pull/1017
    ...
    /// font already covers the ranges kshr would otherwise auto-map.
    ...
    /// Returns only the CJK mappings kshr should auto-inject after respecting
    ...
    /// application-support config paths that kshr may load at runtime.
    ...
    /// Returns the top-level config paths that kshr will actually load before
    ...
    static func loadedCJKScanPaths(
        ...
        let appSupportConfigURLs = kshrAppSupportConfigURLs(
    ...
    /// Scans a single config file for font settings relevant to kshr's
    ...
    static func kshrAppSupportConfigURLs(
    ...
    private func loadKshrAppSupportGhosttyConfigIfNeeded(_ config: ghostty_config_t) {
        #if os(macOS)
        ...
        let urls = Self.kshrAppSupportConfigURLs(
...
#if DEBUG
        dlog(
            "loaded kshr app support ghostty config from: \(urls.map(\.path).joined(separator: ", "))"
        )
#endif
        #endif
    }
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        if action.tag == GHOSTTY_ACTION_SHOW_CHILD_EXITED {
            ...
            // handles this action. For kshr, the correct behavior is to close
        ...
        case GHOSTTY_ACTION_OPEN_URL:
            ...
            if !BrowserLinkOpenSettings.openTerminalLinksInKshrBrowser() {
                #if DEBUG
                dlog("link.openURL kshrBrowser=disabled, opening externally url=\(target.url)")
                #endif
                return performOnMain {
    ...
    private func applyBackgroundToKeyWindow() {
        ...
        if kshrShouldUseClearWindowBackground(for: defaultBackgroundOpacity) {
            window.backgroundColor = kshrTransparentWindowBaseColor()
    ...
    func logBackground(_ message: String) {
        ...
        let line =
            "\(timestamp) seq=\(sequence) t+\(String(format: "%.3f", uptimeMs))ms thread=\(threadLabel) frame60=\(frame60) frame120=\(frame120) kshr bg: \(message)\n"
...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    /// Port ordinal for KSHR_PORT range assignment
    var portOrdinal: Int = 0
    /// Snapshotted once per app session so all workspaces use consistent values
    private static let sessionPortBase: Int = {
        let val = UserDefaults.standard.integer(forKey: "kshrPortBase")
        return val > 0 ? val : 9100
    }()
    private static let sessionPortRangeSize: Int = {
        let val = UserDefaults.standard.integer(forKey: "kshrPortRange")
        return val > 0 ? val : 10
    }()
    private let surfaceContext: ghostty_surface_context_e
    private let configTemplate: KshrSurfaceConfigTemplate?
    ...
    func debugSurfaceContextLabel() -> String {
        kshrSurfaceContextName(surfaceContext)
    }
    ...
    func liveSurfaceForGhosttyAccess(reason: String) -> ghostty_surface_t? {
        ...
        guard registeredOwnerId == id,
              kshrSurfacePointerAppearsLive(surface) else {
...
#if DEBUG
    private static let surfaceLogPath = "/tmp/kshr-ghostty-surface.log"
    private static let sizeLogPath = "/tmp/kshr-ghostty-size.log"
    ...
    private static func sizeLog(_ message: String) {
        let env = ProcessInfo.processInfo.environment
        guard env["KSHR_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1" else { return }
        let timestamp = ISO8601DateFormatter().string(from: Date())
        let line = "[\(timestamp)] \(message)\n"
    ...
    private func createSurface(for view: GhosttyNSView) {
        ...
        let baseConfig = configTemplate ?? KshrSurfaceConfigTemplate()
        ...
        dlog(
            "zoom.create surface=\(id.uuidString.prefix(5)) context=\(kshrSurfaceContextName(surfaceContext)) " +
        ...
        setManagedEnvironmentValue("KSHR_SURFACE_ID", id.uuidString)
        setManagedEnvironmentValue("KSHR_WORKSPACE_ID", tabId.uuidString)
        // Backward-compatible shell integration keys used by existing scripts/tests.
        setManagedEnvironmentValue("KSHR_PANEL_ID", id.uuidString)
        setManagedEnvironmentValue("KSHR_TAB_ID", tabId.uuidString)
        let socketPath = SocketControlSettings.socketPath()
        setManagedEnvironmentValue("KSHR_SOCKET_PATH", socketPath)
        // Backward-compatible alias expected by older scripts and third-party integrations.
        setManagedEnvironmentValue("KSHR_SOCKET", socketPath)
        if let bundledCLIURL = Bundle.main.resourceURL?.appendingPathComponent("bin/kshr"),
           FileManager.default.isExecutableFile(atPath: bundledCLIURL.path) {
            setManagedEnvironmentValue("KSHR_BUNDLED_CLI_PATH", bundledCLIURL.path)
        }
        if let bundleId = Bundle.main.bundleIdentifier, !bundleId.isEmpty {
            setManagedEnvironmentValue("KSHR_BUNDLE_ID", bundleId)
        }
        ...
        do {
            ...
            setManagedEnvironmentValue("KSHR_PORT", String(startPort))
            setManagedEnvironmentValue("KSHR_PORT_END", String(startPort + Self.sessionPortRangeSize - 1))
            setManagedEnvironmentValue("KSHR_PORT_RANGE", String(Self.sessionPortRangeSize))
        }

        let claudeHooksEnabled = ClaudeCodeIntegrationSettings.hooksEnabled()
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("KSHR_CLAUDE_HOOKS_DISABLED", "1")
        }
        if let customClaudePath = ClaudeCodeIntegrationSettings.customClaudePath() {
            setManagedEnvironmentValue("KSHR_CUSTOM_CLAUDE_PATH", customClaudePath)
        }
        ...
        if shellIntegrationEnabled,
           let integrationDir = Bundle.main.resourceURL?.appendingPathComponent("shell-integration").path {
            setManagedEnvironmentValue("KSHR_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("KSHR_SHELL_INTEGRATION_DIR", integrationDir)
            ...
            if shellName == "zsh" {
                if GhosttyApp.shared.shellIntegrationMode() != "none" {
                    setManagedEnvironmentValue("KSHR_LOAD_GHOSTTY_ZSH_INTEGRATION", "1")
                }
                ...
                if let candidateZdotdir, !candidateZdotdir.isEmpty {
                    ...
                    if !isGhosttyInjected {
                        setManagedEnvironmentValue("KSHR_ZSH_ZDOTDIR", candidateZdotdir)
                    }
                }
                ...
                if GhosttyApp.shared.shellIntegrationMode() != "none" {
                    setManagedEnvironmentValue("KSHR_LOAD_GHOSTTY_BASH_INTEGRATION", "1")
                }
                // macOS ships /bin/bash 3.2, where Ghostty's automatic bash
                // integration is unsupported and HOME-based wrapper startup is
                // not reliable. Bootstrap kshr bash integration on the first
                // interactive prompt instead.
                setManagedEnvironmentValue("PROMPT_COMMAND", """
                unset PROMPT_COMMAND; \
                if [[ "${KSHR_LOAD_GHOSTTY_BASH_INTEGRATION:-0}" == "1" && -n "${GHOSTTY_RESOURCES_DIR:-}" ]]; then \
                _kshr_ghostty_bash="$GHOSTTY_RESOURCES_DIR/shell-integration/bash/ghostty.bash"; \
                [[ -r "$_kshr_ghostty_bash" ]] && source "$_kshr_ghostty_bash"; \
                fi; \
                if [[ "${KSHR_SHELL_INTEGRATION:-1}" != "0" && -n "${KSHR_SHELL_INTEGRATION_DIR:-}" ]]; then \
                _kshr_bash_integration="$KSHR_SHELL_INTEGRATION_DIR/kshr-bash-integration.bash"; \
                [[ -r "$_kshr_bash_integration" ]] && source "$_kshr_bash_integration"; \
                fi; \
                unset _kshr_ghostty_bash _kshr_bash_integration; \
                if declare -F _kshr_prompt_command >/dev/null 2>&1; then _kshr_prompt_command; fi
                """)
            }
        }
        ...
        if let inheritedFontPoints = configTemplate?.fontSize,
           inheritedFontPoints > 0 {
            let currentFontPoints = kshrCurrentSurfaceFontSizePoints(createdSurface)
            let shouldReapply = {
...
#if DEBUG
        let runtimeFontText = kshrCurrentSurfaceFontSizePoints(createdSurface).map {
            String(format: "%.2f", $0)
        } ?? "nil"
        dlog(
            "zoom.create.done surface=\(id.uuidString.prefix(5)) context=\(kshrSurfaceContextName(surfaceContext)) " +
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    private static let focusDebugEnabled: Bool = {
        if ProcessInfo.processInfo.environment["KSHR_FOCUS_DEBUG"] == "1" {
            return true
        }
        return UserDefaults.standard.bool(forKey: "kshrFocusDebug")
    }()
    ...
    private static let sidebarTabReorderPasteboardType = NSPasteboard.PasteboardType("com.kshr.sidebar-tab-reorder")
    ...
    private static let keyLatencyProbeEnabled: Bool = {
        if ProcessInfo.processInfo.environment["KSHR_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
    ...
    func applyWindowBackgroundIfActive() {
        ...
        let color = effectiveBackgroundColor()
        if kshrShouldUseClearWindowBackground(for: color.alphaComponent) {
            window.backgroundColor = kshrTransparentWindowBaseColor()
            window.isOpaque = false
        ...
        if GhosttyApp.shared.backgroundLogEnabled {
            let signature = "\(kshrShouldUseClearWindowBackground(for: color.alphaComponent) ? "transparent" : color.hexString()):\(String(format: "%.3f", color.alphaComponent))"
            if signature != lastLoggedWindowBackgroundSignature {
                ...
                let source = hasOverride ? "surfaceOverride" : "defaultBackground"
                GhosttyApp.shared.logBackground(
                    "window background applied tab=\(tabId?.uuidString ?? "unknown") surface=\(terminalSurface?.id.uuidString ?? "unknown") source=\(source) override=\(overrideHex) default=\(defaultHex) transparent=\(kshrShouldUseClearWindowBackground(for: color.alphaComponent)) color=\(color.hexString()) opacity=\(String(format: "%.3f", color.alphaComponent))"
                )
            }
        }
    }
    ...
    private func recordKeyLatency(path: String, event: NSEvent) {
        ...
        KshrTypingTiming.logEventDelay(path: path, event: event)
    }
#endif
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        kshrWriteChildExitProbe(
            [
                "probePerformCharsHex": kshrScalarHex(event.characters),
                "probePerformCharsIgnoringHex": kshrScalarHex(event.charactersIgnoringModifiers),
    ...
    override func keyDown(with event: NSEvent) {
        ...
        defer {
            ...
            KshrTypingTiming.logDuration(path: "terminal.keyDown", startedAt: typingTimingStart, event: event)
        }
        ...
        kshrWriteChildExitProbe(
            [
                "probeKeyDownCharsHex": kshrScalarHex(event.characters),
                "probeKeyDownCharsIgnoringHex": kshrScalarHex(event.charactersIgnoringModifiers),
        ...
        if flags.contains(.control) && !flags.contains(.command) && !flags.contains(.option) && !hasMarkedText() {
            ...
            if text.isEmpty {
                ...
                #if DEBUG
                let sendTimingStart = KshrTypingTiming.start()
                let ghosttySendStart = ProcessInfo.processInfo.systemUptime
                #endif
            ...
            dlog(
                ...
                "handled=\(handled ? 1 : 0) keyCode=\(event.keyCode) chars=\(kshrScalarHex(event.characters)) " +
                "ign=\(kshrScalarHex(event.charactersIgnoringModifiers)) mods=\(event.modifierFlags.rawValue)"
            )
#endif
...
#if DEBUG
        let interpretTimingStart = KshrTypingTiming.start()
    ...
    private func sendTimedGhosttyKey(
        ...
        KshrTypingTiming.logDuration(path: path, startedAt: timingStart, event: event, extra: mergedExtra)
        return handled
    }
#endif
    ...
    static func handleDropForTesting(
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadDetectedSSH: { _, _, _, finish in
                finish(.failure(NSError(domain: "kshr.remote.drop", code: 4)))
            },
            insertText: sendText,
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadWorkspaceRemote: { [weak self] fileURLs, operation, finish in
                guard let workspace = MainActor.assumeIsolated({
                    ...
                    finish(.failure(NSError(domain: "kshr.remote.drop", code: 3)))
                    GhosttyPasteboardHelper.cleanupTransferredTemporaryImageFiles(fileURLs)
                    return
                }
                workspace.uploadDroppedFilesForRemoteTerminal(
    ...
    fileprivate func debugSimulateFileDrop(paths: [String]) -> Bool {
        ...
        let pbName = NSPasteboard.Name("kshr.debug.drop.\(UUID().uuidString)")
...
final class GhosttySurfaceScrollView: NSView {
    ...
    init(surfaceView: GhosttyNSView) {
        ...
        dropZoneOverlayView.layer?.backgroundColor = kshrAccentNSColor().withAlphaComponent(0.25).cgColor
        dropZoneOverlayView.layer?.borderColor = kshrAccentNSColor().cgColor
    ...
    func triggerFlash(style: FlashStyle = .navigation) {
        DispatchQueue.main.async { [weak self] in
            ...
            self.flashLayer.add(animation, forKey: "kshr.flash")
        }
    }
    ...
    func ensureFocus(for tabId: UUID, surfaceId: UUID) {
        ...
        guard let tab = tabManager.tabsById[tabId],
    ...
    /// Called by the portal system after a hosted view becomes display-active,
    /// either by revealing a hidden portal view or by first attaching a view that
    /// was already logically marked visible before the portal bind completed.
    func retryFocusAfterPortalDisplayActivation(reason: String) {
...
extension GhosttyNSView: NSTextInputClient {
    ...
    fileprivate func sendTextToSurface(_ chars: String, preserveLiteralEscape: Bool) {
        ...
        kshrWriteChildExitProbe(
            [
                "probeInsertTextCharsHex": kshrScalarHex(chars),

...
enum SessionRestorePolicy {
    static func isRunningUnderAutomatedTests(
        ...
        if environment["KSHR_UI_TEST_MODE"] == "1" {
            return true
        }
    ...
    static func shouldAttemptRestore(
        ...
        if environment["KSHR_DISABLE_SESSION_RESTORE"] == "1" {
            return false
        }
...
enum SessionScrollbackReplayStore {
    static let environmentKey = "KSHR_RESTORE_SCROLLBACK_FILE"
    private static let directoryName = "kshr-session-scrollback"

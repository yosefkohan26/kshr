...
private var kshrWindowTerminalPortalKey: UInt8 = 0
private var kshrWindowTerminalPortalCloseObserverKey: UInt8 = 0
...
final class WindowTerminalHostView: NSView {
    ...
    // PERF: Cached split-divider regions to avoid O(n) tree walks at 60-120Hz.
    // Invalidated when splits resize, window resizes, or layout changes.
    private var cachedDividerRegions: [DividerRegion]?
    private var cachedDividerRegionsBoundsKey: NSRect = .zero

    // PERF: Cached sidebar-resizer hosted view frames to avoid subview iteration.
    private struct SidebarResizerCache {
        let hostedFrames: [(minX: CGFloat, maxX: CGFloat, width: CGFloat, height: CGFloat)]
        let boundsKey: NSRect
    }
    private var sidebarResizerCache: SidebarResizerCache?

    // PERF: Track drag pasteboard state to avoid reading NSPasteboard on non-drag events.
    // Only updated for actual drag events; reset on mouseUp/exit.
    private var cachedDragPassThrough: Bool?
    private var cachedDragEventNumber: Int?

#if DEBUG
    private var lastDragRouteSignature: String?
#endif

    /// Invalidate all cached hit-test regions. Called when layout changes
    /// (splits added/removed/resized, sidebar width changes, window resizes).
    func invalidateHitTestCaches() {
        cachedDividerRegions = nil
        sidebarResizerCache = nil
        cachedDragPassThrough = nil
        cachedDragEventNumber = nil
    }
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        if isPointerEvent {
            ...
            // Uses cached divider regions to avoid O(n) tree walk at 60-120Hz.
            if let kind = splitDividerCursorKindCached(at: point) {
            ...
            // PERF: Only read the drag pasteboard for actual drag events.
            // For non-drag pointer events, skip the pasteboard entirely.
            ...
            if isDragEvent {
                // Cache drag pasteboard result per event number to avoid
                // repeated pasteboard reads during the same drag event.
                let eventNumber = currentEvent?.eventNumber ?? -1
                let shouldPassThrough: Bool
                if cachedDragEventNumber == eventNumber, let cached = cachedDragPassThrough {
                    shouldPassThrough = cached
                } else {
                    ...
                    cachedDragPassThrough = shouldPassThrough
                    cachedDragEventNumber = eventNumber
#if DEBUG
                ...
                // Reset drag cache on non-drag events so stale state doesn't persist.
                if cachedDragPassThrough != nil {
                    cachedDragPassThrough = nil
                    cachedDragEventNumber = nil
                }
            }
    ...
    private func shouldPassThroughToSidebarResizer(at point: NSPoint) -> Bool {
        ...
        // PERF: Cache hosted view frame data to avoid subview iteration at 60-120Hz.
        // The cache is invalidated on layout changes (frame/bounds/window changes).
        let hostedFrames: [(minX: CGFloat, maxX: CGFloat, width: CGFloat, height: CGFloat)]
        let currentBounds = bounds
        if let cache = sidebarResizerCache,
           cache.boundsKey == currentBounds {
            hostedFrames = cache.hostedFrames
        } else {
            var frames: [(minX: CGFloat, maxX: CGFloat, width: CGFloat, height: CGFloat)] = []
            for subview in subviews {
                guard let hosted = subview as? GhosttySurfaceScrollView else { continue }
                guard !hosted.isHidden, hosted.window != nil else { continue }
                let frame = hosted.frame
                guard frame.width > 1, frame.height > 1 else { continue }
                frames.append((minX: frame.minX, maxX: frame.maxX, width: frame.width, height: frame.height))
            }
            hostedFrames = frames
            sidebarResizerCache = SidebarResizerCache(hostedFrames: frames, boundsKey: currentBounds)
        }
        ...
        let hasLeadingContent = hostedFrames.contains {
        ...
        var leftMostEdge: CGFloat? = nil
        for frame in hostedFrames {
            if frame.minX > Self.sidebarLeadingEdgeEpsilon {
                if leftMostEdge == nil || frame.minX < leftMostEdge! {
                    leftMostEdge = frame.minX
                }
            }
        }
        if let leftMostEdge {
    ...
    private func updateDividerCursor(at point: NSPoint) {
        ...
        guard let nextKind = splitDividerCursorKindCached(at: point) else {
            clearActiveDividerCursor(restoreArrow: true)
            return
        }
    ...
    /// PERF: Cached version that checks point against pre-computed divider rects
    /// instead of walking the full view tree on every pointer event.
    private func splitDividerCursorKindCached(at point: NSPoint) -> DividerCursorKind? {
        guard let window else { return nil }
        let windowPoint = convert(point, to: nil)
        guard window.contentView != nil else { return nil }

        let regions = ensureCachedDividerRegions()
        let expansion: CGFloat = 5
        for region in regions {
            let expandedRect = region.rectInWindow.insetBy(dx: -expansion, dy: -expansion)
            if expandedRect.contains(windowPoint) {
    ...
    /// Build or return cached divider regions. The cache is keyed on the host
    /// view's bounds so any frame/layout change forces a rebuild.
    private func ensureCachedDividerRegions() -> [DividerRegion] {
        let currentBounds = bounds
        if let cached = cachedDividerRegions, cachedDividerRegionsBoundsKey == currentBounds {
            return cached
        }
        guard let window, let rootView = window.contentView else { return [] }
        var regions: [DividerRegion] = []
        ...
        cachedDividerRegionsBoundsKey = currentBounds
        return regions
    }
...
final class WindowTerminalPortal: NSObject {
    ...
    fileprivate func synchronizeAllEntriesFromExternalGeometryChange() {
        ...
        // Invalidate hit-test caches since split/window geometry changed.
        hostView.invalidateHitTestCaches()
    ...
    func detachHostedView(withId hostedId: ObjectIdentifier) {
        ...
        // Invalidate hit-test caches since hosted view set is changing.
        hostView.invalidateHitTestCaches()
    ...
    func bind(hostedView: GhosttySurfaceScrollView, to anchorView: NSView, visibleInUI: Bool, zPriority: Int = 0) {
        ...
        if needsVisibleAttachKick,
           !hostedView.isHidden,
           hostedView.frame.width >= Self.minimumRevealWidth,
           hostedView.frame.height >= Self.minimumRevealHeight {
            // A workspace can mark its hosted terminal visible before the portal host is
            // actually in the window. In that case the first real attach skips the normal
            // reveal path, so explicitly give it the same geometry/redraw/focus nudge here.
#if DEBUG
            dlog(
                "portal.attach.visibleKick hosted=\(portalDebugToken(hostedView)) " +
                "frame=\(portalDebugFrame(hostedView.frame)) visibleInUI=\(visibleInUI ? 1 : 0)"
            )
#endif
            hostedView.reconcileGeometryNow()
            hostedView.refreshSurfaceNow(reason: "portal.attach")
            hostedView.retryFocusAfterPortalDisplayActivation(reason: "portal.attach")
        }
    ...
    private func synchronizeHostedView(withId hostedId: ObjectIdentifier) {
        ...
        if !shouldHide, hostedView.isHidden, revealReadyForDisplay {
            ...
            // Earlier focus retries (from setActive/setVisibleInUI) may have been consumed
            // while the view was hidden with unsettled geometry. Retry now that the view is
            // physically visible so new workspaces receive first responder.
            hostedView.retryFocusAfterPortalDisplayActivation(reason: "portal.reveal")
        }
...
enum TerminalWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &kshrWindowTerminalPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &kshrWindowTerminalPortalCloseObserverKey,
            observer,
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        if let observer = objc_getAssociatedObject(window, &kshrWindowTerminalPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &kshrWindowTerminalPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &kshrWindowTerminalPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow) -> WindowTerminalPortal {
        if let existing = objc_getAssociatedObject(window, &kshrWindowTerminalPortalKey) as? WindowTerminalPortal {
        ...
        objc_setAssociatedObject(window, &kshrWindowTerminalPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)

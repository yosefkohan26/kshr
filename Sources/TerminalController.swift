...
extension Notification.Name {
    static let socketListenerDidStart = Notification.Name("kshr.socketListenerDidStart")
    static let terminalSurfaceDidBecomeReady = Notification.Name("kshr.terminalSurfaceDidBecomeReady")
    static let terminalSurfaceHostedViewDidMoveToWindow = Notification.Name("kshr.terminalSurfaceHostedViewDidMoveToWindow")
    static let mainWindowContextsDidChange = Notification.Name("kshr.mainWindowContextsDidChange")
    static let browserDownloadEventDidArrive = Notification.Name("kshr.browserDownloadEventDidArrive")
}
...
class TerminalController {
    ...
    private var accessMode: SocketControlMode = .kshrOnly
    ...
    private static let v2BrowserEvalEnvelopeTypeKey = "__kshr_t"
    private static let v2BrowserEvalEnvelopeValueKey = "__kshr_v"
    ...
    private final class SocketFastPathState: @unchecked Sendable {
        private let queue = DispatchQueue(label: "com.kshr.socket-fast-path")
        ...
        func shouldUpdatePorts(workspaceId: UUID, panelId: UUID, ports: [Int]) -> Bool {
            let key = SocketSurfaceKey(workspaceId: workspaceId, panelId: panelId)
            let normalizedPorts = Array(Set(ports)).sorted()
            return queue.sync {
                let current = lastReportedPorts[key]
                guard TerminalController.shouldReplacePorts(current: current, next: normalizedPorts) else {
                    return false
                }
    ...
    func start(tabManager: TabManager, socketPath: String, accessMode: SocketControlMode) {
        ...
        PortScanner.shared.onPortsUpdated = { [weak self] workspaceId, panelId, ports in
            ...
            guard let workspace = tabManager.tabsById[workspaceId] else { return }
            let validSurfaceIds = Set(workspace.panels.keys)
            guard validSurfaceIds.contains(panelId) else { return }
            workspace.telemetry.surfaceListeningPorts[panelId] = ports.isEmpty ? nil : ports
        ...
        PortScanner.shared.agentPIDsProvider = { [weak self] workspaceIds in
            ...
            for workspaceId in workspaceIds {
                guard let workspace = tabManager.tabsById[workspaceId] else { continue }
    ...
    private func handleClient(_ socket: Int32, peerPid: pid_t? = nil) {
        ...
        // In kshrOnly mode, verify the connecting process is a descendant of kshr.
        // In allowAll mode (env-var only), skip the ancestry check.
        if accessMode == .kshrOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    let msg = "ERROR: Access denied — only processes started inside kshr can connect\n"
    ...
    private func processV2Command(_ jsonLine: String) -> String {
        ...
        // Skip v2RefreshKnownRefs() for telemetry/high-frequency commands that arrive
        // with explicit UUIDs and don't need handle-ref resolution. This avoids a
        // synchronous main-thread walk of the entire window/workspace/pane/surface graph
        // on every telemetry tick. These commands use v2UUID() for param parsing and
        // v2EnsureHandleRef() lazily when building response payloads.
        if !Self.v2TelemetryMethods.contains(method) {
    ...
    private func v2Identify(params: [String: Any]) -> [String: Any] {
        ...
        v2MainSync {
            ...
            if let wsId = tabManager.selectedTabId,
               let ws = tabManager.tabsById[wsId] {
    ...
    /// High-frequency telemetry and status commands that arrive with explicit UUIDs
    /// and do not need a full ref-table refresh before dispatch. Skipping the refresh
    /// avoids a synchronous main-thread graph walk on every telemetry tick.
    private static let v2TelemetryMethods: Set<String> = [
        "surface.report_tty",
    ...
    private func v2WorkspaceSelect(params: [String: Any]) -> V2CallResult {
        ...
        var success = false
        v2MainSync {
            if let ws = tabManager.tabsById[wsId] {
    ...
    private func v2WorkspaceCurrent(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            if let wsId, let workspace = tabManager.tabsById[wsId] {
    ...
    private func v2SurfaceReportShellState(params: [String: Any]) -> V2CallResult {
        ...
        guard let rawState = v2RawString(params, "state"),
            ...
            return .err(code: "invalid_params", message: "Missing or invalid state (expected prompt or running)", data: nil)
        }
        ...
        guard Self.socketFastPathState.shouldPublishShellActivity(
            ...
            return .ok(["workspace_id": workspaceId.uuidString, "surface_id": surfaceId.uuidString])
        }
    ...
    private func v2SurfaceReportPwd(params: [String: Any]) -> V2CallResult {
        ...
        guard let directory = v2RawString(params, "directory")?.trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func v2WorkspaceAction(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            guard let workspaceId = requestedWorkspaceId,
                  let workspace = tabManager.tabsById[workspaceId] else {
    ...
    private func v2ResolveWorkspace(params: [String: Any], tabManager: TabManager) -> Workspace? {
        if let wsId = v2UUID(params, "workspace_id") {
            return tabManager.tabsById[wsId]
        }
        if let surfaceId = v2UUID(params, "surface_id") ?? v2UUID(params, "tab_id") {
    ...
    private func v2SurfaceMove(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            guard let source = app.locateSurface(surfaceId: surfaceId),
                  let sourceWorkspace = source.tabManager.tabsById[source.workspaceId] else {
                result = .err(code: "not_found", message: "Surface not found", data: ["surface_id": surfaceId.uuidString])
                return
            }
            ...
            if let anchorSurfaceId = beforeSurfaceId ?? afterSurfaceId {
                guard let anchor = app.locateSurface(surfaceId: anchorSurfaceId),
                      let anchorWorkspace = anchor.tabManager.tabsById[anchor.workspaceId],
    ...
    private func v2SurfaceReorder(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            guard let app = AppDelegate.shared,
                  ...
                  let ws = located.tabManager.tabsById[located.workspaceId],
    ...
    private func v2DebugTerminals(params _: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            let terminals: [[String: Any]] = surfaces.enumerated().map { index, terminalSurface in
                ...
                let listeningPorts = (workspace?.telemetry.surfaceListeningPorts[panelId] ?? []).sorted()
    ...
    private func v2NotificationCreateForTarget(params: [String: Any]) -> V2CallResult {
        ...
        var result: V2CallResult = .err(code: "internal_error", message: "Failed to notify", data: nil)
        v2MainSync {
            guard let ws = tabManager.tabsById[wsId] else {
                result = .err(code: "not_found", message: "Workspace not found", data: ["workspace_id": wsId.uuidString])
                return
            }
            guard ws.panels[surfaceId] != nil else {
    ...
    private func v2RunJavaScript(
        ...
        guard let webView else { return .failure("WebView not available") }
    ...
    private func v2RunBrowserJavaScript(
        ...
        guard let webView else { return .failure("WebView not available (evicted or not yet created)") }
        ...
        if let frameSelector = v2BrowserCurrentFrameSelector(surfaceId: surfaceId) {
            ...
            framePrelude = """
            let __kshrDoc = document;
            try {
              const __kshrFrame = document.querySelector(\(selectorLiteral));
              if (__kshrFrame && __kshrFrame.contentDocument) {
                __kshrDoc = __kshrFrame.contentDocument;
              }
            } catch (_) {}
            """
        } else {
            framePrelude = "const __kshrDoc = document;"
        }
        ...
        const __kshrMaybeAwait = async (__r) => {
        ...
        const __kshrEvalInFrame = async function() {
          const document = __kshrDoc;
          \(executionBlock)
          const __value = await __kshrMaybeAwait(__r);
          return {
            __kshr_t: (typeof __value === 'undefined') ? 'undefined' : 'value',
            __kshr_v: __value
          };
        };

        return await __kshrEvalInFrame();
        """
    ...
    private func v2BrowserEnsureInitScriptsApplied(surfaceId: UUID, browserPanel: BrowserPanel) {
        ...
        let injector = """
        (() => {
          window.__kshrInitScriptsApplied = window.__kshrInitScriptsApplied || { scripts: [], styles: [] };
          return true;
        })()
        """
        ...
        for css in styles {
            ...
            let styleScript = """
            (() => {
              const id = 'kshr-init-style-' + btoa(unescape(encodeURIComponent(\(cssLiteral)))).replace(/=+$/g, '');
    ...
    private func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let screenshotsDirectory = FileManager.default.temporaryDirectory
                .appendingPathComponent("kshr-browser-screenshots", isDirectory: true)
    ...
    private func v2BrowserFocusWebView(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            guard let webView = browserPanel.webView else { return }
    ...
    private func v2BrowserFindWithScript(
        ...
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            \(finderBody)
              ...
              if (!__kshrFound) return { ok: false, error: 'not_found' };
              const selector = __kshrCssPath(__kshrFound);
              if (!selector) return { ok: false, error: 'not_found' };
              return {
                ok: true,
                selector,
                tag: String(__kshrFound.tagName || '').toLowerCase(),
                text: String(__kshrFound.textContent || '').trim()
              };
            })()
            """
    ...
    private func v2BrowserDialogRespond(params: [String: Any], accept: Bool) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const q = window.__kshrDialogQueue || [];
              ...
              if (entry.type === 'confirm') {
                ...
                window.__kshrDialogDefaults.confirm = \(acceptLiteral);
              }
              if (entry.type === 'prompt') {
                window.__kshrDialogDefaults = window.__kshrDialogDefaults || { confirm: false, prompt: null };
                if (\(acceptLiteral)) {
                  window.__kshrDialogDefaults.prompt = \(textLiteral);
                } else {
                  window.__kshrDialogDefaults.prompt = null;
                }
              }
    ...
    private func v2BrowserConsoleList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__kshrConsoleLog) ? window.__kshrConsoleLog.slice() : [];
              if (\(clearLiteral)) {
                window.__kshrConsoleLog = [];
              }
    ...
    private func v2BrowserErrorsList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__kshrErrorLog) ? window.__kshrErrorLog.slice() : [];
              if (\(clearLiteral)) {
                window.__kshrErrorLog = [];
              }
              return { ok: true, items };
            })()
            """
    ...
    private func helpText() -> String {
        ...
        Split & surface commands:
          ...
          reload_config                   - Reload Ghostty config, kshr settings, and refresh terminals
    ...
    private func terminalDropOverlayProbe(_ args: String) -> String {
        ...
        DispatchQueue.main.sync {
            guard let selectedId = tabManager.selectedTabId,
                  let workspace = tabManager.tabsById[selectedId] else {
                return
            }
    ...
    private func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("kshr-screenshots")
    ...
    private func layoutDebug() -> String {
        ...
        DispatchQueue.main.sync {
            ...
            let selectedPanels: [LayoutDebugSelectedPanel] = tab.bonsplitController.allPaneIds.map { paneId in
                ...
                if let bp = panel as? BrowserPanel, let wv = bp.webView {
                    let viewRect = windowFrame(for: wv).map { PixelRect(from: $0) }
                    let splitViews = splitViewInfos(for: wv)
		                    return LayoutDebugSelectedPanel(
	                        paneId: paneIdStr,
	                        paneFrame: paneFrame,
	                        selectedTabId: selectedTabId,
	                        ...
	                        inWindow: wv.window != nil,
	                        hidden: isHiddenOrAncestorHidden(wv),
	                        viewFrame: viewRect,
	                        splitViews: splitViews
	                    )
	                }
    ...
    private func resolveTab(from arg: String, tabManager: TabManager) -> Tab? {
        ...
        if trimmed.isEmpty {
            ...
            return tabManager.tabsById[selected]
        }

        if let uuid = UUID(uuidString: trimmed) {
            return tabManager.tabsById[uuid]
        }
    ...
    private func closeWorkspace(_ tabId: String) -> String {
        ...
        DispatchQueue.main.sync {
            if let tab = tabManager.tabsById[uuid] {
    ...
    private func sendInput(_ text: String) -> String {
        ...
        DispatchQueue.main.sync {
            guard let selectedId = tabManager.selectedTabId,
                  let tab = tabManager.tabsById[selectedId],
    ...
    private func resolveTabForReport(_ args: String) -> Tab? {
        ...
        return tabManager.tabsById[selectedId]
    }
        ...
        if let scope = Self.explicitSocketScope(options: parsed.options) {
            DispatchQueue.main.async {
                guard let tabManager = AppDelegate.shared?.tabManagerFor(tabId: scope.workspaceId),
                      let tab = tabManager.tabsById[scope.workspaceId] else {
                    return
                }
                let validSurfaceIds = Set(tab.panels.keys)
                tab.pruneSurfaceMetadata(validSurfaceIds: validSurfaceIds)
    ...
    private func reportPorts(_ args: String) -> String {
        ...
        // Explicit scope path: dedup off-main, dispatch async
        if let scope = Self.explicitSocketScope(options: parsed.options) {
            guard Self.socketFastPathState.shouldUpdatePorts(
            ...
            DispatchQueue.main.async {
                ...
                tab.pruneSurfaceMetadata(validSurfaceIds: validSurfaceIds)
                guard validSurfaceIds.contains(scope.panelId) else { return }
                tab.telemetry.surfaceListeningPorts[scope.panelId] = ports
                tab.recomputeListeningPorts()
            }
            return "OK"
        }

        // Fallback path: resolve panel on main, dedup inside
        var result = "OK"
        DispatchQueue.main.sync {
            guard let tab = resolveTabForReport(args) else {
                result = parsed.options["tab"] != nil ? "ERROR: Tab not found" : "ERROR: No tab selected"
            ...
            // Dedup: skip recompute if ports unchanged
            guard Self.shouldReplacePorts(current: tab.telemetry.surfaceListeningPorts[surfaceId], next: ports) else {
                return
            }

            tab.telemetry.surfaceListeningPorts[surfaceId] = ports
            tab.recomputeListeningPorts()
        }
        return result
    }
    ...
    private func reportPwd(_ args: String) -> String {
        ...
        if let scope = Self.explicitSocketScope(options: parsed.options) {
            // Dedup off-main: skip main-thread dispatch when directory unchanged
            guard Self.socketFastPathState.shouldPublishDirectory(
                workspaceId: scope.workspaceId,
                panelId: scope.panelId,
                directory: directory
        ...
        DispatchQueue.main.sync {
            ...
            // Dedup on-main for fallback path (panel ID resolved here)
            guard Self.socketFastPathState.shouldPublishDirectory(
                workspaceId: tab.id,
                panelId: surfaceId,
                directory: directory
    ...
    private func clearPorts(_ args: String) -> String {
        ...
        DispatchQueue.main.sync {
            ...
            if let panelArg {
                ...
                tab.telemetry.surfaceListeningPorts.removeValue(forKey: surfaceId)
            } else {
                tab.telemetry.surfaceListeningPorts.removeAll()
            }
            tab.recomputeListeningPorts()
        }
        return result

...
enum BrowserFindJavaScript {
    ...
    static func searchScript(query: String) -> String {
        ...
        return """
        (() => {
          const MARK_CLASS = '__kshr-find';
          const CURRENT_CLASS = '__kshr-find-current';

          const query = "\(escaped)";
          if (!query) {
            \(clearBody)
            window.__kshrFindQuery = '';
            window.__kshrFindMatches = [];
            window.__kshrFindIndex = 0;
            return JSON.stringify({total: 0, current: 0});
          }

          // Skip full DOM walk if query unchanged and highlights still present.
          if (query === window.__kshrFindQuery &&
              window.__kshrFindMatches && window.__kshrFindMatches.length > 0 &&
              window.__kshrFindMatches[0].isConnected) {
            const m = window.__kshrFindMatches;
            const idx = window.__kshrFindIndex || 0;
            return JSON.stringify({total: m.length, current: idx});
          }
          ...
          window.__kshrFindQuery = query;
          window.__kshrFindMatches = matches;
          window.__kshrFindIndex = 0;
          ...
          if (!document.getElementById('__kshr-find-style')) {
            const style = document.createElement('style');
            style.id = '__kshr-find-style';
            style.textContent = `
              mark.__kshr-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__kshr-find.__kshr-find-current { background: #f97316; color: #fff; }
            `;
    ...
    static func nextScript() -> String {
        """
        (() => {
          const matches = window.__kshrFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__kshrFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__kshrFindMatches = [];
            window.__kshrFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__kshr-find-current');
          ...
          matches[idx].classList.add('__kshr-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__kshrFindIndex = idx;
    ...
    static func clearScript() -> String {
        ...
        (() => {
          ...
          window.__kshrFindMatches = [];
          window.__kshrFindIndex = 0;
          const style = document.getElementById('__kshr-find-style');
    ...
    document.querySelectorAll('mark.__kshr-find').forEach(mark => {

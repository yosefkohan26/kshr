...
def infer_app_name_for_osascript(socket_path: str) -> str:
    ...
    Examples:
      - /tmp/kshr-debug.sock          -> "kshr DEV"
      - /tmp/kshr-debug-foo.sock      -> "kshr DEV foo"
      - ~/Library/Application Support/kshr/kshr.sock -> "kshr"
      - /tmp/kshr-foo.sock            -> "kshr foo"
    """
    base = Path(socket_path).name
    if base.startswith("kshr-debug") and base.endswith(".sock"):
        suffix = base[len("kshr-debug") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"kshr DEV {suffix[1:]}"
        return "kshr DEV"
    if base.startswith("kshr") and base.endswith(".sock"):
        suffix = base[len("kshr") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"kshr {suffix[1:]}"
        return "kshr"
...
def test_ctrl_enter_keybind(client: kshr) -> tuple[bool, str]:
...
def run_tests() -> int:
    print("=" * 60)
    print("kshr Ctrl+Enter Keybind Test")
    print("=" * 60)
    print()

...
def _expect_not_supported(c: kshr, method: str, params: dict) -> None:
    try:
        ...
        if "not_supported" in text:
            return
        raise kshrError(f"Expected not_supported for {method}, got: {text}")
    raise kshrError(f"Expected not_supported for {method}, but call succeeded")

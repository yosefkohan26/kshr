...
DEFAULT_SOCKET_PATHS = ["/tmp/kshr-debug.sock", "/tmp/kshr.sock"]
...
def _extract_segment_lines(
    ...
    *,
    ...
    if not saw_start:
        raise kshrError(f"start marker not found in scrollback: {start_marker}")
    if require_end and not saw_end:
        raise kshrError(f"end marker not found in scrollback: {end_marker}")
    return out


def _run_once(socket_path: str) -> int:
    workspace_id = ""
    fixture_dir = Path(tempfile.mkdtemp(prefix="kshr-ls-resize-regression-"))
    try:
        with kshr(socket_path) as client:
            ...
            start_marker = f"KSHR_LS_SCROLLBACK_START_{secrets.token_hex(4)}"
            end_marker = f"KSHR_LS_SCROLLBACK_END_{secrets.token_hex(4)}"
...
def main() -> int:
    env_socket = os.environ.get("KSHR_SOCKET")
    if env_socket:
    ...
    if last_error is not None:
        raise last_error
    raise kshrError("No socket candidates configured")

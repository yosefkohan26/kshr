...
LAST_SOCKET_HINT_PATH = Path("/tmp/kshr-last-socket-path")
...
def main() -> int:
    ...
    _must("kshr" in version_out, f"--version output should mention kshr: {version_out!r}")

    # Debug builds should auto-resolve the active debug socket via /tmp/kshr-last-socket-path
    # when KSHR_SOCKET_PATH is not set.
    ...
    try:
        ...
        auto_env.pop("KSHR_SOCKET_PATH", None)
        auto_env.pop("KSHR_SOCKET", None)
    ...
    ping_proc = _run([cli, "--socket", SOCKET_PATH, "--password", "ignored-in-kshronly", "ping"])

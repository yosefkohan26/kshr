...
<!doctype html>
<html>
  <head><title>kshr-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("kshr" in str(out.get("value", "")), f"Expected #out text to include 'kshr': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "kshr", f"Expected eval value 'kshr': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("kshr-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on kshr webview with ref handles")
    return 0

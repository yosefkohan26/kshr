...
def main() -> int:
    token = "KSHR_PALETTE_FOCUS_PROBE_9412"
    restore_token = "KSHR_PALETTE_RESTORE_PROBE_7731"
    ...
    with kshr(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise kshrError("typed probe text leaked into terminal while palette is open")
        ...
        if restore_token not in restore_text:
            raise kshrError("terminal did not receive typing after closing command palette")

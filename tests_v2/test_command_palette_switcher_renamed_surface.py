...
def _rename_surface(client: kshr, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: kshr, workspace_id: str) -> str:
...
def main() -> int:
    with kshr(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise kshrError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise kshrError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise kshrError("switcher returned no rows for renamed surface query")

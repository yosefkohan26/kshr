...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/kshr-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/kshr-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["KSHR_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["KSHR_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["KSHR_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["KSHR_SOCKET_PATH"] = socketPath
        app.launchEnvironment["KSHR_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["KSHR_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["KSHR_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["KSHR_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "kshr_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["KSHR_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["KSHR_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["KSHR_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["KSHR_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["KSHR_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("kshr-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("kshr-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("kshr-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("kshr-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveKshrCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled kshr CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `kshr notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected kshr to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `kshr notify` command to finish without foregrounding kshr. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected kshr to remain in background after bundled `kshr notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `kshr notify` launched from the in-app shell to succeed. " +
    ...
    private func waitForKshrPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runKshrCommand(
        ...
        let result = runKshrCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runKshrCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: KshrCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["KSHRTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveKshrCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled kshr CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeKshrCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled kshr CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["kshr"] + args
        let fallbackResult = executeKshrCommand(
    ...
    private enum KshrCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveKshrCLIPaths(strategy: KshrCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["KSHR_UI_TEST_CLI_PATH", "KSHRTERM_CLI"] {
        ...
        candidates.append("/tmp/kshr-\(launchTag)/Build/Products/Debug/kshr DEV.app/Contents/Resources/bin/kshr")
        candidates.append("/tmp/kshr-\(launchTag)/Build/Products/Debug/kshr.app/Contents/Resources/bin/kshr")
        if strategy == .any {
            candidates.append("/tmp/kshr-\(launchTag)/Build/Products/Debug/kshr")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: KshrCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/kshr DEV.app/Contents/Resources/bin/kshr")
        candidates.append("\(productsDir)/kshr.app/Contents/Resources/bin/kshr")
        if strategy == .any {
            candidates.append("\(productsDir)/kshr")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/kshr")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "kshr" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeKshrCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run kshr command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested KSHR_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/kshr-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/kshr-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/kshr.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("kshr.sock", isDirectory: false)
            .path ?? "/tmp/kshr.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("kshr") && $0.hasSuffix(".sock") }

...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/kshr-wait-for-") {
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"KSHR_CLAUDE_TEAMS_KSHR_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "KSHR_SOCKET_PATH", "KSHR_SOCKET", "TERM_PROGRAM",
		"KSHR_WORKSPACE_ID", "KSHR_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("KSHR_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("KSHR_SOCKET_PATH = %q", os.Getenv("KSHR_SOCKET_PATH"))
	}
	...
	if os.Getenv("KSHR_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("KSHR_WORKSPACE_ID = %q", os.Getenv("KSHR_WORKSPACE_ID"))
	}
	if os.Getenv("KSHR_SURFACE_ID") != "surf-789" {
		t.Errorf("KSHR_SURFACE_ID = %q", os.Getenv("KSHR_SURFACE_ID"))
	}
	// Verify extra env

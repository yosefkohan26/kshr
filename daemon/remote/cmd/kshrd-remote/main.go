...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "kshrd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  kshrd-remote version")
	_, _ = fmt.Fprintln(w, "  kshrd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  kshrd-remote cli <command> [args...]")
}
...
func (w *stdioFrameWriter) writeResponse(resp rpcResponse) error {
	// Responses are protocol boundaries — always flush immediately.
	return w.writeJSONFrame(resp, true)
}

func (w *stdioFrameWriter) writeEvent(event rpcEvent) error {
	// Stream data events are high-frequency; buffer them and let the
	// caller or a subsequent response/eof/error flush the batch.
	flush := event.Event != "proxy.stream.data"
	return w.writeJSONFrame(event, flush)
}

func (w *stdioFrameWriter) writeJSONFrame(payload any, flush bool) error {
	...
	// Flush on protocol boundaries or when the buffer is getting large.
	if flush || w.writer.Buffered() >= 32768 {
...
func (s *rpcServer) streamPump(streamID string, conn net.Conn) {
	...
	// Use a larger buffer and coalesce consecutive reads into a single
	// event when data is arriving faster than we can flush.
	...
	for {
		...
		// Flush accumulated data as a single event when we have data and
		// either an error occurred or the read filled less than the buffer
		// (indicating the kernel buffer is drained for now).
		if len(accumulated) > 0 && (readErr != nil || n < readBufSize) {
			_ = s.frameWriter.writeEvent(rpcEvent{
				Event:      "proxy.stream.data",
				StreamID:   streamID,
				DataBase64: base64.StdEncoding.EncodeToString(accumulated),
			})
			// Explicitly flush after coalesced data events.
			s.frameWriter.mu.Lock()
			_ = s.frameWriter.writer.Flush()
			s.frameWriter.mu.Unlock()

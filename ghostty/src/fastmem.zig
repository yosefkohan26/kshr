...
/// Moves the last `n` items to the start.
/// Generalization of `rotateOnceR` to N items.
///
/// e.g. rotateR(3, `0 1 2 3 4 5`) -> `3 4 5 0 1 2`.
///
/// Uses a single memmove + memcpy via a stack buffer for small `n`,
/// falling back to std.mem.rotate for larger rotations.
pub inline fn rotateR(comptime T: type, items: []T, n: usize) void {
    if (n == 0 or n == items.len) return;
    if (n == 1) return rotateOnceR(T, items);
    std.mem.rotate(T, items, items.len - n);
}

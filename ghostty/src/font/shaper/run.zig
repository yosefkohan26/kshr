...
/// A buffered codepoint entry stored during run iteration, to be replayed
/// into the shaper hooks only on cache miss.
const PendingCodepoint = struct {
    cp: u32,
    cluster: u32,
};
...
pub const RunIterator = struct {
    ...
    i: usize = 0,
    /// Cached trimmed row-end (one past the last non-empty cell).
    /// Computed lazily on first call to next() and reused for all
    /// subsequent calls, avoiding a per-run O(cols) reverse scan.
    cached_max: ?usize = null,

    /// Codepoints collected during the last next() call. These are only
    /// replayed into the shaper hooks when populateHooks() is called,
    /// allowing callers to skip buffer population on cache hits.
    pending_codepoints: std.ArrayListUnmanaged(PendingCodepoint) = .{},

    /// Replay the buffered codepoints from the last next() call into the
    /// shaper hooks (prepare, addCodepoint for each entry, finalize).
    /// This must be called before shape() on a cache miss.
    pub fn populateHooks(self: *RunIterator) !void {
        self.hooks.prepare();
        for (self.pending_codepoints.items) |entry| {
            try self.hooks.addCodepoint(entry.cp, entry.cluster);
        }
        self.hooks.finalize();
    }

    pub fn deinit(self: *RunIterator, alloc: Allocator) void {
        self.pending_codepoints.deinit(alloc);
    }
    ...
    pub fn next(self: *RunIterator, alloc: Allocator) !?TextRun {
        ...
        // Cached so we only reverse-scan once per iterator lifetime.
        const max: usize = self.cached_max orelse max: {
            const m: usize = for (0..cells.len) |i| {
                const rev_i = cells.len - i - 1;
                if (!cells[rev_i].isEmpty()) break rev_i + 1;
            } else 0;
            self.cached_max = m;
            break :max m;
        };
        ...
        // Clear the pending codepoints buffer for this new run.
        self.pending_codepoints.clearRetainingCapacity();
    ...
    fn addCodepoint(self: *RunIterator, alloc: Allocator, hasher: anytype, cp: u32, cluster: u32) !void {
        ...
        try self.pending_codepoints.append(alloc, .{ .cp = cp, .cluster = cluster });
    }

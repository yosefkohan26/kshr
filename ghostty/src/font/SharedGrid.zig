...
/// Mutex protecting the resolver and its underlying collection from
/// concurrent mutation. Font discovery and deferred face loading are
/// expensive and must not be serialized behind the main RwLock, which
/// also guards the (read-hot) codepoint and glyph caches. By using a
/// separate mutex we allow cache readers to proceed while one thread
/// performs fallback discovery or face loading.
resolver_lock: std.Thread.Mutex = .{},
...
pub fn getIndex(
    ...
    // only requires a read lock on the cache.
    {
    ...
    // Slow path: we need to resolve this codepoint. The expensive work
    // (font discovery, deferred face loading) is done under the resolver
    // mutex, which does NOT block cache readers. The main RwLock is only
    // taken briefly at the end to insert the result into the cache.
    //
    // Double-checked locking: after acquiring the resolver lock, we
    // re-check the cache (another thread may have resolved this codepoint
    // while we were waiting for the resolver lock).

    self.resolver_lock.lock();
    defer self.resolver_lock.unlock();

    // Double-check: re-read the cache under a shared lock. Another
    // resolver thread may have already inserted this codepoint.
    {
        self.lock.lockShared();
        defer self.lock.unlockShared();
        if (self.codepoints.get(key)) |v| return v;
    ...
    // Perform the expensive resolver work (font discovery, codepoint
    // map lookup, etc.) outside the main grid lock. Only the resolver
    // mutex is held, so concurrent cache reads are not blocked.
    ...
    if (value) |idx| preload: {
        ...
        // Load the face in case its deferred. If this fails then we
        // would've failed to load it in the future anyways so we want
        // to avoid caching a broken index.
        _ = try self.resolver.collection.getFace(idx);
    }

    // Now take the write lock briefly to insert into the cache.
    self.lock.lock();

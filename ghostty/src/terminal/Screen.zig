...
/// Batched version of cursorScrollAbove that scrolls N lines at once.
/// This avoids the O(N) cost of calling cursorScrollAbove in a loop,
/// which would do N separate grow + rotate operations.
///
/// Precondition: cursor must be on the bottom row of the screen
/// (cursor.y == pages.rows - 1), which is the common case when
/// scrollUp is used with no bottom margin.
pub fn cursorScrollAboveN(self: *Screen, count: usize) !void {
    assert(count > 0);
    assert(self.cursor.y == self.pages.rows - 1);

    if (count == 1) return try self.cursorScrollAbove();
    ...
    // If we have no scrollback, we need to shift rows up and clear.
    if (self.no_scrollback) {
        if (self.pages.rows == 1) {
            // Single-row screen: just clear.
            const page: *Page = &self.cursor.page_pin.node.data;
            self.clearCells(
                page,
                self.cursor.page_row,
            ...
            // Erase `count` rows from the top, shifting everything up.
            const erase_count = @min(count, self.pages.rows);
            for (0..erase_count) |_| {
                const old_pin = self.cursor.page_pin.*;
                try self.pages.eraseRow(.{ .active = .{} });
                self.cursor.page_pin.* = old_pin;
        ...
        // Grow pages by `count` rows. Each grow() may allocate new pages
        // or prune scrollback. We must track cursor pin changes.
        for (0..count) |_| {
            const old_pin = self.cursor.page_pin.*;
            _ = try self.pages.grow();

            // Update cursor pin: if the page was pruned, grow() already
            // adjusted the pin; otherwise move down by 1.
            const page_pin = if (old_pin.node == self.cursor.page_pin.node)
                self.cursor.page_pin.down(1).?
            else reuse: {
                var pin = self.cursor.page_pin.*;
        ...
        // Clear the new rows that are now at the cursor position.
        // After growing, the last `count` rows of the active area are
        // the new blank rows. Since cursor is at the bottom, these are
        // the rows from cursor.y - count + 1 to cursor.y.
        if (self.cursor.style.bg_color != .none) {
            // Apply bg color to the new rows.
            for (0..count) |i| {
                if (self.cursor.page_pin.up(@intCast(i))) |pin| {
                    const page: *Page = &pin.node.data;
                    const rac = pin.rowAndCell();
                    self.clearCells(page, rac.row, page.getCells(rac.row));
                }
            }
        }
    }

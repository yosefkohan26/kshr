...
pub fn scrollUp(self: *Terminal, count: usize) !void {
        ...
        self.scrolling_region.right == self.cols - 1)
    {
        ...
        // Use the batched path when cursor is at the bottom of the screen
        // and count > 1. This avoids N separate grow+rotate iterations.
        if (adjusted_count > 1 and self.scrolling_region.bottom == self.rows - 1) {
            try self.screens.active.cursorScrollAboveN(adjusted_count);
        } else {

...
```
App path:
  /Users/someone/Library/Developer/Xcode/DerivedData/kshr-my-tag/Build/Products/Debug/kshr DEV my-tag.app
```

**Claude Code** outputs:
```markdown
=======================================================
[kshr DEV my-tag.app](file:///Users/someone/Library/Developer/Xcode/DerivedData/kshr-my-tag/Build/Products/Debug/kshr%20DEV%20my-tag.app)
=======================================================
```
...
```
=======================================================
[my-tag: file:///Users/someone/Library/Developer/Xcode/DerivedData/kshr-my-tag/Build/Products/Debug/kshr%20DEV%20my-tag.app](file:///Users/someone/Library/Developer/Xcode/DerivedData/kshr-my-tag/Build/Products/Debug/kshr%20DEV%20my-tag.app)
=======================================================
```

Never use `/tmp/kshr-<tag>/...` app links in chat output.

After making code changes, always use `reload.sh --tag` to build. **Never run bare `xcodebuild` or `open` an untagged `kshr DEV.app`.** Untagged builds share the default debug socket and bundle ID with other agents, causing conflicts and stealing focus.
...
```bash
xcodebuild -project GhosttyTabs.xcodeproj -scheme kshr -configuration Debug -destination 'platform=macOS' -derivedDataPath /tmp/kshr-<your-tag> build
```
...
When rebuilding kshrd for release/bundling, always use ReleaseFast:

```bash
cd kshrd && zig build -Doptimize=ReleaseFast
```
...
`reloads` = kill and launch the Release app as "kshr STAGING" (isolated from production kshr):
...
```bash
tail -f "$(cat /tmp/kshr-last-debug-log-path 2>/dev/null || echo /tmp/kshr-debug.log)"
```

- Untagged Debug app: `/tmp/kshr-debug.log`
- Tagged Debug app (`./scripts/reload.sh --tag <tag>`): `/tmp/kshr-debug-<tag>.log`
- `reload.sh` writes the current path to `/tmp/kshr-last-debug-log-path`
- `reload.sh` writes the selected dev CLI path to `/tmp/kshr-last-cli-path`
- `reload.sh` updates `/tmp/kshr-cli` and `$HOME/.local/bin/kshr-dev` to that CLI
...
- To add a debug toggle or visual option: create an `NSWindowController` subclass with a `shared` singleton, add it to the "Debug Windows" menu in `Sources/kshrApp.swift`, and add a SwiftUI view with `@AppStorage` bindings for live changes.
- When the user says "debug menu" or "debug window", they mean this menu, not `defaults write`.

## Pitfalls

- **Custom UTTypes** for drag-and-drop must be declared in `Resources/Info.plist` under `UTExportedTypeDeclarations` (e.g. `com.splittabbar.tabtransfer`, `com.kshr.sidebar-tab-reorder`).
...
- **Typing-latency-sensitive paths** (read carefully before touching these areas):
  ...
  - `TerminalSurface.forceRefresh()` in `GhosttyTerminalView.swift`: only for geometry changes, attach, reveal, and focus recovery. Do not call per-keystroke. Do not add allocations, file I/O, or formatting here.
...
- **Shortcut policy:** Every new kshr-owned keyboard shortcut must be added to `KeyboardShortcutSettings`, visible/editable in Settings, supported in `~/.config/kshr/settings.json`, and documented in the keyboard shortcut and configuration docs.
...
- **E2E / UI tests:** trigger via `gh workflow run test-e2e.yml` (see kshrterm-hq CLAUDE.md for details)
- **Unit tests:** `xcodebuild -scheme kshr-unit` is safe (no app launch), but prefer CI
- **Python socket tests (tests_v2/):** these connect to a running kshr instance's socket. Never launch an untagged `kshr DEV.app` to run them. If you must test locally, use a tagged build's socket (`/tmp/kshr-debug-<tag>.sock`) with `KSHR_SOCKET=/tmp/kshr-debug-<tag>.sock`
- **Never `open` an untagged `kshr DEV.app`** from DerivedData. It conflicts with the user's running debug instance.
...
```bash
cd ghostty
git remote -v  # origin = upstream, yosefkohan26 = fork
git checkout -b <branch>
git add <files>
git commit -m "..."
git push yosefkohan26 <branch>
```
...
gh run watch --repo yosefkohan26/kshr
```

Notes:
- Requires GitHub secrets: `APPLE_CERTIFICATE_BASE64`, `APPLE_CERTIFICATE_PASSWORD`,
  `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, `APPLE_TEAM_ID`.
- The release asset is `kshr-macos.dmg` attached to the tag.
- README download button points to `releases/latest/download/kshr-macos.dmg`.
